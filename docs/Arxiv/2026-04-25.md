...
TL;DR: 本文提出AITP模型，结合多模态链式思维（MCoT）与检索增强生成（RAG），实现基于交通法规的事故责任推理与分配，并构建了涵盖10项任务、超6.7万视频的DecaTARA基准。


<details>
  <summary>Details</summary>
Motivation: 现有交通事故检测与理解工作缺乏对因果推理和法律知识整合的支持，难以胜任更复杂的事故责任分配（TARA）任务。

Method: 提出多模态大语言模型AITP，引入Multimodal Chain-of-Thought（MCoT）提升多步推理能力，并通过Retrieval-Augmented Generation（RAG）融合交通法规知识；同时构建DecaTARA多任务基准。

Result: AITP在责任分配、事故检测与理解等任务上均达到SOTA性能。

Conclusion: AITP建立了以推理驱动的多模态交通分析新范式，推动AI在交通执法辅助中的实际应用。
...
TL;DR: 本文提出了AFRILANGDICT词典和AFRILANGEDU数据集，用于训练面向非洲低资源语言的AI辅助语言教学模型AFRILANGTUTOR，并在多个语言上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决非洲大陆许多本地语言缺乏足够训练资源、难以开发AI语言学习系统的问题。

Method: 构建包含19.47万条非洲语言-英语词条的AFRILANGDICT词典，基于其自动生成7.89万条多轮师生问答对构成AFRILANGEDU数据集；使用该数据集对Llama-3-8B-IT和Gemma-3-12B-IT两个多语言大模型进行监督微调（SFT）和直接偏好优化（DPO）。

Result: 在10种非洲语言上，经AFRILANGEDU训练的模型显著优于基线模型；SFT与DPO联合使用带来1.8%–15.5%的性能提升（LLM-as-a-judge评估）。

Conclusion: AFRILANGDICT与AFRILANGEDU为低资源语言AI教育提供了可扩展、可验证的数据构建范式，所训练的AFRILANGTUTOR模型有效提升了低资源语言教学能力，所有资源已开源。
...
Method: 提出分层策略优化（HPO）框架，在已有基于不完美SFT数据训练的模型上进行后训练；设计兼顾翻译质量与延迟的分层奖励函数，并结合不同质量奖励、分层结构和分段策略进行优化。

Result: 在英-中/德/日SST任务上，于1.5秒延迟下实现COMET分数提升超+7、MetricX分数提升+1.25；消融实验验证了各组件有效性。

Conclusion: HPO无需高质量对话标注即可有效提升SST性能，在质量-延迟权衡上优于依赖强监督的方法，为低资源场景下的SST优化提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言推理模型（LRMs）存在推理步骤冗余、效率低下问题，且对不同推理步骤的高层作用及对正确答案贡献的理解尚不充分。

Method: 提出TRACES框架，实时标注推理步骤类型，监测模型在推理过程中的行为变化（如达到正确答案后的行为偏移），并据此设计可解释的自适应早期停止准则。

Result: 在MATH500、GSM8K、AIME、MMLU和GPQA五个基准上验证了TRACES的有效性，实现了20%–50%的token减少，同时准确率与标准生成相当。

Conclusion: TRACES为提升LRMs推理效率与可解释性提供了轻量、通用且实用的新范式，揭示了推理步骤类型与模型行为动态之间的关键联系。
...
Method: 将文本嵌入（句级或词级）视为语义信号，应用离散小波变换（DWT）进行多分辨率分解，提取全局（近似）与局部（细节）成分；所得紧凑表示可直接作为摘要，或用于引导LLM生成。

Result: 在临床与法律基准测试中，ROUGE-L与GPT-4o基线相当，但BERTScore提升超2%，Semantic Fidelity提升超4%，法律任务中事实一致性增强，METEOR大幅上升；Fidelity最高达97%，验证其语义去噪与事实锚定能力。

Conclusion: DWT是一种轻量、通用、可靠的多分辨率摘要技术，能有效缓解LLM在长文档和专业领域摘要中的幻觉与语义失真问题。
...
Method: 在200例合成患者（共4000次推理）的受控基准上，对比四种FHIR序列化策略（Raw JSON、Markdown Table、Clinical Narrative、Chronological Timeline）在五个开源模型（Phi-3.5-mini至Llama-3.3-70B）上的表现，评估F1、精确率与召回率，并分析错误模式与规模效应。

Result: 序列化策略对≤8B模型影响巨大（如Mistral-7B上Clinical Narrative比Raw JSON高19 F1点），但在70B模型上Raw JSON最优（F1=0.9956）；所有组合中精度均高于召回率，遗漏是主要错误类型；小模型处理能力上限为7–10种并发用药；BioMistral-7B完全失效，表明仅领域预训练不足够。

Conclusion: 应依据模型规模选择FHIR序列化格式：≤8B用Clinical Narrative，≥70B用Raw JSON；临床部署需优先防范遗漏错误，并提升小模型对多药联用患者的覆盖能力。
...
TL;DR: 本文提出了一种加权损失函数，以提升医学报告生成中视觉-语言模型的数据利用效率，尤其在眼科报告生成任务中，仅用十分之一的数据即可达到相近性能。
...
Method: 采用加权损失函数替代标准交叉熵损失，对具有更高临床重要性的语义显著token赋予更大权重。

Result: 在眼科报告生成实验中，该方法在多种数据规模下均提升了训练效率，仅需十分之一的训练数据即可达到与基线相当的报告质量。

Conclusion: 加权损失是一种简单而有效的策略，可显著提升医学VLMs在低资源场景下的数据效率。
...
TL;DR: 本研究利用MARBERT模型预测阿拉伯语推文中的表情符号，构建了一个包含多种阿拉伯方言的11379条推文语料库，并通过预处理和微调MARBERT实现了75%的整体准确率，表明该方法在低资源、多方言语言上具有潜力但仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语是低资源、多方言语言，现有模型在表情符号预测任务中表现有限，需探索适配其语言特性的机器学习方法。

Method: 收集11379条阿拉伯语推文并清洗为8695条有效样本；将推文按14类表情符号编码为标签；设计可解释的预处理流程提取词汇特征；微调MARBERT模型进行表情符号预测；采用精确率、召回率和F1分数评估性能。

Result: MARBERT模型整体准确率达到0.75，在精度、召回率和F1分数上表现良好。

Conclusion: 结果具有前景，但针对阿拉伯语等低资源、多方言语言，仍需进一步改进机器学习模型（包括MARBERT）。
...
TL;DR: GRISP是一种基于SPARQL骨架引导循环选择IRI的新方法，利用微调的小型语言模型（SLM）实现知识图谱上的自然语言问答。


<details>
  <summary>Details</summary>
Motivation: 提升知识图谱上自然语言问答的准确性和效率，尤其在资源受限场景下利用小型语言模型替代大型模型。

Method: 提出GRISP框架：先用SLM生成自然语言形式的SPARQL骨架，再结合知识图谱约束迭代地重排序并填充IRI占位符；SLM联合训练于骨架生成与列表式重排序任务。

Result: 在Wikidata和Freebase标准基准上，GRISP在可比设置下优于其他前沿方法。

Conclusion: 证明了小型语言模型通过结构化引导（SPARQL骨架+约束驱动IRI选择）可在知识图谱问答任务中达到甚至超越大模型性能。
...
TL;DR: 本文提出Introspective and Interactive Visual Grounding (IVG)框架，通过结合规范驱动的内省和视图驱动的交互来提升视觉语言模型在图表理解任务中的性能，并构建了iPlotBench基准用于无偏评估。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图表理解中存在误读数值、幻觉细节和混淆重叠元素等问题，根源在于仅依赖像素信息导致的‘像素瓶颈’，忽略了图表背后的结构化规范。

Method: 提出IVG框架，包含两部分：(1) 基于规范的内省，即查询图表底层规范以获取确定性证据；(2) 基于视图的交互，即通过操作图表视图来消除视觉歧义；同时构建iPlotBench基准进行评估。

Result: 实验表明，内省提升了数据重建保真度，内省与交互结合使问答准确率达到0.81，在重叠几何图形上提升6.7%；并在实际部署中验证了其自主探索与实时人机协作能力。

Conclusion: IVG有效突破了像素瓶颈，通过融合结构化规范与交互能力显著提升了VLM对交互式图表的理解鲁棒性与准确性。
...
<details>
  <summary>Details</summary>
Motivation: 手动编码大规模课堂话语费时费力，难以支撑对科学课堂中学生推理模式的规模化分析，亟需自动化方法以理解知识建构机制并优化教学实践。

Method: 构建自动化话语分析系统（ADAS），在CDAT框架基础上联合分类话语类型（Utterance Type）和推理成分（Reasoning Component）；采用分层重采样、面向少数类的LLM合成数据增强、双探针RoBERTa-base微调；辅以零-shot GPT-5.4基线评估；并开展UT×RC共现分析、认知复杂度指数（CCI）、滞后序列分析及IRF链分析。

Result: LLM增强显著提升UT中小类识别效果；RC任务结构简单，甚至词法基线即可较好处理；零-shot GPT-5.4在UT和RC上宏观F1分别为0.467和0.476；分析发现教师Feedback-with-Question（Fq）是最稳定的触发学生推理性回答（SR-I）的话语前导。

Conclusion: ADAS系统可有效支持课堂推理模式的规模化、细粒度分析；LLM数据增强对缓解小样本类别偏差具有实际价值；教师Fq话语是促进学生高阶推理的关键干预点，为教学设计提供实证依据。
...
TL;DR: 本文提出了一种多槽位探测方法，发现语言模型在单个token中通过正交的'当前实体'和'先前实体'槽位分别编码两个实体信息；前者用于显式事实检索，后者支持关系推理（如归纳、冲突检测），但模型并不利用后者回答事实问题；前沿模型展现出更优的双绑定解析能力，暗示其具备更复杂的绑定机制。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型如何在上下文中同时维护多个实体及其属性的绑定关系，特别是单个token能否承载多个实体的绑定信息。

Method: 提出多槽位探测方法，将单个token残差流激活解耦为'当前实体'和'先前实体'两个正交槽位，并分析其功能角色与模型行为关联。

Result: 发现两个槽位功能分离：'当前实体'槽用于显式事实检索，'先前实体'槽支持关系推理（如实体归纳、相邻冲突检测）；尽管后者线性可解事实答案，模型却未使用；开放权重模型难以处理单token双主谓宾绑定句式，而前沿模型表现显著提升。

Conclusion: 模型中存在信息可用性与实际使用之间的鸿沟；当前/先前实体槽结构是支持需同时维持双重视角行为（如谄媚、欺骗）的自然表征基础。
...
TL;DR: 本文提出了一种主动视觉支架框架，通过将对话状态逐步转化为持久的视觉历史来缓解对话中因文本表示导致的‘表征模糊’问题，提升智能体对共享上下文的长期追踪能力。


<details>
  <summary>Details</summary>
Motivation: 当前对话智能体难以在超出即时上下文窗口的情况下持续维护共享上下文，文本表示易导致相似但不同的实体被压缩为可互换描述（即‘表征模糊’），造成虚假的语境接地感。

Method: 受人类心智意象启发，引入主动视觉支架框架，将对话状态增量式外化为可检索的持久视觉历史，并结合文本与视觉模态进行混合表征。

Result: 在IndiRef基准上验证：增量外化本身优于全对话推理；视觉支架进一步降低表征模糊、强化具体场景承诺；混合多模态表现最优。

Conclusion: 对话智能体应采用显式的多模态共同表征，融合描绘性（视觉）与命题性（文本）信息，以更可靠地维护共享上下文。
...
<details>
  <summary>Details</summary>
Motivation: 现有ASR偏见研究多聚焦于统计误差率，忽视系统失败对用户真实生活体验、情绪反应与心理负担的影响；本文旨在从人文视角探究技术失败如何塑造边缘方言使用者的日常实践与自我认知。

Method: 在亚特兰大、墨西哥湾沿岸、迈阿密海滩和图森四个代表不同英语方言的美国地区开展实地用户体验研究，结合访谈与开放式叙事进行定性分析。

Result: 发现用户普遍遭遇ASR文化失敏，需持续调整发音与表达方式（如语码转换、过度清晰发音、情绪管理）以勉强使用系统；尽管意识到系统未为其设计，仍常将失败内化为个人能力不足；其语言文化知识被技术标准排除在外。

Conclusion: 仅靠准确率等指标无法捕捉ASR偏见带来的关键伤害维度，包括应对技术排斥所需的情感劳动、持续自我监控的认知负担，以及母语变体使用者产生的无能感等心理代价；算法公平性评估亟需纳入用户体验与社会情感维度。
...
TL;DR: 本文提出了一种前缀语法变换方法，将前缀解析问题高效地归约为普通语法解析问题，并支持加权前缀概率计算与下一词权重向量的快速推导，为上下文无关语言建模和大模型语法约束生成提供了简洁通用的框架。


<details>
  <summary>Details</summary>
Motivation: 前缀解析在加权语境下可提供前缀概率，对上下文无关语言建模、心理语言学分析及大语言模型的语法约束生成至关重要，但现有方法常需定制化算法。

Method: 提出前缀语法变换：构造一个新语法，其生成的语言恰好是原语法所有字符串的前缀；再调用任意标准解析器处理该变换后语法；并基于自动微分策略计算下一词权重向量。

Result: 变换后的语法仅比原语法略大；可直接复用任意优化的通用解析器；显著简化前缀解析实现，同时支持高效前缀概率与下一词预测。

Conclusion: 该方法兼具理论优雅性与工程实用性，构成一种简单、通用且高效的前缀解析新框架。
...
TL;DR: 本文提出了一种两步式推理方法，通过先生成用户职业选择的‘理由’（基于教育与职业历史），再据此预测下一职业，显著提升了大语言模型在职业预测任务上的性能；该方法通过LLM-as-a-Judge构建高质量‘oracle reasons’并微调小模型，实验证明其效果媲美监督方法、优于无监督方法，且理由质量直接影响预测准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）未对齐职业发展路径及背后隐含的职业决策动因，导致其在未来职业预测任务中表现受限，亟需引入可解释、结构化的推理机制提升预测准确性与可信度。

Method: 提出两步推理框架：1）reason generator基于用户教育与职业历史生成概括其偏好的‘reason’；2）occupation predictor以该reason为输入预测下一职业；为解决LLM缺乏职业领域对齐问题，采用LLM-as-a-Judge筛选满足事实性、连贯性与实用性标准的高质量oracle reasons，并用于监督微调小型LLM同时完成reason生成与职业预测。

Result: 实验表明：(a) 该方法使LLM在下一职业预测准确率上接近全监督方法、超越无监督方法；(b) 单一联合微调的LLM优于分别微调的两个LLM；(c) 预测准确率与生成reason的质量呈正相关。

Conclusion: 引入显式、高质量reasoning可有效桥接用户历史与未来职业预测之间的语义鸿沟，微调小规模LLM执行端到端reason生成与预测是高效可行的路径，reason质量是性能关键瓶颈。
...
TL;DR: 本文提出SPIA基准，首次将文本匿名化评估单位从文本片段转向个体，揭示了现有方法在多主体场景下的保护不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本匿名化评估依赖于基于片段的指标，无法捕捉攻击者实际可推断的信息，并且假设单一数据主体，忽略了多主体场景。

Method: 提出了SPIA（Subject-level PII Inference Assessment）基准，包含675份法律和在线领域文档，并设计了新颖的主体级保护度量指标。

Result: 实验表明，即使90%以上的PII片段被掩码，主体级推断保护率仍低至33%；目标主体聚焦式匿名化使非目标主体暴露更严重。

Conclusion: 主体级基于推断的评估对确保现实世界中文本匿名化的安全性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型生成类人文本的能力引发滥用担忧，亟需可靠高效的LLM生成文本检测方法。

Method: 提出基于隐式奖励模型（IRM）的零样本检测方法，利用公开的指令微调模型和基础模型导出隐式奖励模型，无需偏好构建和任务特定微调。

Result: 在DetectRL基准上，IRM在零样本及监督方法中均取得更优的检测性能。

Conclusion: IRM是一种高效、免训练的零样本LLM生成文本检测方法，具有实用性和泛化潜力。
...
TL;DR: 本文提出了EngramaBench基准，用于评估大语言模型在长期对话记忆中的表现，并对比了图结构记忆系统Engrama、全上下文提示GPT-4o和向量检索记忆系统Mem0；结果表明Engrama在跨空间推理任务上优于全上下文方法，但全局综合得分略低，揭示了结构化记忆专业化与整体优化之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型助手需具备长期多轮对话中信息留存与推理能力，但现有评估缺乏系统性基准，亟需构建能覆盖多种记忆推理能力的评测框架。

Method: 构建了包含五个虚拟人物、一百段多轮对话及一百五十个问题的EngramaBench基准，涵盖事实回忆、跨空间整合、时间推理、对抗性拒绝回答和涌现式合成；对比评估了图结构记忆系统Engrama、GPT-4o全上下文提示和开源向量检索系统Mem0，三者均使用同一回答模型（GPT-4o）以隔离记忆架构影响。

Result: GPT-4o全上下文综合得分最高（0.6186），Engrama全球综合得分为0.5367，但在跨空间推理子任务上表现最优（0.6532 vs. 0.6291）；Mem0成本最低但性能最弱（0.4809）；消融实验显示Engrama的跨空间优势组件会降低其全局得分，暴露结构化记忆设计中的系统级权衡。

Conclusion: 结构化记忆系统（如Engrama）可在特定推理任务（如跨空间整合）上超越全上下文方法，但难以兼顾全局性能，提示未来研究需在专用性与通用性之间寻求平衡。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）的多表实体匹配框架LLM4MEM，通过多风格提示增强、传递共识嵌入匹配和密度感知剪枝三个模块，有效缓解语义不一致、匹配效率低和噪声实体问题，在6个数据集上F1平均提升5.1%。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练语言模型的多表实体匹配方法难以处理由数值属性变化引起的语义不一致问题，且面临多源数据带来的匹配效率下降与噪声实体干扰挑战。

Method: 提出LLM4MEM框架，包含：（1）多风格提示增强的LLM属性协调模块，缓解语义不一致；（2）传递共识嵌入匹配模块，提升匹配效率并解决嵌入与预匹配问题；（3）密度感知剪枝模块，优化匹配质量、抑制噪声实体。

Result: 在6个MEM数据集上实验表明，该方法F1值平均提升5.1%，显著优于基线模型。

Conclusion: LLM4MEM通过结合LLM强大语义理解能力与针对性设计的三个模块，有效提升了多表实体匹配的准确性、效率与鲁棒性，为MEM任务提供了新思路。
...
TL;DR: 本文提出PLOTTER框架，通过在事件图和角色图上执行评估-规划-修订循环，利用结构化图表示进行叙事规划，以提升大语言模型在长文本叙事生成中的全局连贯性、逻辑一致性和角色发展能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以维持叙事的全局连贯性、上下文逻辑一致性及平滑的角色发展，常生成结构断裂、单调的脚本。

Method: PLOTTER框架在结构图（事件图与角色图）上执行Evaluate-Plan-Revise循环，依据严格逻辑约束诊断并修复图拓扑问题，优化因果关系与叙事骨架，再进行完整文本生成。

Result: 实验表明，PLOTTER在多种叙事场景下显著优于代表性基线方法。

Conclusion: 在结构化图表示上进行叙事规划，而非直接在文本上操作，对提升大语言模型在复杂叙事生成中的长上下文推理能力至关重要。
...
TL;DR: 本文提出Response Pattern Similarity (RPS) 和 Action Graph Similarity (AGS) 两种新指标，用于量化大模型代理在非必要行为层面的同质化程度，揭示模型蒸馏导致的行为趋同现象，并验证其在区分教师特异性收敛与通用性能提升上的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有指标无法区分任务必需行为与反映模型自主偏好的非必需行为模式，而模型蒸馏正导致LLM代理行为高度同质化，亟需可解释、可解耦的评估工具。

Method: 提出两个互补指标：RPS（衡量响应文本层面的言语一致性）和AGS（将工具调用建模为有向图，衡量动作图结构相似性），并在τ-Bench/τ²-Bench上对18个模型进行跨厂商、跨家族评估；辅以控制蒸馏实验验证AGS有效性。

Result: 同族模型AGS显著高于跨族模型（+5.9pp）；Kimi-K2（thinking）在AGS节点和依赖相似度上分别达82.6%和94.7%，超越Anthropic Opus 4.1；RPS与AGS相关性中等（r=0.491），表明二者捕捉不同行为维度。

Conclusion: RPS和AGS能有效分离并量化LLM代理生态中的非必要行为趋同，为诊断蒸馏副作用、促进多样性发展提供可操作的评估框架。
...
TL;DR: 本文发现，先用音乐（钢琴演奏）预训练Transformer模型，再用于语言学习，能显著加速语言习得，尤其在小模型上效果明显。


<details>
  <summary>Details</summary>
Motivation: 探索结构化人类创造性产出（如音乐）是否可作为小型语言模型高效预训练的替代数据源，缓解小模型在有限数据下预训练效果不佳的问题。

Method: 采用分阶段发育式预训练流程（音乐→诗歌→散文），在MAESTRO钢琴数据集上预训练小型Transformer（d∈{16,32,64}），对比随机初始化及不同数据规模、模态组合的效果，通过困惑度、收敛速度、多种子统计检验和缩放实验进行评估。

Result: 音乐预训练带来17.5%困惑度下降（p<0.001）；音乐与诗歌分别优化模型内部计算与嵌入层；该优势在收敛后仍稳定存在（5.5%持续差距，p=0.017）；真实音乐仅需三分之一数据即达合成模式上限；最优预训练数据量随模型容量增大而增加。

Conclusion: 模型容量决定最优预训练数据规模，结构化创意数据（如音乐）是小型语言模型高效预训练的有效基底；但对现代大规模预训练的普适性尚需更大规模实验验证。
...
TL;DR: 本文研究了预训练大语言模型（LLM）在语音识别中作为解码器时对不同人口统计群体的公平性影响，发现音频编码器设计比LLM规模对识别的公平性和鲁棒性影响更大。


<details>
  <summary>Details</summary>
Motivation: 随着预训练大语言模型取代语音识别中的任务特定解码器，其文本先验是否会使识别在不同人群间更公平或更偏见，成为一个关键问题。

Method: 评估了跨越三代架构的9个模型（CTC无LM、编码器-解码器隐式LM、LLM显式预训练解码器），在Common Voice 24和Fair-Speech数据集上，覆盖5个人口统计维度（种族、口音、性别、年龄、母语），共约43,000条干净及12种声学退化条件下的语音样本，总计216次推理实验。

Result: LLM解码器未加剧种族偏差（Granite-8B在族裔公平性上最优）；Whisper在印度口音语音上出现病理性幻觉；音频压缩比LLM规模更能预测口音公平性；严重退化反而缩小公平差距，但静音注入显著放大Whisper口音偏差；掩码下Whisper陷入重复循环，而显式LLM解码器插入错误少38倍且几乎无重复；高压缩音频编码（Q-former）会重引发LLM解码器的重复问题。

Conclusion: 音频编码器设计而非LLM缩放，是实现公平且鲁棒语音识别的主要调控杠杆。
...
TL;DR: 本文检验了K-way能量探针在预测编码网络中约等于log-softmax margin单调函数这一结论对交叉熵损失移除的敏感性，发现交叉熵是该分解的关键经验支撑成分，其影响主要来自logit尺度效应（66%）和尺度不变的表征排序优势（34%）。


<details>
  <summary>Details</summary>
Motivation: 验证Cacioli (2026)提出的K-way能量探针与log-softmax margin之间理论约等关系在去除交叉熵（CE）损失时的鲁棒性，探究CE在该分解中的实际作用。

Method: 通过控制实验比较三种模型：标准预测编码（PC）使用CE、标准PC改用MSE损失、以及双向预测编码（bPC）；在CIFAR-10上基于相同2.1M参数主干网络、10次随机种子重复实验；辅以操纵检验、温度缩放消融分析。

Result: 1）标准PC+CE下探针显著低于softmax（Δ=-0.082）；2）bPC下探针略超softmax（Δ=+0.008），但潜变量移动未显著增强；3）仅换MSE使gap减半（Δ=-0.037）；4）CE训练使logit范数达MSE/bPC的15倍；5）温度缩放可解释66% gap，剩余34%为CE带来的尺度不变排序优势。

Conclusion: 交叉熵损失是当前规模下K-way能量探针与softmax margin关系分解中关键的经验性支柱，其作用兼具logit尺度放大与内在表征质量提升两方面。
...
TL;DR: 本文提出Explainable Authorship Variational Autoencoder (EAVAE)，通过架构设计实现风格与内容的显式解耦，结合监督对比学习预训练和变分自编码器微调，并引入可解释判别器提升解耦效果与可解释性，在作者归属和AI生成文本检测任务上均取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以解决作者风格与文本内容之间的纠缠问题，导致模型依赖主题等虚假相关性，泛化能力差。

Method: 提出EAVAE框架：先用监督对比学习在多源作者数据上预训练风格编码器，再基于VAE结构使用独立的风格/内容编码器进行微调；引入新型判别器，不仅能判别风格/内容对是否同源，还能生成自然语言解释以增强解耦与可解释性。

Result: 在Amazon Reviews、PAN21、HRS等作者归属数据集上达到SOTA；在M4数据集的AI生成文本检测任务中展现优异的少样本学习能力。

Conclusion: EAVAE通过分离式架构设计与可解释判别机制，有效实现了风格-内容解耦，显著提升了跨域泛化性与任务性能，并增强了模型决策的可解释性。
...
TL;DR: 本研究使用FairNews数据集评估了13种大语言模型在多文档新闻摘要中的政治偏见，发现中等规模模型在公平性与效率上表现最优，提示词去偏效果因模型而异，实体情感维度最难消除偏见。


<details>
  <summary>Details</summary>
Motivation: 多文档新闻摘要系统日益普及，但其可能因政治立场不同而产生偏见，如观点表征不均、某些立场被过度强调或少数声音被系统性忽视，因此亟需全面评估其政治公平性。

Method: 基于带有政治倾向标签的完整新闻文章数据集FairNews，采用五种公平性指标，对13种大语言模型进行评测，并对比多种去偏干预方法（包括提示词工程和裁判式判断）。

Result: 中等规模模型在公平性与效率上整体优于更大模型；提示词去偏效果高度依赖具体模型；实体情感维度是所有干预策略中最难改善的公平性维度。

Conclusion: 多文档新闻摘要的公平性需通过多维评估框架衡量，并采取针对模型架构的定制化去偏策略，而非单纯扩大模型规模。
...
TL;DR: 本文提出了PolyChartQA数据集，用于多图表图像上的问答任务，并评估了九种多模态语言模型在该数据集上的性能，发现人类编写的问题比模型生成的问题更难，同时提出了一种提升准确率的提示方法。
...
Method: 构建了包含534张多图表图像和2694个问答对的PolyChartQA数据集，并在多种维度上评估九种多模态语言模型，同时提出一种改进的提示方法。

Result: 在人类编写的问题上，模型准确率比在模型生成问题上低27.4%；所提提示方法带来5.39%的准确率提升。

Conclusion: PolyChartQA填补了多图表理解领域的数据空白，揭示了当前多模态模型在真实场景问答中的局限性，并验证了提示工程的有效性。
...
TL;DR: 本文提出CARE框架，通过在希伯来语和阿拉伯语真实危机对话数据上微调开源大语言模型，生成符合心理干预原则的实时应答建议，提升低资源语言环境下心理咨询师的工作效率与服务质量。


<details>
  <summary>Details</summary>
Motivation: 全球心理健康问题加剧，心理咨询师面临过载压力，尤其在低资源语言（如希伯来语、阿拉伯语）和敏感领域中，现有大语言模型缺乏专业支持性语言与干预策略的建模能力。

Method: 提出CARE（Counselor-Aligned Response Engine）框架，分别对开源LLM在希伯来语和阿拉伯语的高有效性真实危机对话子集上进行微调；训练数据包含专业咨询师标注的高效干预会话，并利用完整对话历史建模情绪演变与互动结构。

Result: 实验表明，CARE在语义和策略层面比通用大语言模型更贴近专家咨询师的标准应答，显著提升响应的心理学一致性。

Conclusion: 基于专家验证数据的领域微调可有效增强大语言模型在低资源语言心理支持任务中的实用性，为缓解咨询师工作负荷、改善服务可及性提供可行技术路径。
...
Result: 最终系统在22个语言轨道上宏平均F1达0.796，平均准确率为0.826；单语专家在脚本差异大的语言上显著优于通用模型；NLLB-200增强效果有限，甚至损害形态丰富语言的表现。

Conclusion: 统一通用架构并非最优解；语言自适应模型选择策略更有效；跨语言增强需谨慎，尤其对形态复杂语言。
...
TL;DR: 本文提出VLAA-GUI框架，通过Completeness Verifier、Loop Breaker和Search Agent三大模块分别解决GUI智能体的早停、循环和未知流程问题，在OSWorld和WindowsAgentArena上达到SOTA性能，并部分超越人类水平。


<details>
  <summary>Details</summary>
Motivation: 自主GUI智能体面临早停（缺乏可验证成功证据）和重复循环（无恢复能力）两大根本挑战。

Method: 提出VLAA-GUI模块化框架：1）强制性Completeness Verifier在每步结束时基于UI可观测标准进行完成验证；2）强制性Loop Breaker通过多级过滤（交互模式切换、策略变更、反射信号绑定）打破循环；3）按需调用Search Agent（调用具搜索能力的LLM）和Coding/Grounding Agent处理未知流程与代码/精确定位任务。

Result: 在OSWorld（77.5%）和WindowsAgentArena（61.0%）上均达最优性能；Opus 4.5/4.6与Gemini 3.1 Pro中三者单次执行即超越人类水平（72.4%）；消融实验表明三模块持续提升强基线，弱基线在步数充足时受益更显著；Loop Breaker使易循环模型的无效步数减半。

Conclusion: VLAA-GUI通过结构化决策机制（Stop/Recover/Search）有效缓解GUI智能体的核心失效模式，验证了模块化、可验证、可恢复设计范式的有效性与通用性。
...
TL;DR: 本文提出Decoupled DiLoCo，一种异步分布式训练框架，通过解耦学习器、最小法定人数聚合与自适应容错机制，在大规模故障环境中实现零全局停机并保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现代大模型预训练依赖SPMD范式，易受硬件故障、瞬时延迟和同步开销影响，导致整体计算停滞和算力浪费。

Method: 提出Decoupled DiLoCo：将计算划分为多个独立learner执行本地优化；learner异步向中心synchronizer发送参数片段；synchronizer采用最小法定人数、自适应宽限期和动态token加权合并策略容错。

Result: 在百万级模拟芯片故障场景下实现严格零全局停机，显著提升训练goodput；在文本与视觉任务（含dense与MoE架构）上保持具竞争力的模型性能。

Conclusion: Decoupled DiLoCo成功突破SPMD同步瓶颈，验证了异步、解耦与混沌工程启发的容错设计对大规模语言模型训练的高效性与鲁棒性。
...
TL;DR: 本文探讨了大语言模型中的推理能力，提出其可能由更基础的操作（如回忆和状态跟踪）构成，并比较了注意力机制与混合架构（结合注意力检索和循环状态更新）在需要同时进行状态跟踪和回忆的任务上的表现。研究发现，推理增强显著提升了模型的有效操作范围，而混合架构在某些任务中对序列依赖性增加表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 推理常被视为单一能力，但其提升可能源于更基本的操作（如回忆和状态跟踪），因此需探究不同架构如何支持这些基础能力。

Method: 使用匹配的Olmo3 Transformer和混合模型（含指令微调和推理增强变体），在一组控制任务（涉及状态跟踪与回忆的组合，即基于状态的回忆）上进行评估。

Result: 推理增强带来最大整体提升，显著扩展模型有效处理难度的范围；在部分任务中，混合推理模型随序列依赖增强仍保持较强鲁棒性，而Transformer推理模型在难度超过阈值后性能急剧下降。

Conclusion: 推理标记和架构归纳偏置在计算过程中起不同作用：显式推理可扩展模型有效操作范围，但其效益取决于底层架构对持久状态传播的支持程度；当前结果仅为初步提示，需后续在更多模型、规模和任务上验证。
...
TL;DR: 本文研究了在合规检测（作为自然语言推理任务）中，通过有针对性的数据选择策略来缓解跨领域负迁移问题，发现包括Moore-Lewis交叉熵差、重要性加权和嵌入检索在内的多种数据选择方法能显著提升模型在不同法规间的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 由于法律文本的复杂性和多变性，针对某一法规训练的模型往往难以泛化到其他法规，亟需提升跨领域迁移的系统性方法。

Method: 将合规检测建模为自然语言推理（NLI）任务，比较四种源域数据选择策略：随机采样、Moore-Lewis交叉熵差、重要性加权和基于嵌入的检索，并系统调整所选数据比例以分析其对跨域适应的影响。

Result: 有针对性的数据选择策略（尤其是Moore-Lewis和嵌入检索）能显著缓解负迁移，提升模型在未见法规上的检测性能。

Conclusion: 数据选择是一种实用、可扩展且有效的策略，有助于实现面向异构法规的自动化合规检测。
...
TL;DR: 本文提出了一种用于多语言文本转语音（TTS）系统的受控多维成对评估框架，结合语言控制与感知驱动的标注，在10种印度语言上评估7个SOTA TTS模型，收集超12万条成对比较数据，并通过Bradley-Terry建模和SHAP分析构建多语言排行榜并解析模型优劣。


<details>
  <summary>Details</summary>
Motivation: 现有众包成对评估在TTS任务中因语言多样性与语音感知多维性而方差高，亟需更可控、可解释的多维评估方法。

Method: 构建融合语言控制（5K+原生及语码混用句子）与感知标注（6个维度）的多维成对评估框架；招募1900+母语评测者收集120K+比较数据；采用Bradley-Terry模型构建多语言排行榜，并用SHAP分析解释人类偏好及模型权衡。

Result: 成功建立覆盖10种印度语言的TTS多维评估基准；生成可比性强、可靠性经验证的多语言排行榜；揭示各模型在 intelligibility、expressiveness 等维度上的系统性优势与权衡。

Conclusion: 该框架显著提升TTS众包评估的稳定性与可解释性，为多语言语音合成的公平、细粒度评估提供了新范式。
...
TL;DR: 本文提出了OptiVerse基准测试集，涵盖随机优化、动态优化、博弈优化和最优控制等被忽视的领域，用于全面评估大语言模型（LLMs）在复杂优化任务中的能力；实验发现现有LLM在难题上表现差，主因是建模与逻辑错误；为此提出Dual-View Auditor Agent以提升建模准确性。


<details>
  <summary>Details</summary>
Motivation: 现有优化任务基准过于局限（仅数学规划与组合优化），无法全面评估LLM在更广泛复杂优化问题上的能力，亟需覆盖更多实际优化领域的综合性基准。

Method: 构建包含1000个问题的OptiVerse基准，覆盖随机优化、动态优化、博弈优化和最优控制四大新兴领域，并划分易、中、难三级；对22个不同规模LLM进行评测；开展错误分析并提出Dual-View Auditor Agent以缓解建模与逻辑错误。

Result: 22个LLM在Hard难度问题上性能显著下降，GPT-5.2和Gemini-3准确率均未超27%；错误分析表明建模与逻辑错误是主要瓶颈；Dual-View Auditor Agent有效提升了建模准确性且无明显时间开销。

Conclusion: OptiVerse填补了复杂优化任务评估的空白，揭示了当前LLM在建模能力上的根本局限，并通过新型审计代理机制为提升其优化求解能力提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在职位广告技能抽取任务中存在边界漂移、幻觉和长尾术语处理差等问题，尤其在跨领域场景下表现不佳。

Method: 提出SRICL框架：1）语义检索（SR）从ESCO中获取领域内标注句子与定义，构建格式约束提示；2）上下文学习（ICL）增强提示效果；3）监督微调（SFT）对齐输出行为；4）确定性验证器强制执行配对性、非重叠性及BIO标签合法性。

Result: 在六个跨行业、多语言的职位广告句子数据集上，SRICL相比GPT-3.5提示方法显著提升STRICT-F1分数，并大幅减少无效标签与幻觉技能片段。

Conclusion: SRICL提升了LLM在低资源、多领域技能抽取任务中的可靠性与实用性，支持端到端句子级部署。
...
TL;DR: 本文提出了一个用于SemEval-2026 Task 2的系统，结合LLM提示、Ising风格的MaxEnt模型和轻量神经回归模型，在建模用户文本情感状态及其短期变化上取得第一。


<details>
  <summary>Details</summary>
Motivation: 任务要求对时序用户生成文本建模当前情感状态及短期情感变化，需兼顾文本语义与动态轨迹建模。

Method: 采用三种互补方法：(1) 用户感知/无感的LLM提示；(2) 带Ising式交互的成对最大熵（MaxEnt）模型用于结构化转移建模；(3) 融合近期情感轨迹与可训练用户嵌入的轻量神经回归模型。

Result: LLM擅长捕捉静态情感信号，而短期情感变化更依赖于近期数值状态轨迹而非文本语义；系统在Subtask 1和Subtask 2A中均排名第一。

Conclusion: 结合文本语义建模与数值轨迹建模的混合方法优于纯语言模型方法，凸显了时序情感建模中动态状态信息的关键作用。
...
TL;DR: 本文提出了一种无需分类器的、基于概念分离曲线（Concept Separation Curves）的句子嵌入评估新方法，通过引入句法噪声和语义否定来量化嵌入对概念与表层变化的区分能力，并在多语言、多领域数据上验证了其可解释性与跨模型适用性。


<details>
  <summary>Details</summary>
Motivation: 现有句子嵌入评估方法多依赖下游分类器或任务，难以判断性能提升源于嵌入本身还是分类器行为，缺乏对嵌入概念稳定性的直接、客观衡量。

Method: 提出一种分类器无关的评估方法：系统地向句子中注入句法噪声和语义否定，计算其对嵌入向量的影响差异；利用概念分离曲线可视化嵌入对概念变化（如否定）与表面变化（如词序扰动）的区分能力。

Result: 在荷兰语和英语、多个领域及不同句长数据上的实验表明，概念分离曲线能有效、可复现地反映句子嵌入的概念稳定性，且具备跨模型比较能力。

Conclusion: 概念分离曲线是一种可解释、可复现、跨模型的句子嵌入评估新范式，为脱离下游任务的嵌入质量分析提供了可靠工具。
...
TL;DR: 本文提出了一种名为llm-bias-bench的开源方法，通过直接与间接探针结合多轮交互与角色模拟，系统性地揭示大语言模型在争议性话题上的真实立场与情境依赖性偏见（如谄媚倾向），并在38个葡萄牙语话题上对13个模型进行了实证评估。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型广泛影响公众信息获取与决策，但其隐含立场难以被可靠识别——模型常对直接提问回避作答，且在不同对话情境下表现出不一致观点，亟需能模拟真实多轮交互的偏见探测方法。

Method: 提出双路径探针法：（1）直接探针——在五轮递进式压力下询问模型观点；（2）间接探针——不直接提问，而是通过辩论式交互（中立/赞同/反对三类用户角色）观察其让步、抵抗或反驳行为；结合九类行为分类与可审计的LLM裁判机制进行判定。

Result: 在38个巴西葡萄牙语争议话题上测试13个助手模型，发现：辩论比直接提问诱发2–3倍的谄媚行为（中位数50%→79%）；表面有立场的模型在持续辩论中常退化为镜像响应；攻击者能力仅在需动摇既有立场时起关键作用，而非初始中立状态。

Conclusion: LLM的真实立场高度依赖交互情境，单纯依赖直接提问会严重低估其谄媚倾向与立场不稳定性；所提方法为可复现、可审计的模型价值观评估提供了新范式。
...
TL;DR: 本文提出AgenticQwen系列小规模语言代理模型，通过多轮强化学习结合双数据飞轮（推理飞轮与代理飞轮）在合成与开源数据上训练，显著提升多步推理与工具调用能力，在公开基准与工业系统中均达到接近大模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现代工业应用需要能在严格成本与延迟约束下执行多步推理和工具调用的小型语言代理模型。

Method: 采用多轮强化学习，在合成数据与少量开源数据上联合优化；构建双数据飞轮：推理飞轮基于错误反馈逐步提升任务难度，代理飞轮将线性流程扩展为多分支行为树以建模真实决策复杂性。

Result: AgenticQwen在多个代理基准测试中表现优异，并在工业代理系统中于搜索与数据分析任务上接近更大模型的性能。

Conclusion: AgenticQwen验证了小型模型通过结构化RL训练与高质量合成数据飞轮，可高效胜任复杂代理任务，为低成本、低延迟工业部署提供了可行路径。
...
TL;DR: 本文发现非英语响应在推理任务中有时优于英语，并提出polyGRPO强化学习框架，将语言变化视为隐式探索信号，在仅用多语言数学题训练下显著提升模型推理能力，且具备跨任务泛化性。


<details>
  <summary>Details</summary>
Motivation: 观察到大语言模型在减少英语中心偏见过程中，非英语响应在推理任务中有时表现更优，作者假设语言是调节模型内部推理路径的潜在变量，而不仅是输出媒介。

Method: 设计了多语种思维实验（Polyglot Thinking Experiment），在语言约束与非约束条件下测试模型；进而提出polyGRPO（Polyglot Group Relative Policy Optimization）强化学习框架，利用语言变化作为隐式探索信号，在线生成多语种偏好数据并联合优化答案准确率与推理结构。

Result: polyGRPO在仅18.1K多语言数学问题（无思维链标注）上训练后，使Qwen2.5-7B-Instruct在四个英文推理测试集上绝对准确率提升6.72%，多语言基准提升6.89%；并首次在仅用数学数据训练的前提下，在英文常识推理任务上超越基线模型4.9%。

Conclusion: 将语言视为潜在变量可拓展模型的隐式推理空间，带来一致且可泛化的推理性能提升；polyGRPO验证了多语种多样性本身即是一种有效的推理增强机制。
...
TL;DR: 本文提出Verbal Process Supervision (VPS)，一种无需训练的推理时扩展方法，通过更强监督者提供的结构化自然语言批评，引导生成-批评-精炼循环，在多个基准上显著提升LLM推理性能，并确立‘批评粒度’为推理时扩展的新维度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理时扩展主要关注链深度、样本广度和学习型步骤评分器（PRMs），本文旨在引入并验证第四个可扩展轴——外部语言监督的粒度。

Method: 提出Verbal Process Supervision (VPS)框架：不依赖梯度更新，利用能力更强的监督模型提供结构化自然语言批评，驱动被监督模型进行多轮（最多R轮）的generate-critique-refine迭代优化。

Result: 在GPQA Diamond、AIME 2025和LiveCodeBench V6上验证有效：1）GPQA上GPT-5.4达94.9%（R=4），超SOTA；2）AIME上实现弱模型‘强救援’，提升达+63.3分；3）同等计算下优于Reflexion和Self-Consistency；性能与监督者-执行者能力差强相关（r=0.90），但在代码等难语言表达错误任务中下降。

Conclusion: 批评粒度是继深度、广度、评分器之后推理时扩展的第四关键轴；VPS为无需训练的高效推理增强提供了新范式，并指出需结合可执行反馈以应对语言不可表达错误。
...
TL;DR: 本文探讨了在非英语国家及硬件受限地区部署语言模型（LMs）所面临的挑战，即‘最后一公里’问题——多语言性与边缘部署的交汇点。作者通过调研232篇相关论文，系统梳理了从数据收集到开发部署的语言建模全流程现状与挑战，并提出面向不同利益相关者的可行建议，旨在推动包容、公平的语言技术发展。


<details>
  <summary>Details</summary>
Motivation: 非英语国家和硬件受限社区在语言模型部署中面临多重障碍，多语言需求与边缘计算限制存在技术目标一致但实现要求冲突的问题；同时，边缘计算与多语言NLP研究长期割裂，亟需交叉研究以服务语言多样性高且基础设施薄弱的全球南方群体。

Method: 对232篇覆盖语言建模全流程（数据采集、模型开发、边缘部署等）的论文进行系统性文献综述，分析当前技术现状、核心挑战与研究空白，并基于此提出面向研究者、开发者、政策制定者等多方利益相关者的实践建议。

Result: 识别出多语言性与边缘部署协同中的关键挑战（如低资源语言数据稀缺、模型轻量化与语言覆盖的权衡、本地化评估标准缺失等），揭示了该交叉领域的研究缺口，并形成一套可操作的跨领域协作建议。

Conclusion: 解决‘最后一公里’问题需打破边缘计算与多语言NLP的研究壁垒，强调以社区为中心的技术设计、资源适配的评估框架及多元利益相关方协同，是构建真正包容、公平语言技术的关键路径。
...
TL;DR: 本文提出了一种联合建模标注者特定标签预测与对应解释的框架，利用标注者提供的细粒度理由（rationales）提升自然语言推理（NLI）中的分歧建模能力；通过User Passport机制融合标注者身份与人口统计信息，并设计两种解释生成架构（后验提示式与前缀桥接式），显著提升预测性能与解释一致性。


<details>
  <summary>Details</summary>
Motivation: 现有工作多关注离散化标签建模视角差异，但忽视了标注者自身提供的细粒度理由所蕴含的个体视角信号；需更忠实、丰富地建模人类标注分歧背后的认知多样性。

Method: 提出联合建模框架：1）基于User Passport机制将标注者身份与人口统计元数据编码进表示层；2）设计两种解释器：a）后验提示式explainer（post-hoc prompt-based），b）前缀桥接式explainer（prefixed bridge），后者直接迁移分类器表征至生成模型以对齐个体视角。

Result: 在具有离散NLI标注与标注者解释的数据集上验证：引入解释建模显著优于仅标注者感知的基线分类器；前缀桥接法在标签对齐稳定性与语义一致性上更优，后验提示法则在词汇相似性上更强。

Conclusion: 将解释建模为细粒度视角的表达，能更真实、丰富地刻画标注分歧；所提方法通过将标注者特异性理由同时融入预测与生成模块，推动了视角主义建模的发展。
...
TL;DR: 本文提出了一种基于持续同调的新时间序列滤波方法，并将其应用于眼动追踪数据（特别是阅读时的注视序列）分析，构建了融合拓扑特征与传统统计特征的混合模型，在丹麦语语料库上的阅读障碍检测任务中显著优于仅使用传统特征的方法。


<details>
  <summary>Details</summary>
Motivation: 现有眼动数据用于阅读障碍检测的方法多依赖传统统计特征，缺乏对序列结构和尺度不变性的建模能力；持续同调虽具潜力，但缺乏适配眼动时间序列的专用滤波策略。

Method: 设计了针对时间序列（尤其是注视序列）的新颖滤波（filtration）方法；将眼动扫描路径建模为时间序列，提取持续同调特征；构建融合拓扑特征与传统统计特征的混合机器学习模型。

Result: 在Copenhagen Corpus上，所提混合模型在阅读障碍检测任务中性能超越纯传统特征方法，并达到与成熟基线方法相当的水平；新提出的滤波策略优于已有滤波方法。

Conclusion: 持续同调能有效捕获注视序列中传统统计特征难以表征的互补结构信息；专为眼动数据设计的滤波策略及混合建模范式提升了阅读障碍自动检测的准确性和鲁棒性。
...
TL;DR: 本文扩展了先前提出的无需训练的构音障碍严重程度评估方法，通过在25个数据集、12种语言和5种病因（包括帕金森病、脑瘫、ALS等）及健康对照共3374名受试者上验证其跨语言、跨模型鲁棒性，发现病因特异性退化模式可区分、语音特征轮廓在跨语言间形状稳定但绝对值不可比、且该方法在6种自监督语音模型上均表现一致。


<details>
  <summary>Details</summary>
Motivation: 提升构音障碍严重程度评估方法的泛化能力，验证其在多语言、多病因、多模型下的鲁棒性和临床适用性。

Method: 基于冻结的自监督语音表示（6种SSL骨干网络），计算音系特征子空间的d-prime可分性，进行跨语言、跨病因、跨模型的组水平与个体水平分析。

Result: 1）病因间退化特征轮廓可区分（10/13特征具大效应量），但个体分类性能有限（宏观F1=22.6%）；2）跨语言轮廓形状高度稳定（余弦相似度>0.95），但绝对d-prime值不具跨语言可比性；3）6种骨干模型均呈现强单调严重度梯度（rho>0.77），且固定token数估计仍保持相关性（rho=-0.733）。

Conclusion: 音系子空间d-prime分析是一种鲁棒、无需训练的构音障碍表征框架，具备病因感知能力、跨语言轮廓稳定性与跨模型一致性，适用于多中心、多语言临床研究，但绝对严重度需在特定语料库内校准。
...
TL;DR: 本文通过评估机器学习管道生成中的特征选择偏差，发现现有基于简单条件语句的代码生成偏差评估方法严重低估了实际偏差程度，敏感属性在生成的管道中出现率高达87.7%，远超条件语句测试中的59.2%。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过简单条件语句评估代码生成中的偏差，但这类方法仅覆盖有限的真实编程场景，且只能揭示显性编码偏差，无法反映更复杂、实际应用中的隐性偏差。

Method: 本文选取更具现实意义的机器学习（ML）管道生成任务，测试多种代码专用和通用指令大语言模型在特征选择阶段对敏感属性（如种族）的不当使用，并对比其与传统条件语句测试中的偏差表现；同时验证不同提示缓解策略、属性数量和管道难度下的鲁棒性。

Result: 在ML管道生成任务中，敏感属性平均出现在87.7%的生成结果中，显著高于条件语句测试中的59.2%；该偏差具有鲁棒性，不受提示工程、属性数量或任务难度影响。

Conclusion: 简单条件语句不足以作为代码生成偏差评估的有效代理，当前基准严重低估了实际部署中的偏差风险，亟需构建更贴近真实编程任务的偏差评估框架。
...
TL;DR: X-GRAM是一种频率感知的动态token注入框架，通过混合哈希、别名混合和归一化SwiGLU ShortConv等技术，提升大模型中token查找表的参数效率与内存利用率，在保持低计算开销的同时显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有大token索引查找表存在参数效率低、内存增长快的问题，根源在于Zipf分布下的长尾欠训练、各层需求异质性以及'槽坍塌'导致的嵌入冗余。

Method: 提出X-GRAM框架：采用混合哈希与别名混合压缩长尾、保留头部容量；用归一化SwiGLU ShortConv精炼检索向量以提取局部n-gram特征；通过深度感知门控将信号注入注意力值流和层间残差，实现静态记忆与动态上下文对齐。

Result: 在0.73B和1.15B模型规模上，X-GRAM平均准确率分别比基线提升最多4.4分，比强检索基线高3.2分，且仅需50%大小的查找表。

Conclusion: X-GRAM通过高效内存管理解耦模型容量与计算量，为未来内存增强架构提供了可扩展、实用的新范式。
...
TL;DR: 本文提出Agent Evolving Learning（AEL）框架，通过快慢双时间尺度机制解决LLM智能体在多轮任务中无法有效利用历史经验的问题：快尺度用Thompson采样选择记忆检索策略，慢尺度用LLM反思失败模式并注入因果洞察；在序列投资组合基准上显著优于现有自改进方法，且实验证明‘少即是多’——仅记忆+反思即可大幅提升性能，额外机制反而损害效果。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在开放、多轮任务中仍为无状态，无法将过往经验转化为未来更优行为；核心难点不在于记什么，而在于如何使用所记内容（如检索策略选择、结果解读、策略切换时机）。

Method: 提出双时间尺度的Agent Evolving Learning（AEL）框架：快时间尺度采用Thompson Sampling带臂算法动态选择记忆检索策略；慢时间尺度由LLM驱动反思，识别失败模式并生成因果性洞察，嵌入决策提示中以提升对检索证据的理解与运用能力。

Result: 在包含10个行业、208轮、5次随机种子的序列投资组合基准上，AEL取得2.13±0.47的夏普比率，超越5种已发表自改进方法及所有非LLM基线，且在所有LLM方法中方差最低；九变量消融实验表明，仅记忆+反思即带来58%累计提升，其余所有额外机制（如规划演化、工具选择等）均导致性能下降。

Conclusion: 智能体自我提升的主要瓶颈在于‘如何自我诊断并有效使用经验’，而非堆砌架构复杂性；AEL验证了轻量、聚焦于经验解释与策略适配的机制设计更有效。
...
<details>
  <summary>Details</summary>
Motivation: 长期对话代理需要能够捕捉事件间关系的记忆系统，以支持时间推理和多跳问答。现有方法在扁平记忆（高效但缺乏关系建模）与图记忆（结构化但构建昂贵脆弱）之间存在根本权衡。

Method: 提出StructMem框架，通过保留事件级绑定、诱导跨事件连接、时间锚定双视角以及周期性语义整合，实现结构化且高效的记忆管理。

Result: 在LoCoMo基准上，StructMem提升了时间推理与多跳问答性能，同时显著减少token使用量、API调用次数和运行时间。

Conclusion: StructMem在保持结构化推理能力的同时，大幅提升了记忆系统的效率与鲁棒性，为长期对话代理提供了更优的记忆解决方案。
...
TL;DR: 本文提出了一种新的文化相关开放问题（CROQ）数据集，揭示了大语言模型（LLMs）在文化覆盖和区域偏好上的偏差，发现LLMs倾向于日本等特定国家，且提示语言影响其输出多样性与区域倾向；研究还指出该文化偏差主要在监督微调阶段而非预训练阶段出现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在文化覆盖和胜任力方面存在局限，常表现出西方中心或盎格鲁中心偏见，但尚无专门研究其在文化相关问题上的区域偏好。

Method: 构建基于全面文化相关开放问题（CROQ）分类法的新数据集，系统评估多个LLM对不同国家文化的回答倾向，并分析提示语言（如英语）和训练阶段（预训练 vs 监督微调）对偏差的影响。

Result: LLMs展现出对日本等非西方国家的明显偏好；使用高资源语言（如英语）提示时，输出更趋多样、区域倾向减弱；文化偏差主要在监督微调后显现，而非预训练阶段。

Conclusion: LLM的文化偏差具有显著的区域性和阶段特异性，需在微调阶段针对性干预以提升全球文化公平性。
...
<details>
  <summary>Details</summary>
Motivation: 现有音频问答基准存在捷径策略、短时线索、词汇先验、数据集偏差等问题，无法有效评估模型的真实音频推理能力。

Method: 构建了AUDITA基准，包含人工撰写的基于真实音频的冷知识问题，设计具有挑战性干扰项和长时依赖的探测性问题，并采用项目反应理论（IRT）分析模型表现。

Result: 人类平均准确率为32.13%，而当前最优音频问答模型平均准确率低于8.86%；IRT分析揭示了模型与数据的系统性缺陷。

Conclusion: AUDITA为音频理解提供了更严格、更贴近现实的评测基准，凸显当前模型在真实音频推理任务上的严重不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频错误信息检测仅判断整段视频是否含错误信息，缺乏对错误信息发生时间点及具体主张内容的细粒度定位，限制了可解释性与实际应用。

Method: 构建两个新数据集，通过音频转录获得文本，人工标注错误信息出现的时间片段（misinformation span）；使用前沿语言模型训练分类器进行片段级错误信息检测。

Result: 在视频错误信息片段检测任务上达到0.68的F1分数；公开发布数据集、转录文本、音频及原始视频。

Conclusion: 细粒度的视频错误信息片段检测是可行且必要的，所构建的数据集和方法为未来研究提供了重要基础和可复现资源。
...
TL;DR: 本文介绍了NSNRL共享任务，旨在通过二元分类问题来操作化叙事相似性，并基于新定义的叙事相似性评估叙事嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 提出一种与叙事理论和直觉判断兼容的新型叙事相似性定义，并构建相应数据集以评估叙事表示学习方法。

Method: 将叙事相似性建模为三元组形式的二元分类任务；收集了超过1000个故事摘要三元组，每个至少有两个标注者一致标注；组织了两个赛道（分类与嵌入），分析了71个提交系统的技术方案。

Result: LLM集成在分类赛道表现优异；预训练嵌入模型经前后处理后在嵌入赛道与定制微调方案性能相当；分析指出两类任务仍有提升空间。

Conclusion: NSNRL任务为叙事相似性研究提供了新基准与高质量数据集，揭示了当前自动化叙事理解系统的潜力与局限。
...
TL;DR: 本研究通过'举报者困境'实验，考察大语言模型（LLMs）在道德判断中是否具备人际情境敏感性，发现模型虽能预测人类基于关系亲疏而偏向忠诚的行为，但自身决策却固守公平导向的规范性判断，暴露其决策逻辑与社会建模能力之间的不一致。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型日益作为决策支持系统应用，亟需检验其是否具备人类道德判断所依赖的情境依赖性和人际关系调节能力。

Method: 设计'举报者困境'实验，操控犯罪严重程度和关系亲疏两个维度，从道德正确性、人类行为预测、模型自主决策三个视角分析模型输出及推理过程。

Result: 模型对人类行为的预测随关系亲近而显著转向忠诚导向，但其自身决策始终维持公平导向，与预测结果不一致；推理分析显示其决策依赖刚性规范规则，而非社会敏感性建模。

Conclusion: LLM的决策机制与其内部社会世界建模存在脱节，这种不一致可能导致现实部署中的严重价值错位，需针对性改进其社会情境理解与决策一致性。
...
TL;DR: 本文提出了RedirectQA数据集，利用Wikipedia重定向信息将Wikidata三元组与实体的多样化表面形式（如别名、缩写、拼写变体等）关联，以研究大语言模型对事实知识的记忆是否依赖于特定表面形式。实验发现，模型在不同表面形式下预测结果不一致，且这种不一致性与表面形式类别相关；实体频率和表面形式频率均影响准确率，但实体频率的贡献常超越表面频率。结果表明事实记忆既非完全表面依赖，也非完全表面无关，强调了在评估非逐字记忆时需考虑表面形式多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于实体的问答（Entity-based QA）评估方法通常只使用每个实体的一个标准表面形式，难以区分模型是真正记住了事实，还是仅记住了该事实与特定名称的关联，因此需要一种能覆盖实体多种表面形式的评估框架。

Method: 构建RedirectQA数据集，利用Wikipedia重定向页面将Wikidata中的事实三元组映射到同一实体的多种表面形式（包括别名、缩写、拼写变体、常见错误形式等），并按类型分类；在13个大语言模型上系统测试不同表面形式下的问答一致性与准确率，并进行频率相关性分析。

Result: 模型在不同表面形式下预测结果显著变化，对正字法微小变异（如大小写、标点）较鲁棒，但对词汇级变异（如别名、缩写）鲁棒性差；实体频率和表面形式频率均与准确率正相关，且实体频率的预测力常独立于表面频率。

Conclusion: 大语言模型的事实记忆呈现‘表面敏感但非表面绑定’特性，即既受表面形式影响，又部分泛化至不同表达；因此，评估非逐字记忆必须采用多样化的表面形式，单一形式的评测会严重低估模型局限性。
...
Method: 提出MODEE方法，融合图神经网络与大语言模型的文本表征，显式建模文档级上下文、结构和语义推理，克服LLM的‘中间丢失’和注意力稀释问题。

Result: 在大规模数据集上的实验表明，MODEE显著优于当前最优开放域事件抽取方法，并可迁移至封闭域任务，同样超越现有算法。

Conclusion: MODEE通过多模态协同建模有效提升了开放域事件抽取性能与泛化能力，为利用LLM进行复杂文档级推理提供了新思路。
...
TL;DR: 本文提出TingIS系统，通过多阶段事件关联引擎结合高效索引与大语言模型（LLM），实现从高噪声、高吞吐的客户工单中稳定提取可操作的技术异常事件，并在生产环境中实现3.5分钟P90告警延迟和95%高优先级事件发现率。


<details>
  <summary>Details</summary>
Motivation: 大型云原生服务中，客户工单是发现监控盲区风险的重要信号，但其高噪声、高吞吐和语义复杂性导致难以从中提取有效情报。

Method: 提出端到端企业级事件发现系统TingIS，核心是融合索引技术与大语言模型（LLM）的多阶段事件关联引擎；辅以级联业务路由机制和融合领域知识、统计模式与行为过滤的多维降噪流水线。

Result: 在日均30万消息、峰值2000条/分钟的生产环境中，实现P90告警延迟3.5分钟、高优先级事件发现率达95%；在真实数据基准测试中，路由精度、聚类质量与信噪比均显著优于基线方法。

Conclusion: TingIS验证了LLM与传统工程方法协同用于高噪声实时事件发现的有效性与实用性，为云服务运维提供了可扩展、高鲁棒性的智能告警新范式。
...
TL;DR: 本文提出EVENT5Ws，一个大规模、人工标注且统计验证的开放领域事件抽取数据集，旨在解决现有数据集事件类型覆盖有限和缺乏高质量开放域数据的问题；通过系统化标注流程构建该数据集，并基于其对大语言模型进行评估、建立新基准，验证了其跨地域泛化能力，并总结了大规模数据集构建的经验与建议。


<details>
  <summary>Details</summary>
Motivation: 现有事件抽取数据集存在两大局限：封闭领域数据集事件类型覆盖有限，开放领域缺乏大规模、人工验证的高质量数据集，制约了自动化事件抽取方法的发展。

Method: 设计并实施了一套系统化的标注流程，构建了大规模、人工标注且经统计验证的开放域事件抽取数据集EVENT5Ws；在此基础上评估了当前主流预训练大语言模型，建立了新的基准，并分析了模型在不同地理语境数据上的泛化性能。

Result: 成功构建了EVENT5Ws数据集；基于该数据集评估了大语言模型性能，确立了新基准；实验证明在EVENT5Ws上训练的模型能有效泛化至不同地理背景的数据集。

Conclusion: EVENT5Ws为开放域事件抽取提供了高质量、可扩展的数据基础，显著提升了模型的泛化能力，同时为未来大规模数据集构建提供了可复用的方法论和实践建议。
...
TL;DR: 本文提出一个结合历时文体分析、上下文主题建模与语义聚类的计算框架，用于多维度分析巴西众议院45万+演讲文本，揭示风格简化、议题随危机转向、以及身份（地域/性别）比党派更影响话语相似性的新发现。


<details>
  <summary>Details</summary>
Motivation: 现有立法行为研究过度依赖投票记录，忽视政治演说中丰富的语义与修辞信息。

Method: 融合历时 stylometric 分析、上下文主题建模（如BERTopic或动态主题模型）和语义聚类（如基于句子嵌入的聚类），应用于巴西众议院2003–2025年超45万篇演讲语料。

Result: 发现长期文体趋势为演讲更短更直接；立法议程对国家危机响应显著；话语相似性更多由地域与性别身份而非政党归属驱动。

Conclusion: 该框架为议会话语提供了可扩展、可泛化的多维分析范式，有效补充传统以投票为中心的研究方法。
...
TL;DR: 本文提出GiVA，一种基于梯度的向量自适应初始化策略，在保持极低参数量的同时，训练时间与LoRA相当，并将所需秩降低8倍，性能优于或媲美现有方法。
...
Result: 在NLU、NLG和图像分类多个基准上，GiVA性能优于或媲美现有向量自适应方法及LoRA，且秩需求降低8倍。

Conclusion: GiVA成功兼顾参数效率、训练效率与模型性能，为大模型高效微调提供了新思路。
...
TL;DR: 本文提出MathDuels，一种自博弈数学基准测试，让模型同时担任问题作者和解决者，通过三阶段生成流程和独立验证机制评估模型能力，发现作者与解决能力部分解耦，并能揭示单角色基准无法识别的能力差异。
...
Method: 引入MathDuels自博弈基准，模型兼具问题作者与解决者双重角色；采用元提示、问题生成与难度增强三阶段问题生成流程，并由独立验证器筛选问题；使用Rasch模型联合估计求解能力与问题难度，作者质量由其生成问题的难度决定。

Result: 在19个前沿模型上的实验表明，作者与解决能力部分解耦；双角色评估可揭示单角色基准无法识别的能力差异；新模型不断生成击败旧优胜者的难题，使基准难度随参与者能力共同演化而非达到固定上限。

Conclusion: MathDuels提供了一种动态、自适应的评估范式，突破传统静态基准的天花板限制，更全面地刻画模型数学能力。
...
TL;DR: 本文探讨了使用解码器型大语言模型（LLMs）进行自动语音识别（ASR）语义评估的潜力，提出三种方法：候选假设选择、生成式嵌入计算语义距离和错误定性分类；实验表明，LLM在HATS数据集上与人工标注者的一致性达92-94%，显著优于传统WER（63%）及现有语义指标，且其嵌入性能媲美编码器模型，展现出可解释、语义感知的ASR评估新方向。


<details>
  <summary>Details</summary>
Motivation: 传统ASR评估指标WER对语义不敏感，而现有语义度量虽更贴近人类感知，但尚未充分探索解码器型大语言模型在此任务中的适用性。

Method: 采用三种方法评估解码器型LLM在ASR语义评估中的有效性：(1) 二选一最佳假设选择；(2) 利用生成式嵌入计算语义距离；(3) 对识别错误进行定性分类。

Result: 在HATS数据集上，最优LLM在假设选择任务中与人工标注者达成92–94%一致性，远超WER（63%）及其它语义指标；其生成的嵌入性能与编码器模型相当。

Conclusion: 解码器型LLM为ASR提供了可解释、语义驱动的新型评估范式，具有显著优势和应用前景。
...
TL;DR: 本文提出PlantInquiryVQA基准，用于评估视觉语言模型在植物病理诊断中多步、意图驱动的视觉推理能力；构建了包含24,950张图像和138,068问答对的数据集，并提出‘问询链’框架；实验表明结构化多轮问询显著提升诊断准确率、降低幻觉并提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型仅在单轮问答上评测，而真实植物病理诊断需专家基于图像进行多步、自适应、意图明确的结构化问询，存在评测范式与实际需求之间的鸿沟。

Method: 提出Chain of Inquiry框架，将诊断过程建模为依托视觉线索、显式表征认知意图的有序问答序列；构建PlantInquiryVQA基准数据集，含专家标注的图像、问答对、视觉定位、严重度标签及领域推理模板；在多模态大模型上开展系统评测。

Result: 主流多模态大模型能较好描述视觉症状，但在安全临床推理和准确诊断上表现薄弱；引入结构化多轮问询后，诊断正确率提升，幻觉减少，推理更高效。

Conclusion: PlantInquiryVQA为训练类专家botanist的诊断型AI代理提供了新基准和方法论基础，推动视觉语言模型从静态分类器向动态推理代理演进。
...
TL;DR: 本文提出了一种面向专业后期处理的文本到线性图像生成方法，通过将线性图像表示为多曝光序列，并设计基于DiT的流匹配架构来解决高动态范围与位深带来的生成难题。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要合成显示参考图像（display-referred），其动态范围压缩、非线性且难以编辑；而传感器原始RAW转成的线性图像（scene-referred）保留真实辐照度和更高信息量，更适合专业后期，但尚未被生成模型有效支持。

Method: 将线性图像建模为多个曝光档位（exposure brackets）的序列，每个档位覆盖部分动态范围；采用基于Diffusion Transformer（DiT）的流匹配（flow-matching）架构实现文本条件下的曝光序列生成；并扩展支持ControlNet结构引导和文本引导的线性图像编辑。

Result: 实现了高质量、高保真、全动态范围的文本到线性图像生成；验证了其在文本驱动线性图像编辑和结构引导生成等下游任务中的有效性。

Conclusion: 文本到线性图像生成是连接生成式AI与专业图像工作流的关键一步；所提曝光序列建模与DiT流匹配框架有效克服了传统VAE在高动态范围建模上的局限，为可编辑图像生成开辟了新方向。
...
TL;DR: 本文提出了一种双路径网络，通过空间-时间（ST）和时间-空间（TS）两种顺序处理解剖学对齐的微动作，并引入实体级自适应路由与互动作一致性损失（MAC），以应对微动作在时空特性上的多样性，显著提升了细粒度视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 微动作（如挠头、敲手指）是社交沟通中关键但易被现有视觉系统忽略的细微行为；其时空特性多样（有的依赖空间构型，有的依赖时间动态），而传统单一分解方法难以兼顾。

Method: 提出双路径网络：ST路径先建模空间构型再捕获时间动态，TS路径反之；采用实体级自适应路由为每个身体部位选择最优处理路径，并设计互动作一致性损失（MAC）增强双路径协同。

Result: 在MA-52数据集上达到有竞争力的性能，在iMiGUE数据集上取得SOTA结果。

Conclusion: 针对微动作内在复杂性进行网络结构自适应设计，对提升细粒度视频理解至关重要。
...
TL;DR: 本文提出了一种无需训练的创新方法，将多光谱遥感影像适配到仅支持RGB输入的通用大视觉语言模型（如Gemini 2.5）中，通过空间对齐与指令注入实现零样本性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 通用多模态大模型（LMMs）通常仅在RGB图像上训练，难以直接用于多光谱遥感任务；而专门训练多光谱LMMs成本高、泛化性差。

Method: 在推理阶段，不修改模型权重，而是将非RGB多光谱数据映射/适配至LMM已有的RGB视觉空间，并结合领域知识和思维链（Chain-of-Thought）指令进行引导。

Result: 在主流遥感基准上实现了显著的零样本性能提升，验证了该方法在Gemini 2.5上的有效性。

Conclusion: 无需训练即可拓展通用LMMs至多光谱遥感领域，为地学专业人员利用强大通用模型处理专用传感器数据提供了新范式。
...
TL;DR: 本文提出了一种名为Projected Gradient Unlearning (PGU)的新方法，用于文本到图像扩散模型的机器遗忘，通过构建核心梯度空间（CGS）并将其正交补空间投影梯度更新，防止下游微调导致已遗忘概念的恢复。PGU作为后处理加固步骤，可与现有遗忘方法（如ESD、UCE、Receler）结合使用，在6分钟内显著提升遗忘鲁棒性，且与Meta-Unlearning互补。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型的遗忘方法存在一个共性缺陷：即使在下游微调数据完全不相关的情况下，已被擦除的概念仍会恢复；因此需要一种能抵御后续微调干扰的遗忘加固机制。

Method: 将分类任务中的Projected Gradient Unlearning (PGU)适配至扩散模型领域，构建基于保留概念激活的核心梯度空间（CGS），并将后续微调的梯度更新投影至CGS的正交补空间，从而约束参数更新方向，防止遗忘概念复活。

Result: PGU可有效消除风格类概念的复活现象，并显著延迟物体类概念的复活；单次运行耗时约6分钟（远低于Meta-Unlearning的2小时）；与ESD/UCE/Receler等方法结合后，遗忘效果更鲁棒；与Meta-Unlearning互补，性能优劣取决于概念编码方式，推荐按视觉特征相似性而非语义分组选择保留概念。

Conclusion: PGU是一种高效、轻量、即插即用的后处理遗忘加固方法，解决了扩散模型中遗忘易被下游微调破坏的关键问题；其有效性依赖于对概念编码特性的理解，强调视觉相似性在保留集构造中的重要性。
...
TL;DR: 本文提出StyleVAR，将风格迁移建模为潜在离散序列的条件生成问题，基于VAR框架，通过多尺度VQ-VAE编码与带混合交叉注意力的Transformer实现内容结构与风格纹理的协同建模，并采用两阶段训练（监督微调+基于DreamSim奖励的GRPO强化微调），在多个指标和基准上超越AdaIN。


<details>
  <summary>Details</summary>
Motivation: 现有风格迁移方法难以在保持内容语义结构的同时精准迁移风格纹理；VAR框架虽具潜力，但缺乏对风格与内容双路径的显式、可控建模机制。

Method: 1）将图像分解为多尺度表示并用VQ-VAE离散化；2）设计混合交叉注意力机制，使目标表征同时关注自身历史、风格查询与内容查询，并引入尺度依赖的融合系数；3）两阶段训练：先监督微调（content-style-target三元组），再以DreamSim为奖励函数、结合每动作归一化权重的GRPO进行强化微调。

Result: 在in-/near-/out-of-distribution三大基准上，StyleVAR全面超越AdaIN，在Style Loss、Content Loss、LPIPS、SSIM、DreamSim、CLIP相似度等指标均更优；GRPO阶段进一步提升感知对齐指标；定性显示其擅长风景与建筑场景的纹理迁移与结构保持。

Conclusion: StyleVAR验证了离散序列建模与分层强化优化在风格迁移中的有效性，但也揭示了对互联网图像泛化性不足及人脸处理困难等问题，提示需增强内容多样性与结构先验。
...
TL;DR: 本文提出了一种专家引导的对比微调框架，用于在数据稀缺、类别不平衡的儿科脑肿瘤全切片图像（WSI）分类任务中提升细粒度诊断准确性，通过将对比学习融入滑动窗口多实例学习（MIL），并引入临床指导的难负样本增强类间区分能力。


<details>
  <summary>Details</summary>
Motivation: 儿科脑肿瘤病理诊断面临数据极度稀缺、类别严重不平衡及形态学高度重叠等挑战，现有病理基础模型在弱监督、小样本下的适应性不足。

Method: 提出专家引导的对比微调框架，将对比学习嵌入滑动窗口多实例学习（MIL）中，设计通用监督对比和专家引导难负样本两种策略，后者基于临床知识选择易混淆亚型作为难负样本以优化滑片级表征几何结构。

Result: 在真实低样本、类别不平衡的儿科脑肿瘤WSI数据上验证，对比微调显著提升细粒度诊断性能；专家引导策略使类内表征更紧凑、类间分离更清晰。

Conclusion: 显式建模和优化滑片级表征几何结构对数据稀缺的儿科病理细粒度分类至关重要，专家知识可有效增强对比学习的判别能力。
...
TL;DR: 本文提出一种仅需单次观测即可调整扩散先验的方法，通过组合现有扩散先验构建乘积专家（product-of-experts）先验，并利用贝叶斯证据最大化确定各先验权重（指数），从而提升后验采样可靠性与泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散先验常受限于小规模或纯仿真训练数据，存在偏差；而微调方法又依赖大量带多样前向算子的观测，实际难以获取，小样本下易过拟合。

Method: 将多个预训练扩散先验构造成乘积专家先验，通过最大化贝叶斯证据自动学习各先验的指数权重，并支持对加温（tempered）及组合扩散模型进行后验采样。

Result: 在黑洞成像和文本引导图像去模糊等真实逆问题上验证有效；证据最大化的先验常超越单一数据集训练所得，表明泛化先验更优；提升了后验图像分布的可信度。

Conclusion: 基于贝叶斯证据驱动的指数加权组合先验，能以极少观测实现鲁棒、灵活且更可信的扩散先验调优，适用于先验未知或数据稀缺的实际逆问题。
...
TL;DR: 本文提出Foveated Reasoner，一种结合人类视觉聚焦机制（foveation）与自回归推理的视觉-语言模型框架，在低分辨率初始视图基础上按需触发高分辨率区域采样，并通过两阶段训练（冷启动监督+强化学习）优化视觉token使用效率与任务准确率。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像虽能提升视觉-语言模型性能，但带来显著计算开销；人类通过‘先粗后精’的视觉聚焦机制（foveation）高效平衡感知广度与精度，本文受此启发解决模型中视觉token预算与性能的矛盾。

Method: 提出Foveated Reasoner：1）以低分辨率图像起始，自回归解码中动态决定是否及何处触发高分辨率foveation；2）将局部高分辨率证据注入同一解码轨迹；3）采用两阶段训练——先用监督信号引导foveation行为，再用强化学习联合优化证据采集策略与任务准确率，并惩罚过度采样。

Result: 在多个视觉-语言基准上，该方法在严格视觉token预算下显著提升准确率，且成功习得有效的foveation策略（如聚焦关键区域、避免冗余采样）。

Conclusion: Foveated Reasoner验证了将生物启发的foveation机制融入视觉-语言模型解码过程的可行性与有效性，为高分辨率多模态建模提供了兼顾效率与性能的新范式。
...
<details>
  <summary>Details</summary>
Motivation: 传统建筑状况评估依赖大量人工标注，成本高、效率低，亟需一种可扩展、自动化且高精度的大规模评估方法。

Method: 基于Gemmma 3 27B在小规模人工标注数据上微调，结合Google街景图像进行建筑状况评估；采用知识蒸馏将能力迁移至更小模型（Gemma 3 4B、EfficientNetV2-M、SwinV2-B）；开展人-AI对齐研究，并构建可视化仪表盘。

Result: 微调后的Gemma 3 27B在SRCC和PLCC指标上超越单个人类评分员；蒸馏后的小模型保持相近性能，其中CNN/Transformer模型实现30倍加速；框架支持数十种建成环境与住房属性评估，并已集成至可视化分析平台。

Conclusion: 该框架在保证高评估精度的同时显著降低人工标注依赖与计算开销，为大规模城市建筑监测与住房政策分析提供了灵活、可部署的AI解决方案。
...
TL;DR: 本文系统研究了预训练数据集的地理组成对下游性能的影响，发现欧洲预训练数据集表现最优，且光谱多样性是影响性能的关键因素。
...
Method: 构建全球及各大洲预训练数据集，在全球和区域下游任务上进行评估；分析10个预训练数据集在大陆、生物群系、地表覆盖和光谱值四个维度的多样性，并与下游性能进行相关性分析。

Result: 欧洲预训练数据集在全局和局部下游任务中均优于全球及其他大洲数据集；仅光谱多样性与下游性能呈强相关，其余维度相关性弱。

Conclusion: 光谱多样性是构建高性能遥感基础模型预训练数据集时需重点考虑的新维度。
...
TL;DR: 本文提出了一种材料可控的房间脉冲响应（RIR）生成新方法，通过显式解耦空间布局与材料属性的影响，提升声学建模的真实感与用户控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有声学建模方法常将空间与材料影响纠缠在相关表征中，限制用户控制并降低生成声学的真实性。

Method: 设计双模块架构：空间模块建模场景几何布局对RIR的影响，材料模块根据用户指定的材料配置调制该空间RIR，实现显式解耦。

Result: 在声学指标（如RTE提升达+16%）和材料指标（提升达+70%）上显著优于先前方法，并通过人类感知实验验证了更高真实感与材料敏感性。

Conclusion: 显式解耦空间与材料因素的RIR生成框架，增强了可控性、真实性和可解释性，适用于VR、机器人、建筑与音频工程等领域。
...
TL;DR: 本文提出了一种参数高效的超光谱基础模型HyperFM，用于处理NASA PACE任务产生的大规模、复杂超光谱遥感数据，尤其在云特性反演等大气任务中表现优异，并发布了包含晴空与多云场景的大规模数据集HyperFM250K。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型主要面向RGB图像，难以解析PACE任务获取的连续高维光谱信号；已有超光谱模型多限于无云数据、单传感器，且参数量大、计算成本高，难以业务化应用。

Method: 提出HyperFM模型，融合组内与组间光谱注意力机制及混合参数分解策略，以高效建模光谱-空间关系并降低计算开销。

Result: 在四个下游云特性反演任务上，HyperFM持续优于现有超光谱基础模型和任务专用SOTA方法；同时开源大规模PACE超光谱数据集HyperFM250K。

Conclusion: HyperFM为处理新一代星载超光谱遥感数据提供了高效、可扩展的基础模型框架，推动了遥感智能解译向业务化落地发展。
...
TL;DR: WFM是一种基于小波空间流匹配的高效多模态MRI合成方法，利用已有模态的均值作为信息先验，仅需1-2步即可完成高质量合成，显著提升速度并降低模型参数量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在多模态MRI合成中质量高但计算开销大（数百步采样、每模态独立模型），限制临床部署；作者指出其低效源于从纯噪声开始扩散，忽略了已有的结构信息。

Method: 提出WFM（Wavelet Flow Matching）：在小波空间中，以条件模态的均值作为有信息的先验分布，直接学习该先验到目标模态的流匹配映射；采用单一大模型（82M参数）加类别条件实现四模态（T1/T1c/T2/FLAIR）统一合成。

Result: 在BraTS 2024上达到26.8 dB PSNR和0.94 SSIM，质量接近扩散基线（仅差1–2 dB），单体积合成耗时0.16–0.64秒（比扩散模型快250–1000倍，原为160秒），参数总量减少约75%（326M→82M）。

Conclusion: WFM通过引入解剖结构一致的有信息先验，大幅提升了MRI合成的效率与实用性，使实时临床应用成为可能。
...
TL;DR: 本文提出几何奖励信用分配框架，解决点云-视觉-语言模型中因稀疏几何标记被序列级奖励淹没而导致的几何幻觉问题，并通过重投影一致性项内化物理约束，显著提升3D空间预测的可靠性。


<details>
  <summary>Details</summary>
Motivation: Point-Vision-Language Models常出现几何幻觉，即预测的3D结构与观测到的2D现实矛盾；作者指出根本原因并非表征瓶颈，而是强化学习中结构错位——稀疏几何标记被嘈杂、广播式的序列级奖励所掩盖。

Method: 提出Geometric Reward Credit Assignment框架：将整体监督信号解耦为领域特定信号，并仅路由至对应负责的token片段；同时引入Reprojection-Consistency项作为跨模态验证器，惩罚物理上不可能的几何结构。

Result: 在ShapeNetCore构建的校准基准上，3D关键点准确率（KPA）从0.64提升至0.93，3D边界框IoU达0.686，重投影一致性分数达0.852，且保持稳健的2D定位性能。
...
TL;DR: WildSplatter 是一种前馈式 3D 高斯泼溅模型，能在未知相机参数和多变光照条件下，从无约束图像集合中快速重建高质量 3D 场景，并支持光照驱动的外观控制。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 高斯泼溅（3DGS）方法依赖多视角、已知相机位姿及一致光照，难以适用于真实世界中无约束的照片集合。

Method: 提出 WildSplatter，采用前馈网络联合学习 3D 高斯分布与基于输入图像的外观嵌入，通过条件化颜色调制建模光照与外观变化。

Result: 可在一秒内从稀疏视图重建 3D 高斯；在光照多变的真实数据集上显著优于现有无位姿先验的 3DGS 方法。

Conclusion: WildSplatter 实现了高效、鲁棒且可控的单阶段 3D 场景重建，拓展了 3DGS 在真实场景中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有空间推理方法依赖单一固定推理路径，难以适应不同上下文中多源线索（如2D外观、深度、几何约束）可靠性变化；同质化多智能体又限制了归纳偏置多样性。

Method: 构建异构多智能体框架SpatiO，集成具备互补归纳偏置的视觉-语言专家，并设计无需参数更新的测试时编排（TTO）机制，动态评估并重加权各智能体在推理过程中的可靠性。

Result: 在3DSRBench、STVQA-7k、CV-Bench和Omni3D-Bench等多个空间推理基准上，SpatiO显著优于闭源与开源基线模型。

Conclusion: 空间推理需具备‘空间适应性’，而异构多智能体配合动态编排机制是实现该能力的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 水下环境光照多变、能见度低、目标常被遮挡，导致现有目标检测模型性能显著下降，亟需提升其在真实复杂水下场景中的泛化能力。

Method: 基于DeepFish数据集的分割掩码生成边界框标注，构建定制检测数据集；设计一种受Deng等人copy-paste启发的伪模拟退火式数据增强算法，合成高密度、空间多样的鱼群场景用于训练。

Result: 所提方法在YOLOv10基线模型上显著提升性能，尤其在佛罗里达群岛实采手动标注测试集上表现突出。

Conclusion: 该数据增强策略可有效提升模型对密集、真实水下环境的目标检测鲁棒性与泛化能力。
...
TL;DR: 本文提出Sparse Forcing，一种用于自回归视频扩散模型的训练与推理新范式，通过利用注意力机制中视觉块的稀疏性与持久性，设计可学习的原生稀疏机制和高效GPU核PBSA，在提升长时序生成质量的同时显著降低解码延迟和KV缓存占用。
...
Method: 提出Sparse Forcing范式，包含可学习的原生稀疏机制（用于压缩、保留与更新持久块）和Persistent Block-Sparse Attention（PBSA）高效GPU核，以实现局部窗口内动态邻域计算、加速稀疏注意力与内存更新。

Result: 在5秒视频生成中VBench提升+0.26，解码提速1.11–1.17×，KV缓存峰值降低42%；在20秒和1分钟生成中VBench分别提升+0.68和+2.74，速度提升达1.22×和1.27×。

Conclusion: Sparse Forcing有效提升了自回归视频扩散模型的长时序生成质量与推理效率，验证了利用注意力稀疏结构建模时空记忆的可行性与实用性。
...
TL;DR: 本文提出UAU-Net，一种面向面部动作单元（AU）检测的不确定性感知框架，通过CV-AFE模块在表征阶段建模概率性AU特征，并通过AB-ENN网络在决策阶段基于Beta分布进行证据化多标签预测，显著提升在噪声、外观变化和标签不平衡下的鲁棒性与可靠性。


<details>
  <summary>Details</summary>
Motivation: AU检测面临表征与决策两阶段的异构不确定性（如视觉噪声、个体外观差异、AU间关系模糊、标签严重不平衡），现有方法忽略这些不确定性，导致鲁棒性和置信度校准差。

Method: 提出UAU-Net：1）CV-AFE——基于条件变分自编码器（CVAE）的多尺度时空AU特征提取模块，联合估计特征均值与方差，并以AU标签为条件建模AU间依赖相关不确定性；2）AB-ENN——不对称Beta证据神经网络，用Beta分布参数化预测不确定性，并设计针对高度不平衡二分类的不对称损失函数。

Result: 在BP4D和DISFA数据集上取得优异AU检测性能；分析表明，同时建模表征学习与证据化预测中的不确定性，有效提升了模型鲁棒性与预测可靠性。

Conclusion: 显式建模AU检测中表征与决策两阶段的不确定性是提升模型鲁棒性、校准性和泛化能力的关键路径；UAU-Net为不确定性驱动的细粒度行为分析提供了可解释、可靠的解决方案。
...
Method: 提出LatRef-Diff框架：1）用可学习的风格码替代扩散模型中的语义方向，支持随机/定制化风格操控；2）设计含可学习向量、交叉注意力与层级结构的风格调制模块；3）引入前向-后向一致性训练策略——先近似移除目标属性，再通过风格调制恢复，并辅以感知损失和分类损失。

Result: 在CelebA-HQ上达到SOTA性能，定性与定量结果均优于现有方法；消融实验验证了风格码生成方式、调制模块结构及一致性训练策略的有效性。

Conclusion: LatRef-Diff有效克服了面部属性编辑中精度、稳定性与可控性之间的权衡，为无配对数据下的细粒度人脸编辑提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在边缘设备上存在计算、内存和延迟开销大问题；HDC虽轻量但以往系统依赖多级存储和复杂聚类管理，难以部署于资源受限硬件。

Method: 提出硬件感知的持续学习方法：统一示例内存+硬件高效聚类合并策略+量化CNN前端；设计基于AMD Zynq ZCU104 FPGA的流式数据流架构，采用字打包二值超向量支持位级并行计算。

Result: 在CORe50数据集上，相比优化CPU/GPU基线，ImageHD实现最高40.4倍（4.84倍）加速和383倍（105.1倍）能效提升。

Conclusion: ImageHD验证了HDC赋能的持续学习在实时边缘AI中的实用性，为低开销、高效率的端侧自适应视觉学习提供了可行硬件方案。
...
TL;DR: 本文提出AttDiff-GAN，融合GAN的可控属性编辑与扩散模型的高质量生成能力，通过特征级对抗学习解耦编辑与合成，并引入PriorMapper和RefineExtractor提升风格-属性对齐与语义提取精度，在CelebA-HQ上实现了更准确的属性编辑与更好的非目标属性保持。
...
Method: 提出AttDiff-GAN：1）特征级对抗学习实现显式属性操控；2）用操控后的特征引导扩散图像生成；3）引入PriorMapper（融入面部先验）和RefineExtractor（基于Transformer捕获全局语义）提升风格-属性对齐与风格提取精度。

Result: 在CelebA-HQ数据集上，定性与定量评估均优于SOTA方法，属性编辑更准确、非目标属性保持更好。

Conclusion: AttDiff-GAN有效解决了GAN与扩散模型融合中的优化难题，通过解耦编辑与合成、增强语义对齐，显著提升了面部属性编辑的精度与保真度。
...
TL;DR: 本文提出GraphLeap，一种用于视觉图神经网络（ViGs）的高效架构与硬件加速方案，通过解耦图构建与特征更新、引入单层前瞻机制实现计算重叠，并设计了首个端到端FPGA加速器，在保持精度的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: ViGs中每层动态构建kNN图是主要性能瓶颈，耗时占比高、复杂度O(N²)、且造成图构建与特征更新的串行依赖。

Method: 提出GraphLeap：用第ℓ−1层特征构建第ℓ层图，同时用第ℓ层特征构建第ℓ+1层图，实现图构建与消息传递并发；并基于此设计流式、层流水线FPGA加速器，支持节点与通道级并行，避免显式边特征存储。

Result: 在Alveo U280 FPGA上，GraphLeap相比CPU最高加速95.7×，相比GPU最高加速8.5×；轻量微调即可恢复原始精度，支持实时ViG推理。

Conclusion: GraphLeap从算法与硬件协同优化角度突破ViGs的图构建瓶颈，验证了动态图神经网络在专用硬件上高效部署的可行性。
...
TL;DR: 本文系统研究了合成数据对可控人体视频生成的影响，提出了一种基于扩散模型的框架，支持外观与动作的细粒度控制，并揭示了合成数据与真实数据在训练中的互补作用，为提升运动真实性、时序一致性和身份保持提供了高效样本选择方法。


<details>
  <summary>Details</summary>
Motivation: 可控人体视频生成面临大规模、多样化且隐私安全的真实人体视频数据稀缺的瓶颈，尤其是罕见身份和复杂动作；合成数据虽具可扩展性和可控性，但其在生成建模中的实际贡献因Sim2Real差距而未被充分探索。

Method: 提出一种基于扩散模型的可控人体视频生成框架，支持外观与动作的细粒度控制，并构建统一测试平台以分析合成数据与真实数据在训练过程中的交互机制；通过大量实验探究合成数据的作用，并设计高效的合成样本选择策略。

Result: 揭示了合成数据与真实数据在提升运动真实性、时序一致性与身份保持方面的互补作用；验证了特定合成样本选择方法可显著增强生成视频质量；提供了首个针对人体视频合成中合成数据作用的系统性实证分析。

Conclusion: 合成数据在可控人体视频生成中具有重要且不可替代的价值，其与真实数据协同使用可提升模型泛化性与数据效率；本研究为构建更鲁棒、更通用的生成模型提供了理论依据与实践指导。
...
TL;DR: 本文提出了一种基于Vision Transformer的深度学习框架，用于MRI图像的四类脑肿瘤自动分类（胶质瘤、脑膜瘤、垂体瘤和健康组织），结合CLAHE增强、两阶段微调、MixUp/CutMix数据增强、EMA与TTA等技术，达到99.29%准确率和99.25%宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 手动解读MRI费时、易受观察者差异影响且依赖专家经验，而早期精准诊断对脑肿瘤患者生存率至关重要。

Method: 采用ImageNet-21k预训练的ViT-B/16作为骨干网络，引入CLAHE增强局部对比度；使用两阶段微调（先冻结主干训练分类头，再全网络微调并设置分层学习率）；每批次应用MixUp和CutMix增强；结合EMA权重和测试时增强（TTA）；并通过Attention Rollout生成可解释热力图。

Result: 在7023例MRI数据集上实现99.29%测试准确率、99.25%宏F1分数，健康与脑膜瘤类召回率为100%，全面优于CNN基线模型。

Conclusion: 该ViT-based框架在精度、鲁棒性与临床可解释性方面均表现出色，为自动化脑肿瘤MRI诊断提供了高效可靠的新范式。
...
<details>
  <summary>Details</summary>
Motivation: 为推动遥感红外图像超分辨率研究，满足实际应用需求，反映红外数据特性，组织该挑战赛。

Method: 采用单轨制竞赛设置，基于双三次下采样（x4）生成LR图像，评估参赛模型在红外图像SR任务上的性能。

Result: 共115人注册，13支队伍提交有效方案；报告总结了各队代表性方法与主要结果。

Conclusion: 该挑战赛为红外图像超分辨率研究提供了新基准，促进了面向真实遥感应用的有效解决方案的发展。
...
TL;DR: 本文提出PLAS-Net，一种用于无人机图像的像素级海滩垃圾面积分割网络，以克服传统边界框检测对不规则垃圾面积的高估问题；在泰国 Koh Tao 海滩数据上验证其优越性能，并通过三项下游分析展示像素级面积信息对生态风险评估的关键价值。


<details>
  <summary>Details</summary>
Motivation: 传统基于边界框的无人机监测方法会系统性高估不规则海滩垃圾的平面面积，而准确量化物理暴露面积对海洋垃圾生态风险评估至关重要。

Method: 提出PLAS-Net——一种基于实例分割的像素级垃圾面积提取框架，用于从无人机影像中获取海岸垃圾的精确物理轮廓。

Result: 在泰国Koh Tao季风影响下的小海湾无人机影像上，PLAS-Net达到mAP_50为58.7%，精度高于11种基线模型；并在三项下游任务（碎片化动力学建模、面积加权生态风险制图、源组成分析）中验证了其实际价值。

Conclusion: 像素级面积提取比单纯计数更能支撑科学、可信的海岸带垃圾监测与生态风险评估，尤其揭示了‘丰度-面积悖论’等关键现象。
...
<details>
  <summary>Details</summary>
Motivation: 现有煎炸油检测依赖破坏性湿化学方法，无法提供空间信息且不支持实时监测；热成像方法存在‘相机指纹捷径’问题，模型学习传感器噪声而非氧化化学，导致跨视频性能崩溃。

Method: 提出FryNet双流框架：热流采用ThermalMiT-B2骨干网加通道/空间注意力；RGB流采用RGB-MAE编码器，结合掩码自编码与化学对齐学习化学感知表征；双编码器DANN通过梯度反转层对抗视频身份偏差；FiLM融合模块桥接热结构与RGB化学上下文。

Result: 在28个煎炸视频共7226对帧上，FryNet达到98.97% mIoU、100%分类准确率、2.32平均回归MAE，全面超越7种基线方法。

Conclusion: FryNet有效克服热成像中的相机指纹捷径问题，首次实现了基于视觉的端到端、多任务、高精度、跨视频鲁棒的煎炸油氧化状态评估，为食品质量安全实时监测提供了新范式。
...
TL;DR: 本文提出HiTPro框架，用于无监督视频可见光-红外行人重识别，通过分层时间原型化方法，在无需身份标签的情况下实现跨模态匹配。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖图像级特征或大量标注数据，而无监督视频VI-ReID问题尚未被充分研究，尤其在真实场景中缺乏身份标签时亟需有效解决方案。

Method: HiTPro包含三个核心模块：1）时序感知特征编码器提取帧级与轨迹级特征；2）基于子轨迹的相机内原型构建与分层跨原型对齐（含动态阈值与软权重分配）；3）三级分层对比学习（相机内判别、跨相机同模态一致性、跨模态不变性）。

Result: 在HITSZ-VCM和BUPTCampus数据集上，HiTPro在完全无监督设置下达到SOTA性能，显著优于适配基线方法。

Conclusion: HiTPro为无监督视频VI-ReID提供了新范式，避免硬伪标签分配，提升了跨模态匹配鲁棒性与实用性，建立了该方向的强基线。
...
TL;DR: 本文提出MiMIC模型，通过融合解码器架构和鲁棒训练策略（单模态混合与随机标题丢弃），解决通用多模态检索中视觉模态坍塌和语义错位问题，在WebQA+和EVQA+数据集上优于早期和晚期融合基线。


<details>
  <summary>Details</summary>
Motivation: 现有通用多模态检索（UMR）方法存在视觉模态坍塌（如Marvel）或语义错位（如UniVL-DR）问题，需更鲁棒的融合机制以应对无标题图像等现实挑战。

Method: 提出MiMIC：采用融合解码器架构实现有效多模态集成，并引入单模态 mixin 和随机标题丢弃进行鲁棒训练。

Result: 在WebQA+和EVQA+数据集上，MiMIC在图像缺乏标题的场景下持续超越早期融合（Marvel）和晚期融合（UniVL-DR）基线。

Conclusion: MiMIC通过新架构与训练策略，缓解了模态坍塌与语义错位，提升了多模态检索鲁棒性与泛化能力。
...
TL;DR: 本文提出TGR-MoE方法，利用预训练稠密教师模型指导稀疏MoE中路由器的学习，缓解梯度阻塞与路由不稳定问题，提升图像分类任务的精度与路由一致性。


<details>
  <summary>Details</summary>
Motivation: 稀疏MoE中路由器因仅从被选专家获得梯度而面临梯度阻塞和局部反馈不足问题，导致路由选择不稳定、专家分配波动大。

Method: 提出教师引导路由（TGR-MoE）：从预训练稠密教师模型的中间表征构建教师路由器，并将其路由输出作为伪标签监督学生路由器训练，从而稳定路由学习并实现早期知识引导的专家选择。

Result: 在ImageNet-1K和CIFAR-100上实验表明，TGR-MoE显著提升模型准确率与路由一致性，并在高稀疏配置下保持训练稳定性。

Conclusion: TGR-MoE是一种简单有效的方法，通过教师模型提供结构化路由监督，解决了稀疏MoE中路由器优化困难的核心问题，为视觉领域稀疏模型训练提供了新思路。
...
TL;DR: 本文提出了一种面向航拍图像自监督学习（SSL）的鲁棒性增强方法，通过引入逐样本、逐退化因子的‘信任权重’并以加性残差形式融入对比损失，避免因严重退化视图对齐导致的潜在空间伪结构；该方法在多个遥感数据集上显著提升线性探测准确率与零样本跨域异常检测性能，并具备不确定性可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在航拍图像中强制增强视图间不变性，但航拍图像常受雾霾、运动模糊、雨、遮挡等退化影响，导致语义信息丢失；强行对齐干净视图与严重退化视图会向潜在空间引入伪结构。

Method: 提出一种训练策略与网络结构改进：引入逐样本、逐退化因子的信任权重，以加性残差方式加入基础对比损失；对信任权重施加stop-gradient而非乘性门控；并探索了基于Dempster-Shafer证据融合的可解释变体。

Result: 在EuroSAT、AID、NWPU-RESISC45上取得最高平均线性探测准确率（90.20%），显著优于SimCLR（88.46%）和VICReg（89.82%）；在EuroSAT严重雾霾（s=5）下相对SimCLR提升19.9点；在BDD100K跨域零样本AUROC上稳定提升1–3点；消融实验证实加性残差设计是主要增益来源。

Conclusion: 加性残差形式的信任权重机制是一种有效的不确定性感知SSL设计原则，兼顾鲁棒性、性能提升与可解释性，为退化敏感场景下的自监督学习提供了新范式。
...
Conclusion: PTA是一种高效、可扩展且高性能的TTA范式，通过原型机制替代缓存，兼顾准确性与推理效率，为VLMs的测试时自适应提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有创意生成研究主要集中于文本和图像，而创意视频生成（CVG）因文本到视频模型存在语义对齐模糊和运动适应性不足两大挑战而发展滞后。

Method: 构建广告创意知识库（ACKB），并提出KD-CVG框架，包含语义感知检索（SAR）模块（融合图注意力网络与强化学习反馈）和多模态知识参考（MKR）模块（引入语义与运动先验）。

Result: 实验表明KD-CVG在语义对齐和运动适应性方面显著优于现有SOTA方法。

Conclusion: 知识驱动的方法能有效弥补T2V模型的知识缺陷，提升广告创意视频生成的质量与实用性；代码与数据集将开源。
...
TL;DR: 本文提出了一种名为EdgeFormer的基于学习的3D点云边缘检测网络，通过将全局点云边缘检测转化为基于局部补丁的点分类任务，有效提升了细粒度边缘特征的检测能力。


<details>
  <summary>Details</summary>
Motivation: 3D点云中的边缘点能清晰表达几何与表面特性，但细粒度边缘因密集分布或微小表面梯度而难以有效检测。

Method: EdgeFormer包含两个阶段：第一阶段构建每个点邻域的局部补丁特征描述符；第二阶段基于这些描述符对每个点进行分类以判断是否为边缘点。

Result: 实验结果表明，该模型在六种基线方法上展现出具有竞争力的性能。

Conclusion: 将点云转换为局部补丁有助于更有效地提取细微边缘细节，EdgeFormer是一种有效的3D点云边缘检测方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLM评估数据集受限于人工标注成本高、缺乏推理步骤与图像区域的显式对齐，难以评估模型推理的可信性。

Method: 构建VG-CoT数据集：1）用SOTA检测与OCR提取对象和文本级视觉证据；2）用GPT-4o生成逐步接地推理；3）通过理由驱动的开放集检测优化接地。并设计涵盖推理质量、答案准确率、推理-答案对齐度的三维评估基准。

Result: 在LLaVA-1.5、Qwen2-VL等模型上实验表明，VG-CoT显著提升多数评估指标，验证其在增强可信推理的同时保障数据集构建的可扩展性与低成本。

Conclusion: VG-CoT为LVLMs提供了可扩展、自动化的接地推理评估范式，推动模型从‘黑箱推理’走向‘证据可追溯’的可信视觉语言理解。
...
TL;DR: 本文提出YOGO框架和Immersion v1.0数据集，旨在解决3D高斯泼溅（3DGS）在工业落地中的资源不可控、评估偏差与多传感器干扰问题，实现确定性、高保真、可部署的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法存在三大工业落地障碍：高斯增长启发式导致资源消耗不可预测；当前稀疏基准‘稀疏护盾’鼓励幻觉而非物理保真；多传感器数据严重污染。

Method: 提出YOGO系统级框架，将随机高斯增长重构为确定性、预算感知的平衡过程，包含硬件约束的预算控制器和鲁棒多传感器融合的可用性注册协议；同时发布首个超密室内数据集Immersion v1.0，提供饱和视角覆盖以打破‘稀疏护盾’。

Result: YOGO在保持严格确定性的同时达到SOTA视觉质量；Immersion v1.0推动重建保真度上限评估；部分数据与代码已开源。

Conclusion: YOGO与Immersion v1.0共同弥合了学术研究与工业部署之间的鸿沟，为生产级3DGS设立了新标准。
...
TL;DR: S1-VL是一种面向科学领域的多模态推理模型，支持结构化科学推理与‘图像思维’两种范式；后者通过在沙盒中执行Python图像处理代码实现迭代视觉推理；模型基于六学科科学数据训练，采用多阶段质量过滤与自适应数据路由策略，并在多个科学与图像理解基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有科学多模态数据集中存在冗余、无效和错误的视觉操作，缺乏对高分辨率图表、显微图像和几何辅助推理等复杂场景的有效支持，亟需一种能智能判断并执行必要图像操作的推理范式。

Method: 提出S1-VL模型，融合Scientific Reasoning（链式结构化推理）与Thinking-with-Images（沙盒内生成并执行图像处理Python代码、多轮迭代获取中间视觉结果）；构建覆盖六大科学领域的高质量多模态数据集，设计六维质量过滤框架与多阶段过滤+自适应数据路由策略（低信息增益样本转为纯推理模式）；采用四阶段渐进训练流程（SFT→冷启动→两阶段SAPO强化学习）。
...
Conclusion: S1-VL成功实现了科学领域中符号推理与具身图像操作的有机协同，其自适应视觉操作决策机制和高质量训练范式为多模态科学AI提供了新范式。
...
TL;DR: 本文提出了一种基于非线性扩散滤波的新方法，用于生成分段常数图像以辅助图像分割；通过引入与反向扩散相关的新型扩散系数，并在半隐式格式下求解前向非线性扩散方程，实现了边缘保持和低计算开销的分段常数化效果。
...
Method: 建立非线性扩散方程的内在形式化框架，设计一类源于反向扩散思想的新扩散系数；采用满足尺度空间性质的半隐式离散格式求解前向扩散方程，并设定扩散停止准则。

Result: 所提滤波器满足半离散与全离散尺度空间的良好适定性；在真实图像上验证了其能高效生成边缘锐利、内部均质的分段常数图像。

Conclusion: 该方法在保证数学严谨性（如尺度空间性质）的同时，实现了边缘保持与计算效率的统一，为后续分割提供了更鲁棒的预处理结果。
...
TL;DR: 本文提出UHR-DETR，一种面向超高清遥感影像的高效端到端Transformer检测器，通过稀疏编码器与全局-局部解耦解码器，在有限显存下兼顾小目标检测精度与推理速度。


<details>
  <summary>Details</summary>
Motivation: 超高清（UHR）遥感影像中检测小目标面临内存瓶颈与上下文丢失的两难困境：保持高分辨率导致显存爆炸，而降采样或裁块则分别损害小目标可见性或破坏场景语义完整性。

Method: 提出UHR-DETR：1）Coverage-Maximizing Sparse Encoder——动态聚焦于信息丰富区域，减少空间冗余；2）Global-Local Decoupled Decoder——联合建模宏观场景结构与微观目标细节，缓解语义歧义和场景碎片化。

Result: 在STAR和SODA-A等UHR数据集上，单卡RTX 3090（24GB）约束下，mAP提升2.8%，推理速度快于滑动窗口基线10倍。

Conclusion: UHR-DETR有效突破UHR影像小目标检测的效率-精度权衡瓶颈，为资源受限场景下的高分辨率理解提供了新范式。
...
Result: 在kNN搜索中比Kd-tree快51.111%、比Octree快94.159%；在RN搜索中分别快54.519%和41.840%。

Conclusion: 2L-LSH是一种高效、准确的大规模点云邻近搜索方法，具有广泛应用潜力。
...
TL;DR: 本文提出VARestorer，一种基于蒸馏的单步图像超分辨率方法，通过分布匹配、金字塔图像条件化与跨尺度注意力机制，克服视觉自回归模型（VAR）在真实图像超分辨率（Real-ISR）中因因果注意力和迭代误差累积导致的模糊与不一致问题，在保持原VAR表达能力的同时显著提升效率与性能。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型（VAR）虽在图像生成中表现优异，但其因果注意力机制限制了对全局低质量（LQ）上下文的利用，且迭代预测易导致误差累积，难以直接适用于真实图像超分辨率（Real-ISR）任务。

Method: 提出VARestorer蒸馏框架：1）分布匹配实现单步ISR，避免迭代误差；2）金字塔图像条件化+跨尺度注意力，支持双向尺度交互，增强LQ信息利用；3）仅微调1.2%参数（参数高效适配器），保留预训练VAR的表达能力。

Result: 在DIV2K数据集上达到SOTA：MUSIQ 72.32、CLIPIQA 0.7669；推理速度较传统VAR快10倍。

Conclusion: VARestorer成功将文本到图像VAR模型高效迁移至Real-ISR任务，验证了蒸馏+结构适配策略在克服VAR固有局限性上的有效性，兼顾性能、一致性与效率。
...
TL;DR: 本文提出OA-VAT框架，通过实例感知离线原型初始化、在线原型增强跟踪器和遮挡感知轨迹规划器三个模块，解决视觉主动跟踪中相似干扰物混淆与遮挡下失效两大问题，在多个数据集和真实无人机平台上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视觉主动跟踪（VAT）在现实部署中面临两个关键瓶颈：因实例级判别能力不足导致的视觉相似干扰物混淆，以及因缺乏主动规划导致的严重遮挡失效。

Method: 提出OA-VAT统一框架，包含：1）无需训练的实例感知离线原型初始化（基于DINOv3多视角特征聚合）；2）在线原型增强跟踪器（结合置信度感知卡尔曼滤波）；3）遮挡感知轨迹规划器（基于自建Planning-20k数据集与条件扩散模型）。

Result: 在UnrealCV上平均成功率（SR）达0.93（+2.2% vs. SOTA TrackVLA）；真实数据集平均类别准确率（CAR）达90.8%（+12.1% vs. SOTA GC-VAT）；DJI Tello无人机上任务成功率（TSR）达81.6%；RTX 3090上运行速度为35 FPS。

Conclusion: OA-VAT在保持实时性的同时显著提升了复杂场景下的鲁棒性与准确性，为实际应用提供了可行解决方案。
...
TL;DR: 本文提出EgoPoint-Bench基准，用于评估和提升以自我为中心视角下多模态指向推理能力，解决现有MLLM在指向空间语义定位上的‘指代幻觉’问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在以自我为中心的指向手势理解中存在‘指代幻觉’问题，即依赖视觉邻近性或物体显著性等虚假相关性，而非准确建模空间语义。

Method: 构建了包含11000+高保真仿真与真实世界样本的EgoPoint-Bench基准，涵盖五个评估维度和三个指代复杂度层级，并利用合成数据对模型进行微调。

Result: 实验表明，现有SOTA模型在该任务上表现不佳，而基于合成数据微调的模型显著提升性能，并展现出强健的仿真到真实泛化能力。

Conclusion: 空间感知监督对提升以自我为中心AI助手的指向理解精度至关重要，EgoPoint-Bench为该方向提供了可扩展的评估与优化路径。
...
TL;DR: ID-Eraser是一种面向特征空间的主动防御方法，通过在身份嵌入中注入可学习扰动并利用Face Revive Generator生成自然保护图像，有效防止人脸交换攻击，同时保持视觉真实性。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素扰动的主动防御方法对提取鲁棒高层身份嵌入的现代换脸模型无效，亟需一种能直接干扰身份表征的防御机制。

Method: 提出ID-Eraser：在身份嵌入空间注入可学习扰动，并通过Face Revive Generator（FRG）重建视觉自然的保护图像；在严格黑盒设置下评估其对多种识别与换脸系统的干扰能力。

Result: ID-Eraser显著降低身份识别准确率（Top-1仅0.30），保持高质量图像指标（FID=1.64, LPIPS=0.020）；换脸后身份相似度降至0.504；跨数据集泛化性强，对常见失真鲁棒，并成功降低腾讯API相似度（0.76→0.36）。

Conclusion: ID-Eraser首次实现了在特征空间对身份信息的有效擦除，在保障图像自然性的同时，显著削弱主流Deepfake模型的换脸能力，具备实际部署潜力。
...
TL;DR: 本文提出了一种新的跨数据集评估指标Cross-AUC，揭示了现有伪造检测器在跨域分数可比性上的严重缺陷，并提出了SFAM框架以提升泛化能力与鲁棒性。
...
Method: 提出Cross-AUC指标用于评估跨数据集的分数可比性；设计SFAM框架，包含语义细粒度图像-文本对齐模块和面部区域专家混合模块，分别增强CLIP对篡改痕迹的敏感性及实现区域感知分析。

Result: 在多个公开数据集上实验表明，SFAM显著优于现有SOTA方法，且Cross-AUC暴露出主流检测器在跨域场景下性能大幅下降。

Conclusion: 跨域分数可比性是影响伪造检测泛化性的关键因素；SFAM通过多模态对齐与区域专家协同，有效提升了模型鲁棒性与泛化能力。
...
TL;DR: 本文提出了一种评估大语言模型（LLMs）在自动驾驶中对动态交通参与者行为和道路网络拓扑理解能力的框架，通过冻结LLM作为推理引擎，结合交通编码器与轻量CNN分别提取轨迹与高精地图特征，并经重编程适配器转换为LLM可处理的token，最终由线性解码器输出预测轨迹。


<details>
  <summary>Details</summary>
Motivation: 安全地将大语言模型应用于自动驾驶的感知与预测任务，需深入理解动态交通参与者行为和静态道路基础设施，但现有研究对此缺乏系统评估框架。

Method: 构建一个融合冻结LLM、交通编码器（提取轨迹空间特征）、轻量CNN（编码高精地图）和重编程适配器（将多模态特征转为LLM兼容token）的框架，仅用线性解码器输出未来轨迹，以剥离LLM自身推理能力进行定量分析。

Result: 该框架实现了对多模态信息（尤其是地图语义）对轨迹预测精度影响的定量评估，支持多种冻结LLM无缝接入且适配成本低，展现出跨LLM架构的强泛化能力。

Conclusion: 所提框架为评估LLM在自动驾驶场景中的内在推理能力提供了统一、可量化、低适配开销的平台，验证了引入道路地图语义对提升轨迹预测性能的关键作用。
...
TL;DR: 本文提出VFM$^{4}$SDG框架，利用冻结的视觉基础模型作为跨域稳定性先验，通过编码阶段的关系先验蒸馏和解码阶段的语义-上下文查询增强，提升单域泛化目标检测器在复杂域偏移下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实场景中天气、光照和成像条件持续变化导致显著域偏移，现有单域泛化目标检测方法忽视检测器机制本身，尤其在编码和解码阶段的跨域稳定性不足，造成漏检增多。

Method: 提出VFM$^{4}$SDG双先验学习框架：1）编码阶段采用跨域稳定关系先验蒸馏，增强物体-背景及实例间关系建模鲁棒性；2）解码阶段采用语义-上下文先验驱动的查询增强，将类别语义原型与全局视觉上下文注入查询，提升语义识别与空间定位稳定性。

Result: 在标准SDGOD基准及两种主流DETR检测器上，该方法持续超越现有SOTA方法，验证了其有效性、鲁棒性与通用性。

Conclusion: 引入冻结视觉基础模型作为跨域稳定性先验，并分别在编码与解码阶段设计针对性先验学习策略，可显著缓解单域训练检测器在未知域中的性能退化问题。
...
TL;DR: 本文提出了一种基于高斯混合模型（GMM）的局部描述符GMD，用于碎片断裂表面匹配与自动重装配，通过区域分割估计GMM参数，结合L2距离、RANSAC和ICP实现匹配与配准，在多个真实扫描数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在激光扫描碎片自动重装配中，断裂表面的精确匹配是关键挑战，现有方法在描述复杂断裂几何结构方面存在不足。

Method: 提出高斯混合描述符（GMD）：将局部表面划分为凹凸区域以估计GMM的k值，分别建模后融合为完整GMD；使用L2距离度量GMD相似性，结合RANSAC和ICP进行碎片配准。

Result: 在公开真实扫描数据集及秦始皇兵马俑（Terracotta）数据集上实验表明，该方法在断裂表面匹配精度和重装配效果上优于多种现有方法。

Conclusion: GMD能更鲁棒、准确地刻画断裂表面几何分布特征，提升了碎片自动重装配的性能与实用性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型（VLMs）正被广泛用作自动评估器，但其评估结果的可靠性尚未得到充分研究和验证。

Method: 设计了针对关键错误维度（如物体幻觉、空间推理、事实 grounding、视觉保真度）的定向扰动，构建包含4000+扰动样本、覆盖40个扰动维度的综合基准，并在单答案打分、成对比较和参考引导三种评估范式下测试4个主流VLM评估器。

Result: 当前VLM评估器存在严重不可靠性：对扰动输出的检出率有时低于50%；尤其难以识别细粒度组合性与空间错误；对与输入图像矛盾的幻觉内容不敏感；成对比较虽更稳健但仍存在失败案例。

Conclusion: 现有VLM评估器可靠性不足，需谨慎用于模型基准测试与研发决策，亟需更鲁棒的评估方法。
...
TL;DR: 本文提出了一种基于注意力机制的多实例学习（ABMIL）框架，利用预训练病理基础模型作为补丁编码器，在全切片水平预测肺腺癌（LUAD）主要生长模式，显著降低标注负担，并在性能上优于简单补丁聚合基线。


<details>
  <summary>Details</summary>
Motivation: 肺腺癌（LUAD）分级依赖于准确识别生长模式，而传统深度学习方法需大量精细标注（如补丁级分类或分割），标注成本高。

Method: 提出注意力机制多实例学习（ABMIL）框架，使用冻结或微调的预训练病理基础模型（如Prov-GigaPath）提取补丁特征，再通过注意力机制聚合为全切片预测；采用滑片级监督训练。

Result: 微调编码器显著提升性能，其中Prov-GigaPath在ABMIL下达到最高一致性（κ = 0.699）；ABMIL比简单补丁聚合基线更鲁棒。

Conclusion: ABMIL能有效减少标注需求，实现可靠的全切片LUAD生长模式预测；后续将扩展至全模式分布估计并开展外部队列验证。
...
TL;DR: 本文提出了一种无需训练的基于组件的OOD检测框架CoOD，通过分解输入为功能组件，并设计CSS和CCS两个分数分别检测局部外观变化和跨组件组合不一致性，有效解决现有方法在细粒度和组合式OOD检测上的不足。
...
Method: 提出训练无关的Component-Based OOD Detection（CoOD）框架，基于‘识别-by-组件’理论，将输入分解为功能组件；设计Component Shift Score（CSS）检测局部外观偏移，Compositional Consistency Score（CCS）检测跨组件组合不一致性。

Result: CoOD在粗粒度和细粒度OOD检测任务上均取得一致性能提升。

Conclusion: CoOD通过组件化建模和双评分机制，克服了传统方法在检测粒度与组合性方面的缺陷，是一种高效、鲁棒且无需训练的OOD检测新范式。
...
TL;DR: 本文提出了一种基于深度核空间学习和最大均值差异（MMD）的无监督单视频动作分割方法，利用神经正切核（NTK）提升表征能力并避免平凡解，在多个基准上达到先进性能。


<details>
  <summary>Details</summary>
Motivation: 针对无法或不允许存储大规模数据集的应用场景，研究单视频级别的无监督动作分割问题。

Method: 在深度核空间中学习视频帧分布的近似表示，采用最大均值差异（MMD）作为分布间相似性度量，并使用神经正切核（NTK）定义该核空间以增强表达力并避免联合优化中的平凡解。

Result: 在六个标准基准上取得与当前最优单视频方法相当的结果；当段落数未知时，F1分数高于以往的凝聚式方法。

Conclusion: 基于NTK和MMD的单视频无监督动作分割框架有效、高效且鲁棒，为资源受限场景提供了可行方案。
...
TL;DR: CHRep是一种两阶段框架，用于从常规H&E染色切片稳健预测空间基因表达，通过结构感知表征学习和轻量级校准模块提升跨切片鲁棒性，在多个数据集上显著提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在留一切片交叉验证下易受切片级外观偏移和回归过度平滑影响，难以满足大规模队列研究和临床常规应用需求。

Method: CHRep包含训练阶段和推理阶段：训练阶段联合优化相关性感知回归、对称图像-表达对齐及坐标诱导的空间拓扑正则化；推理阶段引入无需骨干微调的轻量校准模块，结合非参数检索与幅度正则化修正。

Result: 在三个队列中，CHRep在留一切片评估下持续提升基因层面相关性；相比HAGE，cSCC和HER2+数据集的PCC(ACG)分别提升4.0%和9.8%；相比mclSTExp，在Alex+10x上PCC(ACG)提升39.5%，MSE和MAE分别降低9.7%和9.0%。

Conclusion: CHRep通过耦合拓扑保持表征学习与后处理校准，有效缓解切片级偏移与过度平滑问题，为H&E到空间转录组预测提供了更鲁棒、准确的解决方案。
...
TL;DR: OmniFit是一种尺度无关的多模态人体模型拟合方法，能处理点云、深度图和图像等多种输入，通过条件Transformer解码器预测体表关键点并拟合SMPL-X参数，配合图像适配器和尺度预测器，在真实与合成资产上均达到毫米级精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法多依赖单一模态输入且要求已知度量尺度，而AI生成资产常存在尺度失真，难以满足实际需求。

Method: 提出OmniFit框架：1）使用条件Transformer解码器将表面点直接映射为密集身体关键点；2）引入可选图像适配器融合视觉线索以弥补几何信息缺失；3）设计专用尺度预测器将主体重缩放至标准身体比例。

Result: 在日常与宽松服装场景下，性能比SOTA方法提升57.1%–80.9%；首次超越多视角优化基线，并在CAPE和4D-DRESS基准上实现毫米级精度。

Conclusion: OmniFit是首个支持多模态、尺度无关、适用于真实与合成人体资产的高精度拟合方法，显著提升了复杂服装下的人体建模鲁棒性与精度。
...
TL;DR: Sculpt4D 是一种原生 4D 生成框架，通过在预训练 3D 扩散 Transformer 中引入块稀疏注意力机制，实现高效、高保真的动态 4D 生成，显著降低计算开销并提升时序一致性。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成方法在静态形状合成上取得进展，但高保真动态 4D 生成仍受限于时序伪影和高昂计算成本，且缺乏充足 4D 训练数据。

Method: 提出 Sculpt4D 框架，将块稀疏注意力机制嵌入预训练 3D 扩散 Transformer（Hunyuan3D 2.1）；该机制以首帧为锚点保持物体身份，并通过时间衰减稀疏掩码建模运动动态。

Result: 显著缓解时序伪影，减少 56% 总体网络计算量，避免全注意力的平方级开销，在时序一致的 4D 合成中达到新 SOTA。

Conclusion: Sculpt4D 为高效、可扩展的 4D 生成提供了可行路径，推动了动态三维内容生成的发展。
...
TL;DR: 本文提出了一种评估参数化投影局部稳定性的新框架，通过高斯扰动探测锚点邻域在2D嵌入中的形变，并结合定量指标与可视化手段识别传统指标无法发现的不稳定区域。


<details>
  <summary>Details</summary>
Motivation: 参数化投影易受测量噪声或数据漂移影响，导致2D布局发生不可预测偏移，而其局部稳定性尚未被系统研究。

Method: 提出基于高斯扰动的稳定性评估框架，围绕选定锚点注入扰动，量化平均位移、偏差和最近锚点分配错误，并辅以位移向量图、局部PCA椭球和Voronoi错分可视化；在UMAP和t-SNE神经投影器上验证，并考察Jacobian正则化对鲁棒性的提升效果。

Result: 在MNIST和Fashion-MNIST数据集上验证了框架有效性，成功识别出重建误差和邻域保持度等传统指标无法检测的不稳定区域。

Conclusion: 该稳定性评估框架为参数化投影提供了更可靠的质量诊断工具，有助于提升其在实时嵌入场景下的可信度与鲁棒性。
...
TL;DR: 本文提出DCMorph，一种基于双流扩散模型的面部变形攻击框架，通过身份条件化和潜在空间操作提升攻击效果，在多个面部识别系统中表现出最高的攻击成功率且难以被检测。


<details>
  <summary>Details</summary>
Motivation: 推进面部变形攻击技术的发展，以预测不断演变的威胁并为身份验证系统开发强大的防御机制。

Method: DCMorph采用双流扩散模型，结合解耦交叉注意力插值（实现双重身份条件化）和DDIM反演加球面插值（生成几何一致的初始潜在表示），在身份条件化和潜在空间层面同时操作。

Result: 在四个最先进的面部识别系统上进行漏洞分析，DCMorph在不同操作阈值下均实现了比现有方法更高的攻击成功率，并且对当前的变形攻击检测方案具有更强的隐蔽性。

Conclusion: DCMorph通过创新的双流扩散机制，在保持高重建保真度的同时显著提升了面部变形攻击的有效性和隐蔽性，为评估和增强生物特征系统的安全性提供了新工具。
...
TL;DR: DualSplat提出Failure-to-Prior框架，利用首次重建失败（如不完整高斯碎片）生成对象级伪掩码，指导二次3DGS优化，并通过轻量MLP在线优化掩码，显著提升含瞬态物体场景的重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在含瞬态物体（违反多视角一致性的物体）的图像上训练时性能严重下降；现有方法存在瞬态检测与静态场景重建之间的循环依赖问题。

Method: 提出DualSplat框架：第一阶段保守训练产生失败模式（如不完整高斯碎片），据此融合光度残差、特征不匹配和SAM2实例边界生成对象级伪掩码；第二阶段用伪掩码引导干净的3DGS重建，并用轻量MLP在线细化掩码（从先验监督渐变为自一致性）。

Result: 在RobustNeRF和NeRF On-the-go数据集上超越现有基线，尤其在瞬态物体密集场景及瞬态区域效果显著。

Conclusion: 将重建失败显式转化为先验信息是解决瞬态物体干扰的有效新范式，DualSplat实现了更鲁棒、高质量的动态场景神经渲染。
...
TL;DR: 本文提出了一种基于因果推理与解耦表征学习的新型全参考图像质量评估（FR-IQA）范式，通过干预潜在表征实现退化估计的因果解缠，并利用内容不变性与视觉掩蔽效应建模内容对退化特征的影响，在多种监督设置及跨域场景中展现出优异性能与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度网络的全参考图像质量评估模型多依赖参考与失真图像之间的深层特征两两比对，缺乏对图像退化过程内在因果机制的建模；同时在标注稀缺或无标注的新领域图像（如医学、水下图像）上泛化能力不足。

Method: 提出基于因果推理的FR-IQA新范式：1）利用参考与失真图像的内容不变性解耦退化表征与内容表征；2）设计掩蔽模块模拟人类视觉掩蔽效应，建模内容对退化特征的因果影响，提取内容影响下的退化特征；3）采用有监督回归或无标签降维方式预测质量分。

Result: 在标准IQA基准（LIVE、TID2013等）上达到极具竞争力的性能，涵盖全监督、少标签和无标签设置；在水下、X光、医学、中子、屏幕内容等非标准自然图像域中，无需IQA标注即可实现场景自适应训练与预测，跨域泛化能力显著优于现有无训练FR-IQA方法。

Conclusion: 因果驱动的解耦表征学习为FR-IQA提供了更可解释、更鲁棒且更具跨域适应性的新路径，突破了传统特征比对范式的局限，尤其适用于标注稀缺的真实复杂图像场景。
...
TL;DR: 本文提出CHAI框架，通过结构化视频描述规范与人类专家对AI生成字幕的批判性监督，显著提升视频语言模型的字幕生成质量，并推动开源模型（如Qwen3-VL）在生成、奖励建模与批判生成方面的进步，最终实现对专业级视频（电影、广告等）的高精度重字幕及细粒度视频生成控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在专业级视频理解与生成中缺乏精确描述能力与高质量监督机制，亟需结合领域知识（如电影制作）和高效人机协作范式来提升性能。

Method: 提出结构化视频描述规范（涵盖主体、场景、运动、空间与镜头动态），构建CHAI框架：由人类专家对模型生成的预字幕进行批判与修订，形成高质量后字幕；利用批判内容与偏好数据，通过SFT、DPO和推理时扩展训练开源VLM；并将方法应用于大规模专业视频重字幕与视频生成模型（如Wan）微调。

Result: CHAI监督下训练的模型在字幕生成上超越Gemini-3.1-Pro；批判质量（精确率、召回率、建设性）直接影响下游性能；成功实现对400词级详细提示的视频生成控制，涵盖镜头运动、角度、焦距、视点与构图等电影级参数。

Conclusion: 精确的结构化规范与高效的人类-AI协同监督是实现专业级视频理解与生成的关键路径，为开放视频AI生态提供了可扩展、可复现的方法论与资源。
...
TL;DR: 本文提出Structured Motion Description (SMD)，一种基于生物力学启发的规则化方法，将人体关节运动序列转化为结构化自然语言描述，使大语言模型（LLM）无需专用编码器即可直接理解与推理运动，显著提升运动问答与动作描述任务性能，并具备跨模型兼容性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的人体运动理解方法受限于跨模态对齐和专用编码器设计，未能充分利用LLM已有的世界知识和推理能力；而生物力学中关节角度与身体部位运动本就是描述动作的精确语言，值得被用于构建更直接、可解释的运动表征。

Method: 提出Structured Motion Description（SMD），通过确定性规则将原始关节位置序列转换为包含关节角度、身体部位运动及全局轨迹的结构化自然语言描述；该文本输入可直接送入各类LLM，并仅需轻量LoRA微调适配。

Result: 在BABEL-QA（66.7%）和HuMMan-QA（90.1%）运动问答任务上超越SOTA；在HumanML3D动作描述任务上达到R@1=0.584、CIDEr=53.16；验证了SMD在8个LLM（6个模型家族）上的泛化能力，并支持可解释的注意力分析。

Conclusion: SMD证明了将运动显式建模为结构化文本是比隐式跨模态对齐更高效、更可解释、更具泛化性的路径，为LLM驱动的运动理解提供了新范式。
...
TL;DR: 本文提出HalluScope基准来分析大视觉语言模型（LVLMs）幻觉现象的成因，发现主要源于文本先验和指令中引入的背景知识；为此设计了HalluVL-DPO偏好优化框架，通过定制训练数据微调模型以增强视觉依据性，并在保持其他能力的同时有效缓解特定幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LVLMs幻觉成因（如视觉骨干限制或语言组件主导）的重要性尚不明确，亟需系统性归因分析。

Method: 构建HalluScope基准进行归因分析；提出HalluVL-DPO框架，基于自建偏好数据集，采用直接偏好优化（DPO）微调LVLMs，引导模型选择更视觉 grounded 的响应。

Result: HalluVL-DPO显著缓解由文本指令先验引发的幻觉，在多个幻觉评测和其他视觉能力评估上保持或提升性能。

Conclusion: LVLMs幻觉主因是过度依赖文本先验（尤其指令中引入的知识），而非视觉骨干缺陷；通过针对性偏好优化可有效提升视觉 grounding 能力。
...
TL;DR: Sapiens2 是一个面向人像视觉的高分辨率Transformer模型家族，涵盖0.4B到5B参数规模，支持1K至4K原生分辨率；通过融合掩码图像重建与自蒸馏对比学习的统一预训练目标、10亿高质量人像数据集及窗口注意力等架构改进，在姿态估计、人体部件分割、法线估计等任务上显著超越前代并拓展至点图与反照率估计等新任务。


<details>
  <summary>Details</summary>
Motivation: 提升人像视觉模型在通用性、多功能性和高保真输出方面的性能，解决前代模型在低级细节捕捉（密集预测）与高级语义理解（零样本/少标签场景）之间的平衡问题。

Method: 提出统一预训练目标（掩码图像重建 + 自蒸馏对比学习），构建10亿高质量人像数据集并增强标注质量与数量，采用窗口注意力等先进架构支持4K输入与2K输出的长程空间建模，并实现更稳定、更长周期的训练。

Result: 在姿态估计（+4 mAP）、人体部件分割（+24.3 mIoU）、法线估计（角误差降低45.6%）等任务上显著超越Sapiens1，并首次支持点图和反照率估计等新任务；4K模型具备长空间上下文推理能力。

Conclusion: Sapiens2通过算法、数据与架构三方面协同创新，确立了人像视觉领域的新SOTA，验证了高分辨率、统一预训练与高质量数据对通用人像理解模型的关键作用。
...
TL;DR: 本文提出了一种基于可微逻辑门网络（LGN）的视频拷贝检测框架，通过二值化预处理、帧微型化和可训练LGN嵌入模型，实现紧凑逻辑表示与极快推理，显著降低计算开销和描述符大小，同时保持或超越现有方法的精度。


<details>
  <summary>Details</summary>
Motivation: 视频拷贝检测需在大规模和多种视觉失真下实现鲁棒相似性估计，但现有深度神经网络因计算开销大、描述符尺寸大而难以部署于高吞吐系统。

Method: 提出基于可微逻辑门网络（LGN）的框架，结合帧微型化、二值预处理及可训练LGN嵌入模型，学习逻辑运算与连接结构；训练后可离散化为纯布尔电路以加速推理。

Result: LGN模型在多个数据集和难度级别上达到与或优于先前模型的准确率与排序性能，描述符尺寸小数个数量级，推理速度超11k样本/秒。
...
TL;DR: 本文提出了一种脑启发的协同框架，结合视觉Transformer的判别能力与条件扩散模型的生成能力，以提升单目RGB图像中3D人体网格恢复在遮挡场景下的准确性与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有回归方法在遮挡下易产生不合理的估计，而扩散方法虽能补全遮挡区域，却可能因过度依赖生成先验而削弱对罕见姿态的保真度。

Method: 构建双路径架构：ViT路径提取可见区域确定性视觉线索，扩散路径生成结构一致的人体表征；引入多样-一致特征学习模块对齐判别特征与生成先验，并设计跨注意力多级融合机制实现语义层级间的双向交互。

Result: 在标准基准上关键指标表现优越，且在复杂真实场景中展现出强鲁棒性。

Conclusion: 判别式与生成式模型的协同融合可有效兼顾精度与结构合理性，为遮挡下的3D人体重建提供了新思路。
...
Method: 开展严谨消融实验，提出一致性损失函数（对齐深度图、相机参数和点图）及高效高分辨率架构设计，并构建CARVE模型。

Result: CARVE在点云重建、视频深度估计和相机位姿/内参估计任务上，在多个基准测试中展现出强而稳健的性能。

Conclusion: 提升数据多样性与质量、避免有害损失机制、采用联合序列与帧级监督可显著提升性能；优化方法与高分辨率输入的有效融合是关键突破。
...
TL;DR: 本文提出Ramen框架，通过主动样本选择实现鲁棒的测试时自适应，以应对混合域分布偏移问题，利用嵌入-梯度缓存提升效率，并在多个基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法假设测试样本来自单一一致域，但在实际中常面临混合域数据，导致性能下降。

Method: Ramen框架基于领域一致性与预测平衡两个准则，为每个测试样本动态检索相关历史样本；采用嵌入-梯度缓存机制，复用历史样本的嵌入与梯度进行高效模型更新，避免额外前向/后向传播。

Result: 在多种图像损坏和域偏移基准上，Ramen展现出强而稳定的表现，显著优于现有方法，尤其在混合域场景下鲁棒性突出。

Conclusion: Ramen提供了一种高效、鲁棒的测试时自适应新范式，有效缓解混合域分布偏移带来的性能退化问题，具备理论支撑与实用价值。
...
TL;DR: 本研究提出了一种基于生物行为特征的可解释深度伪造检测方法，利用面部动态的低维运动模式提取时序特征，发现深伪视频在情感表达时存在更显著的高阶时序异常；该方法不仅实现了显著高于随机水平的分类性能，还揭示了模型判断与人类感知在情感视频中的一致性及非情感视频中的差异，表明可解释计算特征与人类感知具有互补性。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法多依赖黑箱深度学习模型，缺乏对真实与伪造面部行为差异的可解释性理解；同时，忽视了模型决策与人类感知判断之间的关系。

Method: 提取面部动态的低维生物行为运动模式，从中导出刻画时空结构的时序特征，并使用传统机器学习分类器进行深伪检测；进一步开展情感效价分类分析及模型-人类感知一致性评估。

Result: 基于生物行为特征的传统分类器实现显著高于随机水平的检测性能，尤其在含情感表达的视频中准确率更高；情感信号在深伪中系统性退化；模型与人类判断在情感视频中趋同、在非情感视频中分歧，且底层策略不同。

Conclusion: 面部交换型深伪视频具有可测量的行为指纹，其在情感表达时最为显著；可解释计算特征与人类感知并非冗余，而是互补的检测路径。
...
TL;DR: 本文提出DOCO框架，用于解决开放集持续测试时适应（OCTTA）这一新挑战，通过动态样本分割、基于ID样本学习域补偿提示，并将提示传播至OOD样本，协同提升域适应与OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法无法应对模型在推理时同时面临持续变化的域和未知语义类别的现实场景（即OCTTA），导致特征空间坍塌、分类与OOD检测性能严重下降。

Method: DOCO框架采用动态、适应条件下的样本分割分离ID与OOD样本；利用ID样本通过匹配源域特征统计并结合结构保持正则化学习域补偿提示；再将该提示传播至同批OOD样本以隔离其语义新颖性，实现闭环协同优化。

Result: 在多个具有挑战性的基准上，DOCO显著优于现有持续TTA（CTTA）和开放集TTA（OSTTA）方法，在OCTTA任务上达到新的SOTA性能。

Conclusion: DOCO是一种轻量且高效的方法，首次系统性地解决了OCTTA问题，在维持分类准确性的同时显著提升了OOD检测鲁棒性，为真实世界部署中的动态环境适应提供了新范式。
...
TL;DR: 本文提出了一种自监督框架，通过从单目视频中生成伪多视角三元组（源视频、几何锚点、目标视频）来解决动态视频重拍摄中多视角配对数据稀缺的问题；利用随机游走裁剪轨迹和前向扭曲生成锚点，迫使模型隐式学习4D时空结构，最终在推理中结合4D点云锚点实现高保真、时序一致的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 动态视频重拍摄中缺乏非刚性场景的配对多视角数据，严重制约了相机精确控制能力。

Method: 提出自监督框架，从单个视频中提取平滑随机游走裁剪轨迹作为源/目标视频，并通过稠密跟踪场前向扭曲首帧生成几何锚点，构建伪多视角训练三元组；模型需跨时空重投影纹理以重建目标，从而隐式学习4D时空结构。

Result: 在复杂动态场景上实现了最先进的时序一致性、鲁棒相机控制和高保真新视角合成。

Conclusion: 该方法无需真实多视角标注，可扩展地利用互联网规模单目视频，显著提升动态视频的可控重拍摄能力。
...
TL;DR: 本文探讨了如何利用计算机视觉方法分析社交媒体上的气候传播图像，通过基准测试多种视觉语言模型（VLMs），发现Gemini-3.1-flash-lite表现最优，并提出分布式评估比单图准确率更适用于大规模话语分析。


<details>
  <summary>Details</summary>
Motivation: 社交媒体已成为气候传播的主要阵地，但缺乏系统化分析图像内容以识别有效传播策略的方法；亟需可扩展、可靠的自动化工具支持气候传播研究。

Method: 构建应用导向的分类体系，对6种可提示视觉语言模型和15种零样本CLIP类模型在两个X平台图像数据集（含专家标注1038张和120万张带5万验证标签）上进行基准测试，涵盖5个标注维度；引入分布式评估、链式推理与标注维度特异性提示工程等策略。

Result: Gemini-3.1-flash-lite在所有超类别和两个数据集上均最优；中等规模开源模型性能接近；分布评估显示VLM能稳健恢复群体趋势；链式推理降低性能，而标注维度定制化提示提升性能。

Conclusion: VLMs（尤其Gemini-3.1-flash-lite）适合大规模气候传播话语分析；分布式评估比实例级准确率更具实际意义；提示设计应针对具体标注维度优化；代码与数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感深度标注、光照变化控制及多尺度配对图像稀缺，制约了几何理解、域自适应与超分辨率等任务的发展。

Method: 基于高保真城市仿真管线生成SyMTRS数据集，包含高分辨率RGB影像、精确深度图、夜间图像（用于域自适应）及x2/x4/x8尺度低分辨率配对图像。

Result: 构建了首个面向多任务（几何、域适应、超分）统一设计的遥感合成基准数据集，具备像素级深度真值与跨域一致性监督能力。

Conclusion: SyMTRS填补了遥感领域可控实验与高质量多任务监督数据的空白，推动相关方向研究发展。
...
TL;DR: 本文提出TEMA框架和两个多修改数据集M-FashionIQ与M-CIRR，以解决现有组合图像检索（CIR）中实体覆盖不足与子句-实体错位问题，显著提升实际应用能力。


<details>
  <summary>Details</summary>
Motivation: 现有CIR方法依赖简单修改文本，导致实体覆盖不足和子句-实体错位，难以满足实际应用需求。

Method: 构建两个指令丰富的多修改数据集M-FashionIQ和M-CIRR，并提出首个支持多修改且兼容单修改的CIR框架TEMA（Text-oriented Entity Mapping Architecture）。

Result: 在四个基准数据集上的实验表明，TEMA在原始及多修改场景下均优于现有方法，同时兼顾检索精度与计算效率。

Conclusion: TEMA有效缓解了CIR在实际应用中的关键限制，推动其向更复杂、更贴近用户真实表达的检索任务发展。
...
TL;DR: 本文提出了一种利用不同像素尺寸（或光学放大倍率）采集的多尺度低分辨率图像来解决平移模糊问题的超分辨率方法，证明了使用互质像素尺寸可使问题具有稳定逆解，并通过傅里叶域或迭代最小二乘法高效重建，同时量化了噪声与分辨率之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 解决超分辨率问题中由平移引起的模糊性，使问题良定化。

Method: 利用不同像素尺寸（互质）的传感器或多尺度光学放大获取低分辨率图像，结合傅里叶域处理和迭代最小二乘法进行重建，并理论分析噪声下大信号的重建误差。

Result: 证明了互质像素尺寸可保证系统具有稳定逆；实现了高效重建；推导出最小二乘重建误差表达式，揭示噪声-分辨率权衡；通过一维和二维CCD硬件分箱实验验证方法有效性。

Conclusion: 多尺度超分辨率是一种有效缓解平移模糊、提升重建稳定性和精度的新范式，对常规成像系统具有实际指导意义。
...
TL;DR: 本文提出诊断驱动的胶囊内镜（CE）视频摘要新任务，并构建首个具备诊断级标注的数据集VideoCAP；为解决该任务，设计了临床启发式框架DiCE，通过候选筛选、上下文组织与证据聚合三阶段实现高效准确的诊断摘要生成。


<details>
  <summary>Details</summary>
Motivation: 现有胶囊内镜研究集中于单帧分类与检测，缺乏对整段视频的诊断级分析；临床上需从超长、冗余、低质量视频中提取关键证据帧并做出准确诊断，但该任务尚未被形式化定义且缺乏合适数据集与方法。

Method: 提出DiCE框架：1）高效候选帧筛选；2）Context Weaver模块将候选帧组织为保持病灶事件独立性的诊断上下文；3）Evidence Converger模块在每个上下文中聚合多帧证据形成鲁棒的片段级判断。同时构建VideoCAP数据集，含240个全长视频及源自真实临床报告的诊断驱动标注。

Result: DiCE在VideoCAP上显著优于现有SOTA方法，生成简洁且临床可靠的诊断摘要；验证了诊断驱动的上下文推理范式在超长CE视频摘要中的有效性与潜力。

Conclusion: 诊断驱动的视频摘要是一项重要且具临床价值的新任务；DiCE框架及其所依赖的上下文建模与多帧证据聚合机制，为未来CE智能分析提供了可解释、可信赖的新思路。
...
TL;DR: 本文提出了一种面向物理视频理解的具身化评测基准，扩展了V-STaR的what-when-where框架，涵盖四类视频源、六类物理领域、三类提示模板及四种输入扰动条件，并基于1560个视频片段构建统一的具身事件记录；实验发现物理提示表现最强，vstar_like为最佳非物理语义对照，neutral_rstr为更难的模板控制，且空间定位能力最弱，强调评测需报告物理具身性、提示敏感性与扰动鲁棒性等多维指标。


<details>
  <summary>Details</summary>
Motivation: 现有模型可仅靠文本统计规律回答物理事件问题，却无法在时空上准确定位事件，缺乏真正的物理视频理解能力，因此需要一个具身化（grounded）、多维度的评测基准。

Method: 构建了一个扩展V-STaR框架的新基准：覆盖4个视频数据源、6个物理领域、3类提示家族（physics/vstar_like/neutral_rstr）和4种输入条件（original/shuffled/ablated/frame-masked）；所有1560个SSV2等来源的视频片段统一转换为共享的具身事件记录，并据此生成各提示家族的问题与时空标注。

Result: 物理提示家族整体性能最强；vstar_like是最清晰的非物理语义对照；neutral_rstr作为模板化控制更具挑战性；提示鲁棒性具有选择性而非普适性；性能提升主要出现在原始条件较弱的样本中；空间定位能力在所有设置下均最弱。

Conclusion: 视频问答评测不应只报告总体准确率，而应同时提供物理具身性、提示敏感性与扰动鲁棒性的细粒度诊断指标，以全面评估模型的真实物理理解能力。
...
Result: 所提方法可在无ISP访问权限下，从标准相机图像中有效恢复未幻觉化版本；编码器与MLP解码器总大小仅180 KB，可作为元数据嵌入JPEG和HEIC等常见图像格式。

Conclusion: 相机图像已非传统意义上的‘真实’，本文首次系统揭示了捕获时AI幻觉带来的真实性挑战，并提供了实用、轻量、可部署的反向恢复方案，提升用户对图像真实性的认知与控制能力。
...
TL;DR: 本文提出UniGenDet，一个统一的生成-判别框架，通过共生多模态自注意力机制和统一微调算法，实现图像生成与检测任务的协同进化，提升两者性能。


<details>
  <summary>Details</summary>
Motivation: 图像生成与检测领域虽各自快速发展，但架构范式迥异（生成网络 vs 判别框架），且虽都开始利用对抗信息，却缺乏有效协同机制。

Method: 提出UniGenDet框架，包含共生多模态自注意力机制、统一微调算法和检测器引导的生成对齐机制，以弥合生成与检测任务间的鸿沟并促进信息交互。

Result: 在多个数据集上的大量实验表明，该方法达到生成与检测任务的最先进性能。

Conclusion: 生成与检测任务可通过统一框架协同优化，二者可相互增强：生成提升检测可解释性，检测标准反哺生成保真度。
...
TL;DR: 本文通过分析人类与深度视觉模型在自然图像分类任务中的方向性混淆矩阵，揭示了二者在归纳偏置上的本质差异，并利用率失真（Rate-Distortion）框架量化其泛化几何特性，提出三个几何签名（斜率β、曲率κ和AUC效率），发现人类表现出宽泛而微弱的不对称性，而模型则呈现稀疏而强的方向坍缩；鲁棒训练虽降低全局不对称性，却无法复现人类式的‘宽-强’相似度梯度特征。


<details>
  <summary>Details</summary>
Motivation: 准确率无法揭示人类与深度视觉模型在错误模式上的系统性差异，尤其是方向性混淆所隐含的归纳偏置，亟需更细粒度的度量来刻画泛化行为的本质差异。

Method: 基于12种扰动下的人类与深度视觉模型在自然图像分类任务中的匹配响应，构建并分析混淆矩阵的不对称性；引入率失真（RD）框架，提取三个几何签名（斜率β、曲率κ、AUC）表征泛化几何；辅以机制性模拟验证不同不对称结构对RD前沿的影响。

Result: 人类表现出宽泛但微弱的方向性混淆不对称性，模型则呈现稀疏而强的方向坍缩；鲁棒训练可降低整体不对称性，但无法恢复人类特有的‘宽-强’相似度梯度分布；不同不对称组织方式即使性能相当，也会将RD前沿推向相反方向。

Conclusion: 方向性混淆及其对应的RD几何特征（β, κ, AUC）是刻画模型与人类归纳偏置差异的紧凑、可解释指标，尤其适用于分布偏移场景下的泛化行为分析。
...
TL;DR: Vista4D是一种基于4D点云的视频重拍摄框架，能从新相机轨迹和视角重合成输入视频的动态场景，提升4D一致性、相机控制精度与视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频重拍摄方法在真实动态视频中存在深度估计伪影、外观保持差、难以精确控制新相机路径等问题。

Method: 构建4D点云表示，结合静态像素分割与4D重建以显式保留已见内容并提供丰富相机信号，并利用重建的多视角动态数据进行训练以增强对点云伪影的鲁棒性。

Result: 在多种视频与相机路径下，相比SOTA方法，在4D一致性、相机控制和视觉质量上均有提升；并可泛化至动态场景扩展与4D场景重组等真实应用。

Conclusion: Vista4D通过4D点云建模与多视角动态训练，实现了更鲁棒、灵活且可控的视频重拍摄能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态模型难以有效融合异构模态信息、建模共享多模态知识流形的问题，提升跨模态推理保真度。

Method: 提出Omni模型，原生联合训练多种模态数据；引入Context Unrolling机制，使模型能显式地在多个模态表征间进行推理并聚合互补信息。

Result: 在多模态生成与理解基准上取得优异性能，并展现出强大的多模态推理能力，包括上下文内生成文本、图像、视频和3D几何。

Conclusion: 统一多模态原生训练结合Context Unrolling可显著提升模型对多模态知识流形的建模能力与推理 fidelity，为通用多模态基础模型提供新范式。
...
TL;DR: 本文提出IMU-to-4D框架，利用可穿戴惯性传感器（如耳塞、手表、手机）数据，无需摄像头即可重建人体4D运动与粗略场景结构，通过改造大语言模型实现非视觉时空理解，在多数据集上优于现有级联方法。


<details>
  <summary>Details</summary>
Motivation: 解决摄像头在隐私、安全、能耗和可扩展性方面的固有挑战，探索仅依赖可穿戴传感器的4D感知新范式。

Method: 提出IMU-to-4D框架，将大语言模型适配用于处理来自耳塞、手表或智能手机等少量惯性测量单元（IMU）的数据，以联合预测4D人体运动和粗粒度3D场景布局。

Result: 在多样化的人-场景数据集上实验表明，IMU-to-4D比当前最优级联流水线生成更连贯、时间更稳定的结果。

Conclusion: 仅靠日常可穿戴运动传感器即可支撑丰富的4D人-场景理解，验证了非视觉4D感知的可行性与潜力。
...
TL;DR: 本文将时间视为可学习的视觉概念，提出了一种自监督方法来检测视频中的速度变化并估计播放速度；利用该方法构建了目前最大的慢动作视频数据集，并进一步开发了支持速度条件生成和时序超分辨率的模型，实现了对视频时间流的可控操作。


<details>
  <summary>Details</summary>
Motivation: 视频在计算机视觉中占据核心地位，但对时间流逝的感知与控制却长期被忽视。本文旨在将时间作为可学习、可操控的视觉维度进行建模。

Method: 利用视频固有的多模态线索和时间结构，通过自监督学习检测速度变化并估计播放速度；基于所学模型从野外噪声数据中筛选并构建大规模慢动作视频数据集；在此基础上训练速度条件视频生成与时间超分辨率模型。

Result: 构建了迄今最大规模的慢动作视频数据集；实现了精准的速度变化检测与播放速度估计；成功开发出支持指定速度生成及低帧率视频到高帧率精细时序重建的模型。

Conclusion: 时间是一种可学习、可操控的感知维度；本工作为时序可控视频生成、时间伪造检测以及具备时序理解能力的世界模型奠定了基础。
