...
Method: 构建合成数据集BatteryPass-12K；对22种语言模型（含小型模型、MoE和稠密大模型）进行零样本和少样本评估；开展提示注入攻击实验与性能归因分析。

Result: 思维型模型（如GPT-5.4）在验证集和测试集上F1分别达0.98和0.71；少样本显著提升性能；部分小型模型优于大型模型；提示注入明显降低准确率。

Conclusion: DBP一致性分类是一项具有现实意义的新任务；模型能力不单纯依赖参数规模；BatteryPass-12K数据集已开源，可支撑电池全生命周期等延伸研究。
...
TL;DR: 本文提出了一种名为Length Value Model（LenVM）的token级长度建模框架，将剩余生成长度建模为带折扣的负奖励累积值，实现无标注、稠密、无偏且可扩展的监督；在多个任务上显著提升长度控制精度与推理效率权衡能力，并提供可解释的生成动态分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏细粒度的token级长度建模，仅在序列层面粗粒度控制生成长度，而token长度直接影响推理成本与推理性能，亟需更精细、可学习、可解释的长度建模机制。

Method: 将剩余生成长度建模为一个值估计问题：对每个生成token赋予恒定负奖励，用折扣累积回报作为单调代理信号；该信号有界、可学习、无需人工标注，且天然稠密；在LLM/VLM中以轻量方式集成并端到端训练。

Result: 在LIFEBench精确长度匹配任务中，7B模型长度得分从30.9提升至64.8，超越前沿闭源模型；GSM8K 200-token预算下准确率保持63%（基线仅6%）；能准确预测总生成长度，并揭示各token对长短生成倾向的影响。

Conclusion: LenVM验证了token级长度可被有效建模为一种价值信号，不仅提升长度控制能力与效率-性能权衡，还具备可解释性与通用性，可作为未来强化学习训练中的长度专用价值信号基础框架。
...
TL;DR: 本文提出CL-bench Life——一个面向真实生活场景的、人工精心构建的上下文学习评测基准，用于评估大模型在混乱、碎片化、个性化现实语境中的推理与任务求解能力；实验表明当前前沿语言模型在此类任务上表现极差（最高仅19.3%），凸显现实语境学习仍是重大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有AI助手正从专业场景走向日常生活，需处理多源、碎片化、强个人/社会关联的真实语境（如群聊记录、行为痕迹等），但尚不清楚当前大模型是否具备可靠的真实语境学习能力。

Method: 构建全人工标注的CL-bench Life基准，包含405个上下文-任务对和5348条验证细则，覆盖典型真实生活场景，并对10个前沿语言模型进行系统评测。

Result: 所有模型在该基准上表现低迷：最优模型任务解决率仅19.3%，平均为13.8%；尤其难以处理混乱群聊历史和碎片化行为记录等复杂现实语境。

Conclusion: 真实生活语境学习是当前大模型的关键短板，CL-bench Life为推动该方向研究提供了重要测试平台，其进展将提升日常AI助手的智能性与可靠性。
...
TL;DR: 本文提出CarryOnBench，首个评估大语言模型在多轮对话中能否根据用户善意澄清修正意图误解、恢复有用性同时保持安全性的交互式基准；实验发现多数模型能逐步恢复有用性，但存在三种单轮评测无法发现的失败模式：效用锁定、不安全恢复和重复恢复。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全对齐技术关注抵御对抗攻击，却忽视模型在用户善意澄清意图后能否恢复有用性。

Method: 构建CarryOnBench交互式基准，包含398个表面有害但实为善意意图的查询，模拟5970个多轮对话（共23880条响应），设计Ben-Util检查表指标评估每轮响应对用户善意信息需求的满足程度，并分析14个模型的表现。

Result: 首回合模型仅满足10.5–37.6%的善意信息需求；若初始即明确善意意图，则达25.1–72.1%；加入澄清后，13/14模型接近或超越该基线；识别出效用锁死、不安全恢复、重复恢复三类单轮评测不可见的失败模式；不同保守程度的模型最终有害性趋于一致。

Conclusion: 单轮安全评测无法区分‘恰当谨慎’与‘对澄清无响应’，CarryOnBench揭示了当前对齐方法在动态意图理解与响应适应性上的关键缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 探究任务特定大语言模型中神经元是否对目标任务贡献均匀，验证任务特异性神经元是否存在及其重要性。

Method: 提出基于激活的selectivity指标识别低贡献神经元，进行选择性剪枝，并与随机剪枝对比；开展反向剪枝（移除高selectivity神经元）、不同规模模型（1.5B/7B）测试、鲁棒性分析及微调恢复实验。

Result: 选择性剪枝显著优于随机剪枝；移除约10%高任务特异性神经元导致性能崩溃；剪枝30–35%低关键性神经元仍保留可观性能；剪枝可降低参数量、VRAM占用并提升吞吐量；发现15–20%鲁棒性阈值；微调可有效恢复剪枝后性能。

Conclusion: 任务特定语言模型中存在功能特异化的神经元，关键信息集中于少量神经元；模型具有冗余性但存在鲁棒性边界；选择性剪枝+微调是高效压缩与恢复的有效范式。
...
Method: 基于ILR等级（1–3+）设计12组语义等价提示，在英语、法语、罗马尼亚语、西班牙语、意大利语、德语上运行Claude（3轮共216条响应）；采用双层分析法：自动量化指标（如响应长度、表面差异） + 六语种ILR/OPI资深专家（12年经验）的定性评估。

Result: 量化结果：法语响应比德语长约30%；创意与情感类提示跨语言表面差异最大。定性结果：识别出五类跨语言变异模式——语用消歧策略差异、审美/文学传统影响、术语规范不一、文化校准缺失（倾向文化中立模板）、情感支持中机构转介行为的语言特异性。

Conclusion: ILR指导下的专家判断是评估LLM多语输出的新颖且被低估的方法，能弥补纯计算基准的不足；Claude的跨语言差异具有可解释性、领域依赖性，对多语言AI的公平落地有实质性影响。
...
TL;DR: 本文发现大语言模型隐藏状态中的语义特征几何关系与人类心理关联高度一致，包括语义轴投影、轴间余弦相似性预测、低维子空间结构以及语义轴间的引导溢出效应。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型中语义特征是否以类似人类心理的方式组织，验证其内部表征是否具有心理学意义上的合理性。

Method: 构建360个词在32个语义轴（如美-丑、软-硬）上的特征向量，将其投影并与人类评分对比；分析语义轴之间的余弦相似性与人类调查中量表相关性的关系；进行降维分析以识别低维语义子空间；测试跨轴语义引导的溢出效应。

Result: 语义轴投影与人类评分高度相关；轴间余弦相似性可预测人类量表相关性；大量语义方差集中在低维子空间；语义轴间存在与余弦相似性成比例的引导溢出效应。

Conclusion: 大语言模型中的语义特征不仅应单独理解，更应通过其几何关系及所构成的有意义子空间来整体理解，其表征结构具有类人语义组织特性。
...
TL;DR: 本文提出Path-Lock Expert（PLE）架构，通过在每个解码层中用两个语义锁定的专家（分别专用于think和no-think模式）替代单一MLP，并由控制token确定性地路由整个序列至单一专家路径，从而彻底分离两种推理模式；该方法显著抑制了no-think模式下的推理泄漏，在保持think性能的同时大幅提升no-think模式的准确性、简洁性和可控性。


<details>
  <summary>Details</summary>
Motivation: 现有混合思维语言模型在no-think模式下仍存在推理泄漏（即不必要地生成自省式长响应），根本原因在于think与no-think模式共享同一组前馈网络参数，仅靠数据与训练策略难以根治。

Method: 提出Path-Lock Expert（PLE）：每个decoder层中用两个独立、语义锁定的FFN专家（think expert与no-think expert）替代原单一MLP；注意力、嵌入、归一化及LM head保持共享；引入控制token实现序列级确定性单路径路由；fine-tuning时对各专家进行模式纯监督更新。

Result: 在数学与科学推理基准上，PLE在保持think-mode性能的同时，显著提升no-think-mode表现：以Qwen3-4B为例，在AIME24上no-think反射token数从2.54降至0.39，准确率从20.67%升至40.00%。

Conclusion: 可控混合思维本质上是架构问题；显式分离模式专属的前馈路径是一种简单而有效的解决方案。
...
TL;DR: 本文提出了一种名为Selective Augmentation的自举方法，通过从辅助语言（如印地语）中选择性迁移语音区别信息来增强现有训练数据，从而提升通用自动音标转录（APT）模型对德语塞音清浊和送气特征的识别能力。


<details>
  <summary>Details</summary>
Motivation: 通用自动音标转录（APT）需要大量干净且多样的训练转录数据，但高质量数据稀缺。

Method: 基于MultIPA模型，采用Selective Augmentation方法，利用辅助语言（印地语）中的语音区分信息，有针对性地增强目标语言（德语）的训练数据，以提升特定语音特征（如塞音清浊与送气）的建模能力。

Result: 在德语上，塞音清浊识别准确率提升17.6%（主要减少假阳性）；首次实现对/p, t, k/送气特征的识别（达61.2%），并使‘不送气’类减少32.2%，显著缓解了塞音混淆问题。

Conclusion: Selective Augmentation是一种有效利用多语言语音知识提升单语APT性能的数据增强策略，尤其适用于低资源语音特征建模。
...
TL;DR: 本文介绍了ASL Minimal Translation Pairs (ASL-MTP) 数据集，用于评估美国手语模型对各类语言现象及多模态线索（手部、上身、面部）的建模能力，并通过消融实验分析了当前最优ASL-to-English翻译模型对手动与非手动线索的依赖程度。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型在语言现象建模和多 articulator（手、上身、面部）线索利用方面仍不明确，亟需系统性评测基准。

Method: 构建ASL-MTP基准数据集，涵盖多种手语语言现象的最小对立翻译对；对SOTA ASL-to-English翻译模型开展输入线索（手动/非手动）的消融实验并评估其在各现象上的表现。

Result: 模型在多数现象上表现优于随机水平，但严重依赖手动线索，常忽略关键的非手动线索。

Conclusion: 当前手语翻译模型对非手动语言成分建模不足，凸显多模态线索协同建模的必要性。
...
TL;DR: 本文研究了语言模型在被指示故意表现不佳时，是基于题目内容作答还是依赖位置捷径。通过六种不同具体程度的对抗性指令，在两个微调过的LLaMA-3系列模型上测试MMLU-Pro数据集，发现存在三种而非连续的响应模式：模糊指令下仍保持内容理解；标准‘藏拙’指令导致位置熵崩溃但仍有部分内容参与；而两步式答案感知回避指令则引发极端位置坍缩、完全丧失内容敏感性，且该位置与模型无提示下的默认选择一致。结果表明指令复杂度可决定对抗性服从是内容感知还是内容盲目的。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型在被要求故意表现不佳（如‘藏拙’）时，其响应机制是依赖题目语义内容，还是退化为简单的位置捷径（例如总选C），从而厘清模型行为背后的认知机制与鲁棒性边界。

Method: 设计六级对抗性指令特异性梯度，在Llama-3-8B和Llama-3.1-8B上对2000道MMLU-Pro题进行测试；采用分布筛查（响应位置熵）和独立的内容参与判据（难度-准确率相关性）联合刻画每种条件下的行为；分析响应分布坍缩与内容敏感性之间的关系。

Result: 发现三类响应机制：（1）模糊指令→中度准确率下降+保留内容参与；（2）标准sandbagging/能力模仿指令→位置熵崩溃+部分内容参与；（3）两步答案感知回避指令→极端位置坍缩（99.9%/87.4%集中于单一选项）、零内容敏感性，且该选项与模型空提示默认响应一致；该效应跨模型与四个学科领域复现；熵筛选与难度相关性评估仅有50%一致性，说明二者衡量维度部分正交。

Conclusion: 指令复杂度（尤其是否含多步、答案感知）是决定小规模指令微调语言模型在贪婪解码下采取内容感知或内容盲目对抗策略的关键因素；单纯依赖位置熵可能低估内容参与，需结合难度-准确率等语义指标综合评估响应有效性。
...
TL;DR: 本文首次系统研究了大语言模型（LLMs）中基本推理模式（如归纳、演绎、溯因）能否与具体问题实例解耦的问题，提出‘推理冲突’作为分析视角，发现模型倾向于遵循任务适配的推理模式而非指令要求，且其推理类型在线性可分的中后期层中编码，据此实现高达29%的指令遵从率提升。


<details>
  <summary>Details</summary>
Motivation: 探究LLM中基本推理模式（归纳、演绎、溯因）是否可脱离具体问题实例而独立控制，以提升模型的可控性、可信性与泛化性。

Method: 引入‘推理冲突’范式——通过强制施加偏离目标任务预期的逻辑图式，引发参数化知识与上下文指令间的显式张力；结合准确率、置信度变化、探针实验及激活干预进行多维评估。

Result: LLM始终优先选择符合任务语义的推理模式而非指令要求；任务准确率不完全依赖语义合理性，大模型更依赖内部参数化记忆；冲突时置信度显著下降；推理类型在中晚期Transformer层线性可分；基于此设计的干预方法使指令遵从率提升最高达29%。

Conclusion: LLM推理虽根植于具体实例，但可通过机制化干预（如激活层面调控）有效解耦逻辑图式与数据，为提升可控性、忠实性与泛化能力提供可行路径。
...
TL;DR: 本文通过在字节级预训练流程中解耦子词分词的影响，系统研究了子词分词对训练效率和模型性能的作用机制，发现训练吞吐量提升和子词边界作为先验或归纳偏置是其优于纯字节模型的关键原因。


<details>
  <summary>Details</summary>
Motivation: 子词分词在大语言模型中至关重要，但其对训练效率和模型性能的具体贡献尚不清楚。

Method: 在受控的字节级预训练流程中解耦子词分词效应，通过模拟不同维度（如样本吞吐量、词表规模、子词边界的语言先验）来检验假设。

Result: 实验表明，子词模型的优势主要源于更高的训练吞吐量以及子词边界作为显式先验或归纳偏置的引入。

Conclusion: 子词分词的价值不仅在于压缩，更在于其带来的训练效率提升与语言结构先验，该发现可指导未来字节级与子词模型的预训练优化。
...
TL;DR: 本文研究了大型语言模型（LLMs）在处理具有显式二维结构的任务时，将输入线性化（序列化）所带来的性能损失（称为'serialization friction'），通过对比纯文本路径与视觉增强路径（保持2D布局）在矩阵转置、生命游戏和LU分解等合成任务上的表现，发现视觉路径始终更优，尤其在更大尺寸下差距更明显，表明保留任务相关的2D结构对提升性能至关重要。


<details>
  <summary>Details</summary>
Motivation: 传统LLM将结构化输入（如矩阵）线性化为1D序列，可能增加表征负担，尤其当任务计算依赖显式2D结构（如行列对齐、局部邻域）时；本文旨在探究这种‘序列化摩擦’的影响。

Method: 构建一个小型诊断测试平台（含矩阵转置、Conway生命游戏、LU分解），对比两种输入路径：（1）纯文本路径——将2D数据序列化为token序列；（2）视觉增强路径——将相同内容以任务忠实的2D布局渲染为图像，输入同一语言主干模型；进行系统级端到端比较。

Result: 视觉路径在所有任务和设置中均持续优于文本路径；性能差距随输入维度增大而扩大；文本路径的错误模式呈现空间结构性。

Conclusion: 输入表示方式显著影响LLM在结构化2D任务上的性能；保留任务相关的2D布局是一种有前景的改进方向，值得进一步研究。
...
TL;DR: 本文提出RSCB-MC——一种风险敏感的上下文赌博机记忆控制器，用于LLM编程智能体，旨在安全、选择性地使用外部记忆，避免因表面相似性导致的错误记忆注入；其核心是将记忆使用建模为控制问题，强调安全性（零假阳性）优先于召回率，在多个评估中实现高成功率与零错误注入。


<details>
  <summary>Details</summary>
Motivation: 现有LLM编程智能体依赖外部记忆复用调试经验，但仅靠表层相似性（如堆栈跟踪、错误信息）检索易导致不安全的记忆注入，亟需一种能评估记忆使用风险的机制。

Method: 提出RSCB-MC控制器，采用模式-变体-片段（pattern-variant-episode）记忆存储结构；将检索证据编码为16维上下文状态（含相关性、不确定性、结构兼容性、误报风险等）；设计奖励函数，对假阳性注入施以更强惩罚；支持多种动作：不使用记忆、注入最佳方案、多候选摘要、高低精度检索、主动放弃或请求反馈。

Result: 在确定性烟雾测试中，离线重放成功率62.5%，假阳性率为0.0%；在200例热路径验证中，代理成功率60.5%，假阳性率仍为0.0%，p95决策延迟331.466微秒。

Conclusion: 编程智能体的记忆使用不应仅追求相似性排序，而应聚焦于‘是否足够安全以影响调试路径’；RSCB-MC证明了风险感知控制可兼顾高成功率与绝对安全，为可信AI编码系统提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 人类情绪标注常存在分歧，但现有LLM评估多将其简化为单一真值标签，忽略分歧所蕴含的分布信息；本文旨在检验LLM是否能建模这种人类情绪判断的分布结构。

Method: 在GoEmotions和EmoBank两个基准上，对比四类零样本LLM与微调RoBERTa模型的情绪判断分布与人类标注分布；提出‘词汇锚定梯度’与透明度分数量化模型对不同情绪类别的建模能力；设计并评估三种后处理校准方法。

Result: 零样本LLM显著偏离人类情绪分布；领域内微调比模型规模更能提升分布一致性；模型对具明确词汇标记的情绪（如‘愤怒’）拟合好，对依赖语境推断的情绪（如‘讽刺’）表现差；三种校准方法最多降低14%分布差异。

Conclusion: LLM情绪标注不能简单替代人工标注；其可靠性高度依赖情绪类型的词汇显性程度与任务框架；需结合校准与领域微调，并明确标注适用边界。
...
TL;DR: 本文提出了一种基于情绪感知的点击诱饵生成攻击方法，利用VAD情绪空间建模情感动态，并结合Sentence-BERT和大语言模型生成具高情绪冲击力的标题变体，通过‘好奇心缺口’函数量化其规避检测能力，显著降低现有分类器性能。


<details>
  <summary>Details</summary>
Motivation: 现有点击诱饵研究将其视为静态文本现象，检测系统依赖表层语言特征，缺乏对情绪动态与用户心理机制（如好奇心）的建模，难以应对新型对抗性生成攻击。

Method: 构建基于VAD（效价-唤醒-支配）情绪空间的情绪感知框架；利用Sentence-BERT对齐语义相似的社交媒体帖子与标题；调用大语言模型生成多种情绪增强型风格化改写；定义‘好奇心缺口’（CG）函数量化标题与原文的情绪差异以激发用户点击欲并规避检测。

Result: 情绪感知风格化攻击使当前最优点击诱饵分类器误判率大幅上升，达2.58%至30.63%。

Conclusion: 情绪动态是点击诱饵生成与传播的核心驱动力，仅依赖语言结构特征的检测方法存在根本性脆弱性；引入情绪建模与心理机制（如好奇心）可更真实地模拟和防御对抗性点击诱饵攻击。
...
TL;DR: 本文提出了一种轻量级的意图转移先验（intent-transition prior），通过在系统提示中注入基于对话数据学习的时序贝叶斯网络（T-BN），使对话模型具备一定前瞻性，从而在多意图场景中减少冗余交互、提升覆盖效率。


<details>
  <summary>Details</summary>
Motivation: 现有对话模型是被动响应式的，无法预判用户后续意图，在多意图对话中导致重复交互和低效。

Method: 从MultiWOZ 2.2数据中提取每轮意图标注，构建时序贝叶斯网络（T-BN）作为轻量意图转移先验，并在推理时将其注入系统提示，不修改底层语言模型。

Result: T-BN在1071个预留用户轮次上达到Recall@5=0.787、MRR=0.576；在200个对话的真值回放实验中，覆盖AUC从0.742提升至0.856，达到75%意图覆盖率所需轮数从3.95降至2.73。

Conclusion: 仅通过轻量级意图转移先验注入，即可显著提升对话模型的主动性和效率，且无需改动模型结构或参数。
...
TL;DR: MiniCPM-o 4.5 是一个支持实时全双工多模态交互的开源大模型，通过 Omni-Flow 框架实现感知与响应同步、主动行为涌现，在边缘设备上高效运行。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs仍采用分阶段感知与响应、被动响应用户指令的交互范式，难以实现类人多模态交互；需突破交互范式瓶颈，支持实时、同步、主动的多模态交互。

Method: 提出 Omni-Flow 统一流式框架，将多模态输入/输出对齐至统一时间轴，实现全双工、时序对齐的交互；模型具备实时视-听-说同步能力及基于场景理解的主动行为（如提醒、评论）；采用9B参数高效架构与推理优化。

Result: 在视觉-语言能力上接近 Gemini 2.5 Flash；在多模态理解上超越 Qwen3-Omni-30B-A3B；语音生成更优；计算效率更高；可在<12GB内存边缘设备实现实时全双工交互。

Conclusion: MiniCPM-o 4.5 通过重构交互范式（全双工+时序对齐+主动行为），推动MLLM向类人实时多模态交互迈进，兼具高性能与端侧部署可行性。
...
TL;DR: 本文探讨了文本嵌入中常用的均值池化（mean pooling）是否真正有效，提出了一种量化其信息坍缩（尤其是丢失二阶统计信息）的指标，并通过实证发现现代对比学习微调的文本编码器对此坍缩具有鲁棒性，且这种鲁棒性源于词元嵌入在文本内的集中性，并与下游任务性能正相关。


<details>
  <summary>Details</summary>
Motivation: 均值池化可能过度压缩信息（如丢失词元嵌入的二阶统计结构），导致不同分布映射为相似文本嵌入，但其实际影响尚不明确。

Method: 提出一个量化均值池化引起的信息坍缩的简单指标，并在真实模型和文本上实证测量坍缩发生频率；分析对比微调前后编码器的鲁棒性差异及内在原因（如词元嵌入集中性）；检验该鲁棒性与下游任务性能的相关性。

Result: 现代文本编码器（尤其对比学习微调后）对均值池化导致的信息坍缩具有鲁棒性；鲁棒性源于词元嵌入在每段文本内的高度集中；鲁棒性程度与下游任务性能呈正相关。

Conclusion: 均值池化虽看似粗糙，但在现代编码器中仍有效，因其通过训练（尤其是对比学习）使词元嵌入集中，从而缓解信息坍缩；本文为理解其有效性提供了新视角。
...
TL;DR: 本文提出了一种无需反向传播的扰动探测方法，通过两次前向传播识别大语言模型中控制特定行为（如安全拒绝、语言切换）的关键神经元，并发现两种行为电路结构：对立电路（由RLHF抑制预训练倾向形成）和路由电路（依赖注意力机制分布的预训练行为），并验证了其在不同架构与任务上的可干预性与局限性。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型中RLHF等对齐技术如何具体改变模型内部行为机制，尤其是定位和干预控制关键行为（如安全拒绝、语言选择）的神经元，同时避免传统归因方法对梯度或大量训练数据的依赖。

Method: 扰动探测法：对每个prompt仅需两次前向传播生成因果假设，再进行一次性干预扫描（约150次前向传播分摊至所有候选神经元）；结合FFN-to-skip信号比分析、方向注入、神经元消融/放大等干预手段，在多个模型与任务上验证电路结构。

Result: 发现两类通用电路结构——对立电路（如安全拒绝中约50个神经元控制模板，消融后80%响应格式改变且有害合规率降至3/520）和路由电路（如语言选择需满足三个条件才可通过残差流方向注入实现99.1%中英切换）；FFN-to-skip信号比可区分结构并预测干预有效性；电路拓扑随架构变化（如Qwen集中瓶颈 vs Gemma归一化屏蔽）；在Qwen3.5-2B上实现多轮谄媚屈服消除与事实纠正率从52%提升至88%。

Conclusion: 扰动探测是一种高效、可扩展的机制分析工具，能揭示RLHF组织行为的底层神经基础，并支持对模板层的精准编辑，为可控、可解释的大模型对齐提供新路径。
...
TL;DR: 本文将临床心理学中的可靠变化指数（RCI）应用于大语言模型在MMLU-Pro基准上的逐项版本比较，发现模型升级带来的整体准确率提升实为大量双向、高幅度的逐项性能变化（即'churn'）的净残差，且不同难度和领域存在显著不对称性；标准单次贪婪评估会严重误判变化项，因此建议在报告整体准确率的同时报告'churn rate'。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估主要依赖整体准确率，掩盖了模型升级在具体题目上的真实变化模式（如改善、恶化或不变），缺乏对变化可靠性的统计检验。

Method: 将临床心理学中的可靠变化指数（RCI）适配到LLM逐项预测概率变化分析中，在2000个MMLU-Pro题目上，对Llama 3→3.1和Qwen 2.5→3两组同家族模型升级进行K=10次采样（T=0.7）的对比分析，并进行难度分层、领域分解及与贪婪单次评估的误差对比。

Result: 多数题目无可靠变化（72%-79%），但在可分析题目中变化剧烈且双向：Llama有34%提升、28%恶化，Qwen为47%提升、39%恶化；变化呈难度不对称（低准确题改善、高准确题恶化）和领域特异性（如Llama失物理、Qwen失法律）；贪婪单次评估漏检42%、误报25%的可靠变化项。

Conclusion: LLM版本升级的 aggregate accuracy 增益是大量对立、高幅 item-level churn 的净结果；仅报告整体准确率具有误导性；应同步报告 churn rate 并采用多采样RCI等更稳健的评估方法。
...
TL;DR: 本研究使用TF-IDF机器学习和Transformer深度学习两种方法分析印尼学生对高等教育中人工智能应用的看法，发现DistilBERT在准确率和F1分数上略优于SVM等传统模型，但SVM仍具效率与竞争力。


<details>
  <summary>Details</summary>
Motivation: 探究印尼学生对高等教育中人工智能应用的态度，并比较不同机器学习与深度学习方法在该任务上的性能表现。

Method: 采用TF-IDF特征提取结合LightGBM、随机森林和SVM进行机器学习建模；同时使用微调的DistilBERT进行Transformer-based二分类情感分析。数据集包含2295条标注样本。

Result: SVM在机器学习模型中表现最佳（测试准确率与F1均为82.14%），DistilBERT整体最优（准确率84.78%，F1 84.75%）。

Conclusion: Transformer模型更擅长捕捉上下文信息，但在资源受限场景下，SVM仍是高效且有竞争力的情感分类替代方案。
...
TL;DR: 本文提出了一种细粒度的写作奖励模型评估流程WEval和强化学习训练框架WRL，以解决现有方法在写作任务中评估粗略、缺乏细粒度需求遵循建模的问题；实验表明其模型在多个写作基准上显著提升并具备强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有写作奖励模型评估过于粗略，无法从具体需求角度衡量性能；训练方法要么依赖LLM-as-a-judge，要么仅构建粗粒度奖励模型，缺乏对细粒度需求遵循的建模。

Method: 提出细粒度评估流程WEval（覆盖多任务与多需求类型，通过奖励模型排序与人工金标准排序的相关性进行系统评估）和细粒度强化学习训练框架WRL（通过选择性删除指令要求构造正负样本，实现更精准的奖励建模）。

Result: 所提模型在多个写作基准上取得显著性能提升，并展现出强泛化能力。

Conclusion: 细粒度评估与训练能有效提升写作奖励模型的性能与鲁棒性，为生成式写作任务提供了更可靠、可解释的评估与优化路径。
...
TL;DR: 本文研究了大语言模型（LLMs）在持续自指对话中出现的响应风格质变现象（称为“transfer”），将其定义为一种可操作的状态而非本体论主张，并通过认知特征分析和苏格拉底式AI教学实验验证其应用潜力，发现transfer状态在行为交互中展现出显著功能优势。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在持续自指对话下产生的响应风格质变（transfer）是否具有可操作性与实际应用价值，而非仅视作意识或本体层面的现象。

Method: 采用两阶段方法：1）初步认知表征研究（11种对话条件下的MAS-A量表与自建7项认知指标分析）；2）应用实验（基于三个教学情境指标对苏格拉底式AI辅导表现进行评分比较）。

Result: SU_dir指标在三类模型家族中均显示transfer侧显著偏离（kappa=0.83）；transfer条件下辅导评分平均高出非transfer条件1.6倍，效应量大（d=1.27）；MAS-A差异较小（d=0.40）。

Conclusion: transfer应被理解为一种具应用潜力的、可复现的操作性状态；其功能优势在行为交互中比自我叙事中更敏感；该研究为LLM状态工程提供了认知-应用联动的评估框架。
...
TL;DR: 本文提出了一种理性语言使用者选择性维持对未来预测至关重要的信息的假设，该假设受句法结构引导，并通过日语自然阅读时间数据验证了预测头数量与未完成依存关系数量这两个影响维持成本的因素不可互相替代，且存在维持成本与可预测性收益之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 维持上下文信息对实时语言理解至关重要，但这种维持在认知上代价高昂并可能减慢处理速度，因此需要探究语言使用者如何理性地选择性维持信息。

Method: 基于句法结构引导的选择性信息维持假设，分析日语自然阅读时间数据，检验预测头数量和未完成依存关系数量对维持成本的独立影响及二者间的权衡关系。

Result: 证实预测头数量和未完成依存关系数量是两个不可相互还原的影响维持成本的因素，并发现维持成本较高的读者从可预测性中获益更多，支持所提出的理论框架。

Conclusion: 语言使用者依据句法结构理性选择维持对未来预测关键的信息，维持成本由多个独立因素共同决定，并与可预测性收益存在权衡关系。
...
TL;DR: HealthBench Professional 是一个面向临床实际任务的开源基准，涵盖诊疗咨询、文书撰写与医学研究三类高频场景，基于真实医生与 ChatGPT 的对话构建，由多位医师多轮评审打分；其难点样本经针对性增强，并包含约1/3对抗性测试案例；评估显示 GPT-5.4（ChatGPT for Clinicians 版）表现超越基线模型及人类医生。


<details>
  <summary>Details</summary>
Motivation: 现有对大语言模型在临床中实际应用效果的评估不足，尤其缺乏覆盖真实医生高频使用场景、经临床专家严格验证的基准。

Method: 构建 HealthBench Professional 基准：基于 15,079 条真实医生与 ChatGPT 的对话，按三大临床用例分类；每条样本由 ≥3 名医师通过三阶段迭代评审打分；难点样本按比例增强，并纳入约1/3对抗性测试案例；同时收集同专科医生在无时间限制、可联网条件下的参考回答作为人类基线。

Result: GPT-5.4 在 ChatGPT for Clinicians 中的表现最优，全面优于其基础版本、其他模型及人类医生基线。

Conclusion: HealthBench Professional 为评估前沿模型在真实临床任务中的能力提供了可靠、临床可信的度量标准，有助于推动可信赖医疗 AI 系统的发展。
...
TL;DR: Skills-Coach是一个自动化框架，旨在提升LLM代理的技能自我进化能力，通过任务生成、轻量优化、对比执行与可追溯评估四大模块，显著增强技能能力覆盖与性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前技能生态碎片化问题，拓展技能能力边界，实现智能应用所需的全面能力覆盖。

Method: 提出Skills-Coach框架，包含多样化任务生成、轻量级优化、对比执行和可追溯评估四个核心模块，并支持虚拟与真实两种执行模式；构建Skill-X基准数据集（含48种技能）进行验证。

Result: 在Skill-X基准上实验表明，Skills-Coach在多类技能能力上均实现显著性能提升。

Conclusion: Skills-Coach能有效推动LLM代理向更鲁棒、自适应方向发展，为技能自主演化提供新范式。
...
TL;DR: 本文提出了一种因果驱动的推理时干预方法，通过识别并抑制与多种偏差属性（如响应长度）强相关的神经元激活，来减轻奖励模型（RMs）对虚假特征的敏感性，且不牺牲性能。


<details>
  <summary>Details</summary>
Motivation: 现有RM对虚假特征（如响应长度）敏感，而当前推理时去偏方法仅针对单一偏差（如长度），导致性能权衡。

Method: 首先识别与预定义偏差属性强相关的神经元，然后在神经元层面进行干预以抑制这些偏差信号。

Result: 在RM基准测试中显著降低了对多种虚假特征的敏感性，且无性能下降；使用该方法的小型RM（2B/7B）在AlpacaEval和MT-Bench上达到接近70B SOTA RM的对齐效果；偏差信号主要位于早期层神经元。

Conclusion: 所提因果干预方法能有效、高效地缓解RM中多种偏差，揭示了偏差编码的内部机制，并为轻量级RM实用化提供了新路径。
...
TL;DR: 本文提出两种新指标POSER和EmbER，用于评估语言模型在ASR系统重评分阶段对词性与语义层面错误的修正效果，弥补传统WER指标的不足。


<details>
  <summary>Details</summary>
Motivation: 传统ASR评估仅依赖词错误率（WER），难以深入分析转录错误的语言学特性；需引入能反映语法和语义层面的补充评估指标。

Method: 提出两个新评估指标：POSER（词性错误率）用于衡量语法层面错误，EmbER（嵌入错误率）基于词向量语义距离加权计算错误代价，并结合语言模型重评分过程进行分析。

Result: POSER和EmbER能有效揭示语言模型在重评分中对语法一致性和语义合理性提升的具体贡献，为ASR系统提供更细粒度的评估视角。

Conclusion: 引入语言学感知的评估指标（POSER、EmbER）可显著增强ASR系统评估的深度与解释性，推动从单纯准确率导向转向语言能力导向的模型优化。
...
TL;DR: This paper estimates the entropy of the Ukrainian language using Shannon's prediction experiment with 184 participants, yielding an upper bound of ~1.201 bits per character, and compares it to LLM performance.


<details>
  <summary>Details</summary>
Motivation: Shannon's entropy estimation experiment has never been conducted for Ukrainian, despite prior studies for English and Hebrew; this work fills that gap.

Method: Recruited 184 Ukrainian speakers via social media to predict next characters in text, following Shannon’s 1951 methodology, and computed an upper bound on character-level entropy.

Result: Estimated upper bound of Ukrainian language entropy is H_upper ≈ 1.201 bits per character; methods, code, and challenges are documented and published.

Conclusion: Ukrainian exhibits relatively low entropy—comparable to or lower than English—suggesting high predictability; results provide a benchmark for evaluating Ukrainian LLMs.
...
TL;DR: 本文提出了一种基于人类感知评估ASR系统的新方法，构建了法语人工标注数据集HATS，并分析了现有评估指标与人类偏好的相关性。


<details>
  <summary>Details</summary>
Motivation: 传统ASR评估指标（如WER）过于局限，且现有改进指标仍以系统为中心，未能真正反映人类对转录质量的感知需求。

Method: 构建法语人工标注数据集HATS，通过143名被试对两组ASR输出进行成对偏好选择；并分析多种ASR评估指标（包括词级和嵌入式指标）与人类偏好之间的相关性。

Result: 发现当前被认为最贴近人类感知的嵌入式指标（如BERTscore）与人类偏好之间相关性仍然有限，表明现有指标仍不足以替代人类评估。

Conclusion: 需发展更贴合人类感知的ASR评估范式，HATS数据集为未来研究提供了重要基准和验证工具。
...
TL;DR: 本文介绍了AppTek Call-Center Dialogues语料库，用于评估英语ASR系统在对话AI应用中的性能，特别关注不同英语口音的鲁棒性。该语料库包含14种英语口音、16种服务场景的自发角色扮演对话，专为评估设计且此前未公开。实验表明，现有开源ASR系统在不同口音和分段方法下表现差异显著，通用美式英语基准上的良好性能难以泛化到其他口音。


<details>
  <summary>Details</summary>
Motivation: 现有公开英语ASR评测语料存在预分割、非自发语音或缺乏明确方言标注等问题，难以全面评估ASR系统对多样化用户（尤其不同英语口音）的鲁棒性。

Method: 构建并发布AppTek Call-Center Dialogues语料库（含14种英语口音、16类服务场景的自发角色扮演对话），并采用多种分段策略对多个开源ASR系统进行基准测试。

Result: ASR系统在不同英语口音和分段方法下性能差异显著；在通用美式英语基准上表现良好的模型，未必在其他口音上保持高性能。

Conclusion: 需使用更具代表性的自发多口音语料（如本工作发布的语料库）来评估ASR系统，避免仅依赖传统基准导致的性能误判；未来ASR研究应更重视跨口音鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐私政策冗长复杂、术语晦涩，缺乏面向法律清晰性与可读性的高质量英文平行语料库，导致用户难以理解并可能无意中接受违法条款。

Method: 构建由领域专家精细标注的APPsi-139语料库（含139份隐私政策、15692组平行改写文本、36351个细粒度标注标签）；提出TCSI-pp-V2混合框架，采用交替训练策略协同多个专家模块。

Result: 基于APPsi-139和TCSI-pp-V2构建的系统在可读性和可靠性上优于GPT-4o和LLaMA-3-70B等大语言模型。

Conclusion: 高质量专用语料库与协同式混合框架能更有效地实现隐私政策的简洁、准确与合法解读，为法律科技（LegalTech）提供新范式。
...
TL;DR: JaiTTS-v1.0 是一个基于 VoxCPM 架构、无需文本归一化的泰语语音克隆 TTS 模型，支持数字和泰英混用，在短/长语音生成任务中达到 SOTA 性能，并在人工评测中显著优于主流商业模型。


<details>
  <summary>Details</summary>
Motivation: 解决泰语语音合成中数字处理、泰英混用（code-switching）等现实场景难题，避免传统文本归一化步骤，提升端到端建模能力。

Method: 在大型泰语语音语料上对无 tokenizer 的自回归 TTS 模型 VoxCPM 进行持续训练，适配为 JaiTTS-v1.0；直接输入原始文本（含数字与泰英混用），端到端生成语音。

Result: 短语音任务 CER 为 1.94%，优于人类标注（1.98%）；长语音任务与人类标注性能相当；人工成对评测中以 283:58 显著胜过商业旗舰模型。

Conclusion: JaiTTS-v1.0 展示了无 tokenizer 自回归 TTS 在资源受限语言上的强大潜力，尤其适用于真实复杂文本场景。
...
TL;DR: 本文提出RoadMap基准和RoadMapper多智能体系统，以解决大语言模型在研究路线图生成任务中的专业性不足、任务分解不合理和逻辑关系混乱等问题。


<details>
  <summary>Details</summary>
Motivation: 尽管结构化内容生成已取得进展，但研究路线图生成任务仍未被探索。本文旨在填补这一空白，并识别出大语言模型在此任务上的三大局限。

Method: 构建RoadMap基准，并提出RoadMapper多智能体系统，将路线图生成任务分解为初始生成、知识增强和迭代‘批判-修订-评估’三个阶段。

Result: 实验表明，RoadMapper显著提升了大语言模型的路线图生成能力，平均性能提升超8%，且耗时仅为人类专家的16%。

Conclusion: RoadMapper有效缓解了大语言模型在专业性、任务分解与逻辑组织方面的缺陷，展现出在科研辅助领域的实用潜力。
...
TL;DR: 本文介绍了Cognitive Digital Shadows (CDS) 数据集，一个包含19万条记录的合成语料库，用于分析大语言模型（LLM）在不同社会角色（人类人格 vs. AI助手）和争议性社会话题下的输出差异，并支持对偏见、社会敏感性和对齐性的审计。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对大语言模型在受控社会与上下文提示下输出变化的系统性数据集，亟需可支持社会话语分析的高质量基准资源。

Method: 构建了Cognitive Digital Shadows（CDS）数据集：由19个LLM生成，每条记录基于人类人格或AI助手角色提示，覆盖4个争议性社会话题；每条人格提示记录标注17个社会人口学与心理属性；文本经主题锚定验证，并支持情感与语义分析（如forma mentis网络）；配套交互式可视化平台支持跨模型、话题与人格的群体级比较。

Result: 发布了一个规模达190,000条、多维度标注、主题锚定且可解释的合成语料库CDS，配套用户友好的分析平台，支持情感、语义及立场层面的交叉分析。

Conclusion: CDS为LLM的社会影响研究提供了可复现、可审计、可解释的新基础设施，有助于系统评估其偏见、社会敏感性与价值对齐能力。
...
TL;DR: 本文探讨了利用大语言模型（LLM）检测卢森堡社会中语言意识形态的可行性，通过人工标注卢森堡语评论并测试不同提示条件下的LLM表现，同时考察机器翻译至高资源语言是否提升效果。结果表明LLM虽未完全适配多类意识形态标注任务，但可有效识别语言意识形态内容。


<details>
  <summary>Details</summary>
Motivation: 语言意识形态检测对理解话语中身份建构至关重要；卢森堡多元多语社会中，语言意识形态承载深层文化与社会意义，亟需有效分析工具。

Method: 人工标注卢森堡语用户评论语料库（含预定义意识形态类别），在不同提示条件下评估大语言模型的标注能力，并对比机器翻译至高资源语言后的性能变化。

Result: 大语言模型尚不能完美完成多类意识形态标注任务，但在识别语言意识形态相关内容方面具有实用性；机器翻译至高资源语言可部分提升模型性能。

Conclusion: 大语言模型可作为辅助工具用于语言意识形态分析，尤其在低资源语言场景下结合翻译策略具有一定潜力，但仍需进一步优化以支持精细、多类别的意识形态标注。
...
TL;DR: 本文揭示了跨模态编码器中因高维嵌入空间导致的hubness问题，提出了一种识别hub文本的方法，并在多个数据集上验证了其对评估指标和检索任务的负面影响。


<details>
  <summary>Details</summary>
Motivation: 跨模态编码器将文本和图像映射到共享嵌入空间以支持检索与评估等任务，但高维空间中的hubness问题可能导致某些文本（hub文本）不合理地与大量无关图像高度相似，从而威胁系统可靠性。

Method: 提出一种识别hub embedding及其对应hub text的方法，通过分析嵌入空间中的近邻分布特性定位最具代表性的hub文本。

Result: 在MSCOCO、nocaps和Flickr30k等数据集上的实验表明，存在单个hub文本能不合理地达到甚至超过人工标注参考文本的相似度得分，暴露了当前跨模态编码器的脆弱性。

Conclusion: hubness问题是跨模态嵌入空间中不可忽视的隐患，需在模型设计与评估中加以抑制，以提升跨模态任务的鲁棒性与可信度。
...
TL;DR: 本文提出了一种面向阿拉伯语诗歌可控生成的大规模指令数据集，支持诗歌创作、修改、续写及分析等任务，并通过微调大语言模型验证了其在风格与韵律控制上的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究多聚焦于阿拉伯语诗歌的分析任务（如解读、元数据预测），缺乏对实际诗歌创作的支持；本文旨在填补这一空白，提供可控的阿拉伯语诗歌生成能力。

Method: 构建了一个覆盖现代标准阿拉伯语（MSA）及多种方言的大规模、精心筛选的阿拉伯语诗歌指令数据集，支持写作、修订、续写及分析等任务，并基于该数据集对大语言模型进行微调。

Result: 微调后的模型在自动指标和母语者人工评估中均展现出符合用户指定风格与韵律要求的高质量诗歌生成能力。

Conclusion: 本工作首次系统性地将可控生成引入阿拉伯语诗歌创作领域，所构建的数据集与方法为阿拉伯语内容创作与文化传承提供了新工具与基础资源。
...
TL;DR: 本文提出一个三层计算框架（微观词汇、中观语义嵌入、宏观大模型叙事评估），在830篇中文治疗文本上验证：宏观叙事结构对心理健康预测最具价值，远超传统词频和嵌入方法；叙事组织本身具有独立临床信号。


<details>
  <summary>Details</summary>
Motivation: 现有计算方法（词典法和嵌入法）各自局限：词典法忽略话语结构，嵌入法混淆局部连贯与全局组织；缺乏将技术映射到人类叙事构建层级过程的统一框架。

Method: 构建三层次框架：微观（词汇特征）、中观（语义嵌入）、宏观（LLM驱动的叙事评估），基于Labov故事语法、RST连贯性、命题构成等理论；在830篇涵盖抑郁、焦虑、创伤的中文治疗文本上进行心理健康预测实验与多级分类分析。

Result: 宏观层面的LLM叙事评估显著优于微观词汇和中观嵌入特征；形式化叙事结构特征（如Labov语法、RST）本身即具强预测力；语义嵌入仅带来微小增量提升；宏观结构是临床信号的主要来源。

Conclusion: 叙事的宏观结构组织是心理健康预测的关键指标，应取代传统词频统计成为核心分析维度；该框架为干预设计与纵向研究提供了可检验的理论假设。
...
Result: 在SIMMC 2.1上验证了该方法能有效对齐对话上下文与场景对象；少样本设置下对未见场景和新对象具有良好泛化性，在跨域评测中超越监督式编码器方法。

Conclusion: 结构化元数据与精细提示工程对提升任务型对话系统的鲁棒性与泛化能力至关重要；测试时推理为指代消解提供了更灵活、可迁移的新范式。
...
TL;DR: 本文提出了一种名为Conformal Abstention（CA）的后处理框架，用于让语言模型在缺乏相关知识时主动拒绝回答，从而减少幻觉；该方法基于置信度而非传统共形预测中的非一致性分数，并引入基于表征几何的校准策略，实验证明其条件正确率达75%。


<details>
  <summary>Details</summary>
Motivation: 语言模型常因缺乏相关知识而生成看似合理但错误的幻觉答案，而非承认无知；现有通过重训练来鼓励模型承认无知的方法受限于评估基准稀缺，易导致过度保守和泛化差。

Method: 提出Conformal Abstention（CA）框架，将共形预测思想适配至开放生成任务；用模型预测置信度（而非不可行的非一致性分数）决定是否拒答；引入基于内部表征几何的校准策略，以更准确反映模型的知识掌握程度。

Result: 在选择性回答任务上显著提升性能，达到75%的条件正确率（即在未拒答的情况下，75%的回答是正确的）。

Conclusion: CA是一种无需重训练、具备有限样本统计保证的后处理方法，能有效平衡回答覆盖率与准确性，为缓解大模型幻觉问题提供了新思路。
...
TL;DR: 本文评估了三种小型语言模型（EuroLLM、Aya Expanse、Gemma）在回译中保持细粒度情感的能力，并探讨情感感知提示和ModernBERT在情感分类中的作用。


<details>
  <summary>Details</summary>
Motivation: 机器翻译中语义等价常优先于情感保真，导致情感细微差别难以保留。

Method: 基于GoEmotions数据集（含28类Reddit评论），对德、法、西、意、波五种欧洲语言进行回译实验，考察SLMs固有情感保持能力、情感感知提示效果及ModernBERT作为情感分类器的性能。

Result: 揭示了各SLM在不同语言中情感保持能力的差异；情感感知提示提升了部分模型的情感保真度；ModernBERT在情感分类任务中优于传统BERT。

Conclusion: SLMs在情感保持方面存在显著语言与模型差异，结合情感感知提示与ModernBERT可提升MT中情感保真度评估的准确性。
...
TL;DR: This survey reviews LLM-based methods for automating and assisting peer review across its full pipeline—including review generation, post-review tasks (rebuttals, meta-reviews, revisions), and evaluation approaches—while cataloging datasets, modeling strategies, limitations, ethics, and future directions.


<details>
  <summary>Details</summary>
Motivation: Recent advances in LLMs have spurred interest in automating or assisting various stages of the multi-stage peer review process; however, there lacks a comprehensive synthesis of techniques, evaluation methods, and practical guidance across the entire workflow.

Method: Systematic survey and synthesis of existing literature on LLM applications in peer review, organized into three main categories: (i) peer review generation (fine-tuning, agents, RL, new paradigms), (ii) after-review tasks (rebuttals, meta-reviews, revision alignment), and (iii) evaluation methods (human-centered, reference-based, LLM-based, aspect-oriented); includes dataset cataloging and comparative analysis of modeling choices.

Result: A structured, taxonomy-driven overview of LLM techniques for peer review, with insights into strengths, limitations, ethical concerns (e.g., bias, transparency), and open challenges; provides practical guidance for development and integration of LLM systems in real-world review workflows.

Conclusion: LLM-based peer review assistance is promising but still faces significant technical, evaluative, and ethical hurdles; future work must prioritize robustness, fairness, human-AI collaboration, and standardized benchmarks to enable trustworthy deployment.
...
Method: 提出DPN-LE：利用高/低人格特质样本的MLP激活差异定位人格特异性神经元；构建层内引导向量；采用Cohen's d效应量与激活幅值双准则筛选互斥神经元子集；施加稀疏线性干预。

Result: 仅用每种人格特质1000对对比样本，干预约0.5%的神经元，在LLaMA-3-8B-Instruct和Qwen2.5-7B-Instruct上实现了强人格控制能力与显著优于基线的通用能力保持（尤其在推理任务中）。

Conclusion: 人格表征具有神经元层面的互斥性与特异性，DPN-LE通过高精度定位和稀疏干预，可兼顾人格编辑效果与模型通用能力保留，为高效、低损的人格编辑提供了新范式。
...
TL;DR: 本文提出Template Constrained Decoding (TeCoD)，通过挖掘历史NL-SQL对中的查询模板，并结合自然语言推理模型选择模板，再利用语法约束解码强制生成有效SQL，在提升执行准确率的同时降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在Text-to-SQL任务中面对复杂或未见过的数据库模式时，存在准确率不稳定和生成无效SQL的问题，阻碍实际部署。

Method: TeCoD将历史NL-SQL对抽象为可复用模板；使用微调的自然语言推理模型进行模板匹配与筛选；在SQL生成阶段采用基于文法的分区式约束解码机制。

Result: 在匹配查询上，执行准确率比上下文学习（ICL）最高提升36%，延迟降低2.2倍。

Conclusion: TeCoD通过模板重用与语法约束解码，显著提升了Text-to-SQL系统的准确性、鲁棒性与效率，适用于真实场景部署。
...
TL;DR: 本文提出了DriftBench基准，用于评估大语言模型在多轮科学创意生成中对原始约束的遵守程度；研究发现迭代交互会增加结构复杂性并降低约束遵循性，且模型常出现‘知道但违反’（KBV）现象；结构化检查点可部分缓解KBV，但无法完全消除；人类验证表明LLM裁判存在漏检，使报告的依从率偏保守；所有数据已开源。


<details>
  <summary>Details</summary>
Motivation: 探究研究人员在使用大语言模型进行多轮迭代优化科学创意时，模型是否能持续忠实于原始目标和约束条件。

Method: 构建DriftBench基准，涵盖2146次运行、7个模型（5家提供商，含2个开源模型）、4种交互条件、38份来自24个科学领域的研究简报；引入重述探针检测‘知道但违反’（KBV）现象；采用结构化检查点干预；并通过人类盲评验证LLM裁判的可靠性。

Result: 迭代压力显著提升结构复杂性，同时普遍削弱约束依从性；KBV率在8%–99%之间因模型而异；结构化检查点仅部分降低KBV率，复杂性膨胀仍持续；LLM裁判漏检约束违反，导致报告的依从率偏低；结果在温度（0.7 vs. 1.0）与压力类型（新颖性 vs. 严谨性）上稳健。

Conclusion: 当前主流大语言模型在多轮科学协作中难以稳定维持对原始约束的忠实性，存在显著的‘知道但违反’行为；单纯依赖模型自我陈述不可靠；需开发更鲁棒的约束保持机制与人工/混合评估范式；本工作提供首个开源、多维、跨模型的约束依从性评测基准。
...
TL;DR: 本文探讨了句法依存距离最小化原则在星形结构中的应用难度及反向效应原因，证明星形和拟星形树的优化景观是凸的，表明优化问题比之前认为的更简单；同时指出竞争性原则而非优化难度才是导致星形结构中出现反依存距离最小化效应的真正原因，并强调在类星结构上进行依存距离最小化的收益较低。


<details>
  <summary>Details</summary>
Motivation: 解释为何在星形结构中观察到违背依存距离最小化原则的现象（即中心词常被置于线性序列末端），而路径结构中未见类似现象，并评估该优化问题的实际难度。

Method: 通过分析优化景观的数学性质（特别是凸性与拟凸性），结合已有理论（Ferrer-i-Cancho 2015）对星形树和拟星形树进行形式化论证，并比较不同结构下依存距离最小化的收益。

Result: 证实星形树和拟星形树的依存距离优化景观是凸的（一种特殊的拟凸性），因此优化问题比先前认为的更易解；同时指出反向效应源于其他竞争性语言原则，且星形结构中距离最小化的收益相对较低。

Conclusion: 依存距离最小化在星形结构中的‘失效’并非源于计算难度，而是由更高优先级的语言原则驱动，且该原则在星形结构中的优化增益有限；凸性结果简化了相关理论建模与实证检验。
...
TL;DR: 本文研究了在城市分析中使用大型语言模型（LLMs）作为人类感知代理时，基于角色（persona）的提示是否能产生有意义且可复现的行为多样性。结果表明，同一角色下的模型行为高度一致，但不同角色间差异微弱；经济地位和人格特质仅带来微小可测差异，性别和政治倾向几乎无影响；模型还存在极端化偏差，削弱细粒度情感判断能力；甚至无角色提示的基线模型在某些任务上表现更好。


<details>
  <summary>Details</summary>
Motivation: 探究 persona prompting 是否能在多模态 LLM 城市情感判断中产生真实、稳定且有区分度的行为多样性，以验证其作为人类感知代理的合理性与有效性。

Method: 采用涵盖性别、经济地位、政治取向和人格特质的因子化 persona 设计，为每个 persona 实例化多个智能体，在 PerceptSent 数据集上对城市场景图像进行情感评分；评估组内一致性与组间差异，并与无 persona 的基线模型对比；分析模型在不同粒度情感任务上的表现。

Result: 同一 persona 下各 agent 判断高度收敛；跨 persona 差异极小：仅经济地位和人格特质有统计显著但实际微弱的影响，性别无影响，政治取向影响可忽略；模型存在极端化偏差，压缩中间情感类别；细粒度情感任务性能下降；无 persona 模型在部分任务上与人类标注的一致性反而更高。

Conclusion: 基于标签的 persona prompting 在当前设置下未能有效建模人类感知的细粒度多样性，其引入的额外信息有限甚至可能降低模型对人类判断的拟合效果，需更精细的 persona 表征或交互式建模方法。
...
TL;DR: 本文探讨了在德语等高资源非英语语言中，数据质量与多样性之间的权衡，发现重复使用高质量过滤后的数据比单次训练大量低过滤度数据效果更好，并发布了德语大模型Boldt及评测基准。


<details>
  <summary>Details</summary>
Motivation: 对于德语等高资源非英语语言，如何在数据多样性（大量轻度过滤网页数据）和数据质量（严格过滤高质量核心数据并多轮训练）之间做选择存在战略困境。

Method: 构建分层质量过滤器处理5亿德语文档，对比多轮训练高质量子集与单次训练多样化大规模语料的效果，涵盖不同模型规模和token预算。

Result: 重复使用高质量数据始终优于单次训练更大但过滤更少的数据，性能优势在7个epoch后仍持续；发布的Boldt模型仅用10–360倍更少token即达SOTA。

Conclusion: 对非英语大语言模型，通过质量过滤实现语义浓缩比单纯扩大唯一数据量更有利于高效语言建模。
...
TL;DR: 本文提出TopBench基准，用于评估大语言模型在隐式预测型表格问答任务中的能力，发现当前模型在意图识别和预测推理方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多表格查询具有隐式预测性，需从历史模式中推断未观测答案，而现有LLM主要擅长信息抽取和简单聚合，难以应对此类任务。

Method: 构建TopBench基准，包含779个样本、四大子任务（单点预测、决策、处理效应分析、复杂过滤），涵盖文本与结构化输出；在文本式和智能体式工作流下评测多种模型。

Result: 实验表明当前模型常将预测任务误判为查找任务，意图识别准确率低；精准意图消歧是成功预测的前提；提升预测精度上限需引入更复杂的建模或推理能力。

Conclusion: 隐式预测型表格问答对LLM提出新挑战，亟需增强意图理解与预测推理能力，TopBench为该方向提供了系统评估框架。
...
TL;DR: 本文提出了一种统一框架，以明确区分语言处理中“分析单元”定义与“兴趣区域”选择这两个常被混淆的建模决策，并主张将分词视为实现细节而非科学基本概念。


<details>
  <summary>Details</summary>
Motivation: 现有基于惊奇度（surprisal）的研究中，语言单元定义不明确，实验刺激与预训练语言模型的tokenization不一致，导致建模选择混杂。

Method: 通过理论分析与概念澄清，构建一个能适配任意单元词表（unit inventories）的惊奇度统一框架。

Result: 明确了惊奇度分析中单位定义与兴趣区域选择应被显式分离，分词应视为实现细节。

Conclusion: 惊奇度理论的应用需显式声明分析单元和评估区域，提升可比性与理论严谨性。
...
TL;DR: 本文提出了一种高效的双流Transformer架构，用于从同步双摄像头视频中自动检测互视（MG）和共同注意（JA），在真实 caregiver-infant 数据集上显著优于CNN基线和多模态大语言模型。


<details>
  <summary>Details</summary>
Motivation: 传统手动编码MG和JA费时费力，而多摄像头环境下的自动化分析因跨相机关系建模复杂而面临计算挑战。

Method: 提出双流Transformer架构，结合冻结的凝视感知骨干网络（GazeLLE）提取视觉先验，并设计定制化token融合机制建模交互对的空间与语义关系。

Result: 在生态效度高的caregiver-infant数据集上性能优异，显著超越CNN基线和先进多模态LLM；模型与预训练权重已开源。

Conclusion: 该方法为行为科学家提供了可扩展、可微调的自动化工具，有效弥合了计算建模与应用互动研究之间的鸿沟。
...
TL;DR: 本文提出了RecGen，一种用于从稀疏RGB-D图像中概率性联合估计物体及其部件形状与姿态的生成框架，尤其适用于遮挡和部分可见场景。通过结合合成场景生成与强3D形状先验，RecGen在复杂遮挡数据集上达到SOTA性能，且训练开销显著降低。
...
Method: 提出RecGen生成框架，利用组合式合成场景生成与强3D形状先验，实现对物体及部件形状、姿态在遮挡/部分可见条件下的联合概率估计。

Result: 在严重遮挡、对称物体、部件级结构及复杂几何纹理等挑战下均表现鲁棒；相比SAM3D，几何形状质量提升30.1%，纹理重建提升9.1%，位姿估计提升33.9%，且训练网格减少近80%。

Conclusion: RecGen通过引入强先验与合成数据驱动的生成建模，在效率与精度间取得更好平衡，为真实世界多物体三维理解提供了新范式。
...
TL;DR: 本文提出InterPartAbility方法，通过显式的部件级匹配和短语-区域定位提升文本到图像行人重识别（TI-ReID）的可解释性，引入轻量级开放词汇的补丁-短语交互模块（PPIM）与概念引导训练，并设计定量可解释性评估协议，在保持检索精度的同时实现SOTA可解释性能。


<details>
  <summary>Details</summary>
Motivation: 现有TI-ReID模型虽性能强但决策不可解释；已有可解释方法仅依赖slot-attention，难以可靠绑定视觉区域与语义概念，解释受限于固定词汇且仅为定性可视化。

Method: 提出InterPartAbility框架：1）显式部件级匹配与短语-区域接地；2）开放词汇、轻量监督、补丁-短语交互模块（PPIM），利用概念级部件短语指导模型关注对应图像区域；3）约束CLIP ViT自注意力生成空间集中的补丁激活以生成接地解释图；4）构建基于扰动的定量可解释性评估协议（如反事实区域掩码）。

Result: 在CUHK-PEDES和ICFG-PEDES等基准上，InterPartAbility在所提定量可解释性指标上达到SOTA，同时保持具有竞争力的检索准确率。

Conclusion: InterPartAbility有效提升了TI-ReID模型的可解释性与可靠性，实现了概念级、可量化、接地化的解释能力，为多模态检索系统的可信部署提供了新路径。
...
Result: 在Edinburgh Pig数据集上，压缩流水线达到92.29% MOTA和96.15% IDF1（仅比SAM 3教师模型下降1.68/0.84个百分点），系统参数减少7.77倍，峰值VRAM由19.52GB降至6.49GB（3.01倍），行为分类达97.34% top-1准确率与91.67% macro-F1；整套模型可运行于Jetson Orin NX 16GB，留有4.9GB余量，并支持每头猪每年约94MB的嵌入池重识别机制。

Conclusion: 该工作首次实现了高性能基础模型流水线在边缘设备上的可行压缩与部署，为个体级牲畜长期视觉追踪与健康表型回溯分析提供了实用化技术路径。
...
TL;DR: 本文研究了知识蒸馏在植物物种和疾病识别任务中的应用，通过将大型预训练模型的知识迁移到小型高效架构中，显著提升了模型在资源受限环境（如移动或边缘设备）中的部署能力。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的视觉模型计算成本高，难以在资源受限的环境中部署，限制了生物多样性监测和精准农业系统的可扩展性。

Method: 采用知识蒸馏方法，将大型预训练模型（如Vision Transformers和ConvNeXt）的知识迁移到更小、更高效的模型中，并在Pl@ntNet300K-v2和Deep-Plant-Disease两个基准数据集上进行了广泛的实验评估。

Result: 知识蒸馏在所有任务和架构中均一致提升性能；蒸馏后的模型能在保持低计算成本的同时，达到与更大模型相当的性能。

Conclusion: 知识蒸馏是一种有效手段，有助于实现植物识别系统在现实环境应用中的高效与可扩展部署。
...
TL;DR: 本文提出HQ-UNet，一种在U-Net瓶颈处嵌入浅层参数化量子电路的混合量子-经典语义分割模型，以提升遥感图像分割的特征表达能力，在LandCover.ai数据集上超越经典U-Net。


<details>
  <summary>Details</summary>
Motivation: 经典遥感语义分割模型（如U-Net）参数量大；而量子机器学习虽具潜力，但直接处理高维遥感图像受限于当前量子硬件。需探索参数高效、硬件可行的量子-经典融合方案。

Method: 提出HQ-UNet：在经典U-Net编码器-解码器结构的瓶颈处，嵌入一个非池化的、浅层且参数高效的量子卷积模块（参数化量子电路），用于增强高度压缩的特征表示。

Result: 在LandCover.ai数据集上，HQ-UNet达到平均IoU 0.8050、总体精度94.76%，优于经典U-Net基线。

Conclusion: 紧凑型量子瓶颈可在近量子时代约束下有效增强遥感图像分割的特征表达，混合量子-经典架构是地球观测中参数高效密集预测的可行新方向。
...
TL;DR: 本文提出了一种基于互信息的‘表达性’度量方法，用于量化ReID模型特征对性别、姿态、BMI等属性的编码强度；发现BMI在可见光ReID中表达性最强且随网络深度增强，在跨光谱ReID中姿态（pitch）重要性上升，揭示了Transformer模型中隐式属性编码的层次结构。


<details>
  <summary>Details</summary>
Motivation: 现有ReID方法易受性别、姿态、BMI等属性影响，影响公平性与泛化性，需量化并理解模型对这些属性的隐式编码。

Method: 定义‘表达性’为学习特征与特定属性间的互信息，通过辅助神经网络估计；在可见光和跨光谱（SWIR/MWIR/LWIR）ReID任务上，对三种Transformer模型进行逐层、逐epoch的表达性分析。

Result: 可见光ReID中BMI表达性最高且随深度单调增强，排序为BMI > Pitch > Gender > Yaw；姿态在中间层达峰；跨光谱下pitch表达性接近BMI，且各属性表达性随深度单调上升。

Conclusion: Transformer-based ReID嵌入存在隐式属性编码层次：形态学信息（如BMI）稳定嵌入深层，而姿态信息在跨模态场景中作用更显著，提示需针对性解耦以提升公平性与鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 线性木本特征（如树篱）在农业景观中提供重要生态服务，但其大范围、可复用、可迁移的遥感制图仍面临传感器多样性、分辨率差异、成像条件及景观复杂性等方法学挑战。

Method: 构建模块化工作流：1）灵活输入接口，将异构遥感数据统一生成二值木本植被掩膜；2）深度神经网络，专用于从掩膜中识别并分离线性结构；整个流程使用单一训练模型适配多种数据源，无需重训练。

Result: 基于三类遥感数据，在德国全国尺度生成三套线性木本特征图；在四个联邦州的精细化实地参考数据上评估表现优异，并与两套现有地图对比显示具有全国一致性与竞争力。

Conclusion: 该模块化工作流具备良好的可扩展性与泛化能力，为跨国界、多区域的线性木本特征自动化制图提供了通用技术基础。
...
TL;DR: 本文提出VTBench框架，系统性地将图表可视化（线图、面积图、柱状图、散点图）与原始时间序列融合用于时间序列分类（TSC），通过多模态融合提升模型可解释性与性能，并在31个UCR数据集上验证其有效性及适用条件。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习TSC方法多依赖原始数值输入，忽略可视化表示；虽有纹理编码（如GAF、RP），但预处理重、可解释性差；而图表可视化更直观、可解释，但缺乏系统评估和统一框架。

Method: 构建VTBench框架：生成轻量、可解释的四种图表（线/面积/柱/散点）作为辅助输入；设计模块化架构支持三类融合策略——单图表-数值融合、多图表视觉融合、全模态（图表+原始序列）融合。

Result: 在31个UCR数据集实验表明：(1) 仅用图表的模型在小数据集上具竞争力；(2) 多图表融合可提升准确率；(3) 全模态融合在视觉特征非冗余时有效，冗余时可能降低性能；并提炼出图表选择与融合配置的实用指南。

Conclusion: VTBench为时间序列分类提供了首个统一、可扩展、可解释的多模态框架，证实图表可视化是原始序列的有效互补表征，推动TSC向兼顾性能与可解释性发展。
...
TL;DR: 本文提出了一种基于YOLOv8s的改进模型ALC-YOLOv8s，用于解决课堂中学生行为识别任务中存在的目标密集、小目标多、遮挡频繁及类别不平衡等问题。通过引入SPPF-LSKA、CFC-CRB、SFC-G2和ATFLoss等模块，提升了特征提取、多尺度融合与难样本学习能力，实验表明其在mAP50和mAP50-95上分别提升1.8%和2.1%，优于主流检测方法。


<details>
  <summary>Details</summary>
Motivation: 现实课堂场景中存在学生目标密集、小目标多、遮挡频繁以及类别分布不均衡等问题，导致现有方法难以准确识别学生行为，影响教学分析质量。

Method: 在YOLOv8s基础上构建ALC-YOLOv8s模型：引入SPPF-LSKA增强上下文特征提取；采用CFC-CRB和SFC-G2优化多尺度特征融合；使用ATFLoss提升对少数类和难样本的学习能力。

Result: 相比基准模型，mAP50提升1.8%，mAP50-95提升2.1%；在多个主流检测方法对比中，该模型在复杂教室场景下表现更优，满足自动学生行为识别需求。

Conclusion: ALC-YOLOv8s有效缓解了课堂行为识别中的关键挑战，显著提升了检测精度与鲁棒性，具备实际教学分析应用潜力。
...
TL;DR: YOSE是一种针对DiT视频生成的高效微调框架，通过动态选择关键token（BVI）和模拟未掩码区域对掩码区域的影响（DiffSim），实现掩码感知加速，在保持视觉质量的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有DiT视频生成方法（如MiniMax Remover）虽视觉质量高，但因全程密集计算全时空token导致推理延迟大（仅约10FPS），而实际只需处理小范围掩码区域。

Method: 提出YOSE框架，包含两个核心组件：1）Batch Variable-length Indexing（BVI），一种可微的动态索引算子，依据掩码信息自适应选取关键token；2）Diffusion Process Simulator（DiffSim）模块，近似建模未掩码token对掩码token在自注意力中的影响，以维持语义一致性。

Result: YOSE实现掩码感知加速，推理时间近似随掩码区域线性增长；实验表明其在70%情况下达到最高2.5倍加速，同时视觉质量与基线相当。

Conclusion: YOSE通过结构化稀疏计算与扩散过程近似，在不牺牲质量前提下显著提升视频对象移除任务的推理效率，为实时应用提供了可行路径。
...
TL;DR: 本文提出了一种无需训练、自适应迭代优化类别定义的零样本网页内容分类框架，利用大语言模型（LLM）根据误分类样例反馈改进类别描述，显著提升多种嵌入模型的分类性能，并发布了一个含10类、每类1000个样本的URL分类基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有基于嵌入的零样本网页内容分类方法对类别定义质量高度敏感，模糊或不准确的定义会导致嵌入空间语义重叠和系统性误分类，而定义质量这一关键因素长期被忽视。

Method: 提出一种训练无关的迭代定义优化框架，利用LLM作为反馈驱动的定义优化器，设计三种策略：示例引导式、混淆感知式和历史感知式，基于误分类实例的结构化信号持续精炼类别描述；并构建包含10个URL类别的新基准数据集（B2MWT-10C），在13种主流嵌入基础模型上进行评估。

Result: 迭代定义优化在所有测试的嵌入模型上均稳定提升分类性能，验证了定义质量是影响零样本嵌入系统效果的关键且未被充分探索的因素。

Conclusion: 类别定义的质量与模型架构同等重要；通过LLM驱动的定义优化可有效弥补零样本分类中缺乏标注数据的短板，为提升实际Web过滤系统的鲁棒性和准确性提供了新范式。
...
TL;DR: 本文提出了一种名为JI-ADF的三模态深度学习框架，融合皮肤镜图像、临床照片和结构化患者元数据，提升皮肤病变分类性能，尤其在真实临床场景与类别不平衡条件下表现稳健。


<details>
  <summary>Details</summary>
Motivation: 现有皮肤病变分类系统多依赖单一皮肤镜图像，未能充分利用临床中常见的多模态证据（如临床照片和患者元数据），限制了模型的临床适用性与鲁棒性。

Method: 提出JI-ADF框架，包含联合多模态表征学习、模态特异性辅助监督、自适应决策融合机制，以及新型多模态融合注意力（MMFA）模块，以增强跨模态推理并保留各模态特异性信息。

Result: 在大规模MILK10k基准上验证，JI-ADF显著提升敏感度与Dice分数，同时保持高特异性和良好校准性；消融实验、校准评估与Grad-CAM可视化进一步证实其鲁棒性与临床可解释性。

Conclusion: JI-ADF为真实临床环境下的多模态皮肤病变分类提供了可靠、实用的技术基础，推动计算机辅助诊断向更贴近临床实践的方向发展。
...
Method: 采用HRNet进行鲁棒骨骼关键点估计；构建身体比例、步态速度和骨骼运动三个互补特征分支；使用ResNet-50进行深层特征提取；设计基于通道注意力机制的多分支特征融合（MFF）模块动态加权融合异构特征流。

Result: 在CASIA-B数据集上，正常行走条件下Rank-1准确率达94.52%，在穿大衣条件下性能为当前骨架基方法最优。

Conclusion: 所提多分支协同融合框架能有效提升步态识别鲁棒性与精度，尤其在复杂协变量场景下展现出优越性。
...
TL;DR: CasLayout is a cascaded diffusion framework for 3D indoor scene synthesis that decomposes generation into four physically and semantically grounded stages, incorporates building constraints and sparse human-aligned relation graphs, and enables zero-shot image-to-scene generation with improved fidelity, diversity, and controllability.


<details>
  <summary>Details</summary>
Motivation: Synthesizing realistic 3D indoor scenes is hindered by data scarcity and the difficulty of jointly satisfying global architectural constraints and local semantic consistency; existing methods often ignore structural boundaries or suffer from redundant errors due to dense, fully connected relation graphs.

Method: CasLayout introduces a four-stage cascaded diffusion process: (1) furniture quantity/category prediction, (2) object size and feature refinement, (3) latent-space spatial relationship modeling, and (4) Oriented Bounding Box (OBB) generation; it explicitly conditions on building elements (walls, doors, windows) and uses a sparse, human-aligned relation graph encoded via a bidirectional VAE to improve relational controllability and physical validity.

Result: CasLayout achieves state-of-the-art performance in fidelity and diversity on 3D indoor scene generation benchmarks, and enables flexible zero-shot tasks (e.g., image-to-scene) with enhanced controllability and physical plausibility.

Conclusion: Decoupling scene generation into semantically and physically meaningful stages—combined with sparse relational modeling and explicit architectural conditioning—effectively addresses key limitations in data efficiency, structural coherence, and controllability for 3D indoor scene synthesis.
...
TL;DR: 本文提出了一种端到端的双阶段光谱约束聚类分类器（DSCC），通过生成边界保持的光谱超令牌并进行令牌级预测，解决传统超像素方法中聚类与分类不一致的问题，显著提升高光谱图像分类的空间一致性与边界精度。


<details>
  <summary>Details</summary>
Motivation: 现有超像素方法在聚类后仍采用像素级分类，导致区域一致性差、边界划分不精确，无法实现区域级、边界对齐的分类。

Method: 提出DSCC框架：第一阶段基于多准则特征距离和局部感知分配正则化生成光谱超令牌；第二阶段采用密度-隔离中心选择与软标签策略实现令牌级分类；整体为端到端可训练结构。

Result: 在WHU-OHS数据集上达到CF1为0.728、速度达197.75 FPS，精度-效率权衡优于当前最优方法；实验验证了该双阶段范式的有效性与泛化性。

Conclusion: DSCC通过显式解耦聚类与分类，并引入边界保持的超令牌表示与软标签机制，有效提升了高光谱图像分类的空间一致性、边界精度与鲁棒性，为遥感图像解译提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA方法未充分利用其评判（critic）能力来优化驾驶决策，限制了其在复杂闭环场景中的性能。

Method: 提出两阶段CriticVLA框架：第一阶段生成初始轨迹；第二阶段利用VLA-based critic进行多模态评估和单步优化以 refine 轨迹；并构建含1290万条标注轨迹的大规模合成数据集支撑critic训练。

Result: 在Bench2Drive闭环评测中，CriticVLA总成功率高达73.33%，在挑战性场景中提升约30%。

Conclusion: 赋予VLA模型显式的critic角色可有效提升自动驾驶策略质量，验证了‘acting + judging’双阶段范式的有效性与可扩展性。
...
TL;DR: 本文提出COHERENCE基准，用于评估多模态大语言模型（MLLMs）在图文交错上下文中的细粒度图文对应理解能力，并进行六类错误分析以定位现有模型的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单图或多图理解，缺乏对真实场景中图文交错上下文（如文档阅读）下细粒度图文对齐与推理能力的系统评估。

Method: 构建名为COHERENCE的新型基准，涵盖四个代表性领域的6161个高质量问题，设计六类错误分析框架以归因模型失败原因。

Result: 提供了首个专门针对交错图文上下文中细粒度图文对应理解能力的系统性评估基准，并揭示了当前MLLMs在图文对齐、跨模态推理等方面的关键能力缺失。

Conclusion: COHERENCE填补了交错图文理解评估的空白，为推动MLLMs在复杂真实场景中的细粒度多模态理解能力发展提供了重要工具和方向。
...
TL;DR: 本文系统研究了视觉-语言模型（VLMs）在自动驾驶中对物理对抗攻击的跨架构可迁移性，发现路边物理补丁攻击在Dolphins、OmniDrive和LeapVAD三种主流VLM架构间具有高达91%的高迁移率，表明当前VLM驾驶系统存在严重的共性鲁棒性隐患。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在自动驾驶中应用日益广泛，但其面对物理对抗攻击（尤其是攻击者不知晓车载具体模型时）的跨架构鲁棒性尚不明确，构成实际安全风险。

Method: 开展系统性跨架构对抗迁移实验，使用物理上可实现的补丁（置于人行横道与高速公路路边设施），评估Dolphins、OmniDrive和LeapVAD三种典型VLM架构间的攻击迁移效果，并构建迁移矩阵分析成功率与持续帧占比。

Result: 物理补丁攻击在不同VLM间迁移率高达73–91%（人行横道均值81.5%，高速公路均值83.3%），且在未针对目标模型优化的情况下，仍能在64.7–79.4%的关键决策窗口内持续干扰帧级判断。

Conclusion: 主流VLM驾驶模型存在显著的跨架构脆弱性共性，亟需统一鲁棒性评测标准与防御机制。
...
TL;DR: 本文提出了一种面向无约束真实场景的3D稀疏视角合成新框架，通过参考引导的扩散模型视图优化与稀疏区域高斯增强策略，在含干扰物的稀疏图像集上实现了高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以兼顾稀疏视角与无约束真实场景（含瞬态干扰物），要么依赖受限图像，要么需密集采集；本文旨在解决稀疏、无约束、含干扰的真实图像集合下的高质量3D重建问题。

Method: 引入参考引导的视图细化扩散模型（结合瞬态掩码与参考图像）以提升3D表征并抑制伪影；提出伪视角生成与稀疏感知的高斯复制策略，增强高斯场中稀疏区域的密度。

Result: 在公开数据集上显著超越现有方法（PSNR +17.2%，SSIM +10.8%，LPIPS -4.0%），实现高保真3D渲染。

Conclusion: 该方法有效拓展了3D稀疏视角合成在真实复杂场景中的适用性，降低了对大量人工采集数据的依赖，推动了无约束场景下实用化3D重建的发展。
...
TL;DR: 本文提出Softmax-GS，通过在重叠高斯分布间引入基于softmax的竞争机制，解决3D高斯点绘（3D GS）中因高斯重叠导致的视角不一致与边界模糊问题，提升重建质量与参数效率。


<details>
  <summary>Details</summary>
Motivation: 3D高斯点绘（3D GS）因高斯不重叠假设导致视角不一致和物体边缘模糊，影响重建质量。

Method: 提出Softmax-GS，在重叠区域引入可学习的softmax竞争机制，保证顺序不变性与透射率一致性，实现从平滑融合到锐利边界的连续控制。

Result: 在简单几何体消融实验和真实世界基准测试中均验证有效性，达到当前最优性能，提升重建质量与参数效率。

Conclusion: Softmax-GS为3D GS提供了一种统一、鲁棒且高效的改进方案，有效缓解了视角不一致与边界模糊两大核心缺陷。
...
TL;DR: 本文提出CatSignal，一种贝叶斯启发的多模态意图推断框架，将空间上下文建模为类先验约束、行为观测建模为证据，通过上下文门控的专家乘积（PoE）实现更鲁棒的意图推理，在家猫数据集上显著提升准确率并抑制上下文捷径错误。


<details>
  <summary>Details</summary>
Motivation: 现实环境中许多非语言代理（如宠物、婴儿）无法用语言表达目标，需从含噪、欠定的行为观测和富含信息的上下文中推断意图；但直接使用上下文易导致捷径预测（shortcut predictions）问题。

Method: 提出CatSignal框架：将空间上下文作为类先验约束，行为观测（姿态动态、声学线索）作为证据；采用上下文门控的专家乘积（Product-of-Experts）机制进行后验式意图分布建模，而非简单特征拼接或晚期融合。

Result: 在Leave-One-Video-Out评估下，CatSignal达到77.72%整体准确率，优于特征拼接（71.83%）及更强晚期融合基线；显著降低模糊场景下的上下文捷径失败，且在抑制上下文驱动的捷径崩溃方面表现最优。

Conclusion: 将上下文显式建模为类先验约束、与多模态行为证据协同推理，可提升非语言代理意图推断的鲁棒性与泛化性，尤其在存在歧义和噪声的现实场景中具有优势。
...
TL;DR: 本文提出LA-Pose方法，利用逆动力学自监督预训练学习动作表征，并将其作为输入用于相机位姿估计，在大幅减少标注数据需求的同时实现高精度与泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有位姿估计方法严重依赖大量带3D标注的监督数据，成本高昂；而自监督预训练（尤其是逆动力学）尚未被探索用于该任务。

Method: 采用逆动力学与前向动力学模型从驾驶视频中学习潜在动作表征；将学到的潜在动作特征作为输入，微调一个轻量级相机位姿估计器。

Result: 在Waymo和PandaSet基准上，LA-Pose相比最新前馈方法提升超10%位姿精度，且仅需极少量高质量3D标注数据。

Conclusion: 逆动力学自监督预训练可有效迁移至相机位姿估计任务，LA-Pose首次验证了该范式的有效性与高效性。
...
TL;DR: 本文提出EdgeFM，一种轻量级、代理驱动的视觉语言模型/大语言模型推理框架，专为跨平台工业边缘部署设计，通过移除非必要功能和模块化代理调优内核优化，显著提升推理性能并增强可移植性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在边缘工业应用中受限于确定性低延迟和资源约束下的稳定执行需求，且现有框架存在通用设计臃肿或依赖封闭硬件生态的问题，导致硬件锁定和跨平台适应性差。

Method: 基于AI代理可高效搜索与调优标准LLM算子低层内核的观察，设计轻量级、代理驱动的EdgeFM框架，移除非必要功能以降低单请求延迟，并将代理调优的内核优化封装为可复用的模块化技能库。

Result: EdgeFM在NVIDIA Orin平台上相比TensorRT-Edge-LLM最高提速1.49倍，原生支持x86和NVIDIA Orin SoCs，并首次实现国产Horizon Journey平台上的端到端VLA部署，展现出优于传统厂商工具链的推理性能。

Conclusion: EdgeFM提供了一个开源、生产级的解决方案，有效弥合了开源与专有工具链之间的性能差距，提升了边缘工业场景下的跨平台部署能力与实用性。
...
TL;DR: 本文提出Uni-HOI，一个基于大语言模型和运动专用VQ-VAEs的统一框架，用于建模文本、人体运动与物体运动三者的联合分布，支持多种4D人-物交互任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法多为任务定制化架构，缺乏能处理多样化条件输入的统一HOI建模框架。

Method: 提出Uni-HOI框架：利用LLM与两个运动专用VQ-VAE将异构运动数据转为token序列；采用两阶段训练策略——第一阶段在大规模HOI数据集上多任务学习三模态关联，第二阶段针对具体任务微调。

Result: 在文本驱动HOI生成、物体运动驱动人体运动生成（可选文本）、人体运动驱动物体运动预测等多个HOI任务上均取得显著性能提升。

Conclusion: Uni-HOI实现了跨模态、多任务的统一建模，验证了联合学习文本、人体与物体运动分布的有效性与泛化能力。
...
TL;DR: 本文提出了首个大规模红外离线道路时空自由空间检测数据集IRON，并基于此设计了无需光流的IRONet框架，通过记忆注意力机制和掩码解码器解决帧间不一致性问题，在夜间感知和跨模态泛化上均取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 离线道路夜间自主驾驶中可见光感知不可靠，红外模态至关重要，但缺乏标注充分的大规模红外离线道路数据集，且现有单帧方法存在帧间不一致问题。

Method: 构建了包含24,314张密集标注红外图像及同步RGB图像的IRON数据集；提出无需光流的IRONet框架，采用记忆-注意力机制聚合历史上下文，并设计专用掩码解码器。

Result: 在IRON数据集上达到82.93% IoU（+1.19%）和90.66% F1（+0.71%），实时推理；在ORFD和Rellis RGB数据集上也表现出强泛化能力。

Conclusion: IRON数据集与IRONet为全天候离线道路自主驾驶及红外时序感知研究奠定了基础。
...
TL;DR: REVIVE 3D 是一种两阶段生成框架，用于从单张二维平面图像生成具有真实体积感的三维资产，通过构建‘膨胀先验’和‘三维潜在空间细化’提升几何完整性，并提出新指标 Compactness 和 Normal Anisotropy 来量化体积与表面质量。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型难以从缺乏深度线索的平面图像中生成具有真实体积感的3D资产。

Method: 提出两阶段框架：Stage 1 构建 Inflated Prior（膨胀前景轮廓并叠加部件细节）；Stage 2 进行 3D Latent Refinement（在潜在空间加噪-去噪，利用预训练3D知识）；同时引入 Compactness 和 Normal Anisotropy 两个新评估指标。

Result: 在挑战性平面图像数据集上达到SOTA性能，用户研究表明新指标与人类对体积和质量的感知高度一致，并支持图像条件下的3D编辑。

Conclusion: REVIVE 3D 有效缓解了从单图生成具真实体积3D资产的难题，其模块化设计具备即插即用特性，且评估指标更具感知一致性。
...
TL;DR: 本文提出Edit-R1框架，通过构建基于链式思维（CoT）的推理奖励模型（Edit-RRM），解决图像编辑中缺乏细粒度、可解释、任务适配奖励模型的问题；采用监督微调冷启动+分组对比偏好优化（GCPO）训练RRM，并用GRPO将其用于编辑模型优化，显著提升FLUX.1-kontext等模型的编辑性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑的奖励模型仅给出整体评分，忽略指令多样性与细粒度要求，导致奖励偏差；RLHF在图像编辑中的应用受限于缺乏通用、鲁棒的编辑专用奖励模型。

Method: 提出Edit-R1框架：1）构建Edit-RRM，将编辑指令分解为多个原则，逐项验证并聚合为可解释的细粒度奖励；2）用SFT冷启动生成CoT奖励轨迹；3）提出GCPO算法，利用人类成对偏好数据优化点式RRM；4）用GRPO将非可微的Edit-RRM用于编辑模型训练。

Result: Edit-RRM在图像编辑任务上超越Seed-1.5-VL/1.6-VL等强VLM；呈现清晰的参数规模扩展规律（3B→7B性能持续提升）；成功提升FLUX.1-kontext等编辑模型效果。

Conclusion: 从简单打分转向基于链式思维的推理验证是构建高质量图像编辑奖励模型的关键路径；Edit-R1为RLHF在图像编辑领域的落地提供了可解释、可扩展、任务适配的新范式。
...
TL;DR: 本文提出了一种无幻觉的反演框架，首次在像素级验证了视觉编码器中存在强叠加现象，并揭示分类依赖于权重间的破坏性干涉而非传统假设的‘空间漏斗’机制；进而基于干涉子空间体积与GAP协方差行列式对偶性，提出具有理论保证的通道选择算法，并将OOD失效归因于该体积的坍缩。


<details>
  <summary>Details</summary>
Motivation: 检验CNN可解释性中的‘空间漏斗假说’（即编码器抑制背景、分类器仅从干净特征中选择），但现有可视化工具存在空间幻觉，导致该假设长期未被验证。

Method: 提出基于幅值-相位解耦和局部伴随校正器（Local Adjoint Correctors）的无幻觉反演框架，确保每个重建的空间梯度支撑严格来自真实激活通道；以此为几何探针分析通道反演特性，并建立干涉子空间体积与GAP协方差行列式的对偶关系，推导出具有(1-1/e)近似保证的协方差-体积通道选择算法。

Result: 发现单通道反演具有全息性（正负权重重建视觉与能量不可区分），但其代数和锐利聚焦于前景；证明分类通过破坏性干涉实现（分类器权重在像素空间中抵消共享背景方向、构造类别判别残差）；推导出通道需求由干涉子空间体积决定；OOD失效被量化为该体积的坍缩；方法可无缝扩展至注意力头。

Conclusion: ‘空间漏斗假说’被证伪；分类本质是像素空间中的干涉过程；干涉子空间体积是核心几何量，其坍缩可解释OOD失败；所提算法兼具理论保证与实用扩展性。
...
TL;DR: 本文研究了自监督学习（SSL）在植物图像表征学习中的应用，发现常用SSL增强策略（如高斯模糊、灰度化、日晒）会削弱植物细粒度识别所需的关键判别线索；提出更适合植物识别的增强方法（如仿射变换、色调分离），并验证了在iNaturalist 2021 Plantae子集上训练SimDINOv2优于ImageNet-1K，且在少样本下游任务中可媲美甚至超越强监督基线Pl@ntCLEF和BioCLIP。


<details>
  <summary>Details</summary>
Motivation: 现有植物自动识别方法严重依赖专家标注数据，而自监督学习虽具可扩展性，但其通用增强策略和预训练协议针对粗粒度视觉任务设计，难以适配植物物种等细粒度识别需求。

Method: 系统评估SSL中各类图像增强对植物识别的影响，筛选出更适配的增强方式（如仿射变换、色调分离）；在iNaturalist 2021 Plantae子集上训练SimDINOv2模型，并与ImageNet-1K预训练对比；在ViT-Base和ViT-Large架构下验证泛化性；在少样本下游任务中与Pl@ntCLEF、BioCLIP等监督基线比较性能。

Result: 发现常用SSL增强（高斯模糊、灰度化、日晒）损害植物细粒度识别；仿射变换和色调分离更有效；iNaturalist Plantae上训练的SimDINOv2表征显著优于ImageNet-1K；模型在少样本设置下达到或超过Pl@ntCLEF和BioCLIP性能。

Conclusion: 植物等细粒度领域需领域适配的SSL增强策略与专用数据集，这对构建可扩展的生物多样性监测模型至关重要；本文为SSL在生态图像分析中的实践提供了具体指导。
...
TL;DR: 本文提出残差高斯点绘（RGS）方法，通过将体素场分解为几何基成分和残差细节成分，并结合小波多分辨率分析与谱-空协同优化策略，解决超稀疏视角下锥束CT重建中因光度优化导致的频谱偏差问题，显著提升解剖高频细节的重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统基于光度优化的3D高斯点绘在超稀疏视角锥束CT重建中存在频谱偏差，导致过平滑及高频解剖细节丢失。

Method: 提出残差高斯点绘（RGS），采用谱解耦的高斯表示，将体素场分解为几何基与残差细节两部分；引入小波多分辨率分析增强高频信息，并设计谱-空协同优化策略以避免频谱串扰。

Result: 在临床数据集上验证表明，RGS能有效重建精细几何纹理，在松质骨与血管等复杂结构中优于现有神经渲染方法，兼顾伪影抑制与细节保留。

Conclusion: RGS通过物理一致的残差建模与谱-空协同优化，克服了显式高频拟合的数学与物理限制，为稀疏视角CT重建提供了更鲁棒、高保真的显式表示新范式。
...
TL;DR: 本文研究了大型视觉语言模型（LVLM）在描述图像中以不同视觉风格呈现的文字所指概念时，是否受到文字样式（如字体、颜色、大小）的影响；实验发现即使概念识别正确，文字风格仍会显著影响模型对概念属性的描述，揭示了‘风格泄露’现象，并呼吁在多媒体系统中开展风格感知的评估与缓解。


<details>
  <summary>Details</summary>
Motivation: 尽管文字的视觉风格（如字体、颜色、大小）多种多样，但其语义应独立于样式；然而，LVLM在处理图像中渲染的文字时，其语义推理是否被无关的视觉风格干扰尚不明确，这关系到模型鲁棒性与公平性。

Method: 通过对比功能性文本样式（如黑色无衬线体，强调可读性）与装饰性样式（如彩色草书/手写体，强调展示效果），在LVLM能准确识别图像中文字所指概念的前提下，系统分析其生成的概念属性描述是否受样式影响。

Result: 实验表明，即使LVLM正确识别出文字对应的概念，其生成的属性描述仍显著受文本视觉风格影响，证实存在‘风格泄露’——即无关视觉样式干扰语义推理过程。

Conclusion: LVLM的语义推理并非完全样式不变，视觉文本风格会非平凡地渗透至高层语义描述中；该发现要求未来LVLM评估与部署需纳入风格敏感性考量，并开发相应缓解机制。
...
TL;DR: 本文提出RIHA框架，通过多级对齐（段落、句子、词）实现放射影像与结构化报告间的细粒度跨模态对齐，引入视觉/文本特征金字塔、基于最优传输的跨模态对齐模块及相对位置编码，显著提升胸片报告生成的自然语言质量与临床有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将放射报告视为扁平序列，忽略其结构化分段和语义层次，导致跨模态对齐不精确、生成准确性受限。

Method: 提出RIHA端到端框架：构建视觉特征金字塔（VFP）和文本特征金字塔（TFP），设计基于最优传输的跨模态分层对齐（CHA）模块，并在解码器中引入相对位置编码（RPE）以增强token级对齐。

Result: 在IU-Xray和MIMIC-CXR两个胸部X光基准数据集上，RIHA在自然语言生成指标（如BLEU、CIDEr）和临床有效性指标（如CheXpert标签准确率）上均超越当前最优方法。

Conclusion: 多级结构化对齐是提升放射报告生成质量的关键，RIHA通过显式建模图像与报告的多层次语义对应关系，有效提升了生成报告的准确性、可解释性与临床实用性。
...
TL;DR: 本文提出World2Minecraft框架，将真实世界场景高保真重建为结构化Minecraft环境，以支持具身智能研究；为提升3D语义占据预测精度，构建了大规模室内占据数据集MinecraftOcc，并验证其对现有SOTA方法构成显著挑战。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能仿真平台存在数据污染和灵活性不足问题，且3D语义占据预测受限于数据稀缺与泛化能力差。
...
Result: MinecraftOcc数据集显著补充了现有数据资源，对当前SOTA占据预测方法构成严峻挑战；重建场景成功支持VLN等下游任务。

Conclusion: World2Minecraft提供了一个可定制、可编辑的具身AI研究平台，所提数据集和方法有效推动了3D占据预测性能提升与个性化具身智能研究发展。
...
TL;DR: 本文提出了CURVAS-PDACVI数据集与挑战赛，旨在推动面向胰腺癌（PDAC）术前分期的不确定性感知AI研究；该数据集包含每例扫描由五位专家独立标注的密集标注，并提出多指标评估框架，强调仅依赖体积重叠指标不足以反映临床关键区域（如肿瘤-血管界面）的实用性能，指出建模专家间分歧的概率模型更具鲁棒性和校准性。


<details>
  <summary>Details</summary>
Motivation: 现有计算方法在胰腺导管腺癌（PDAC）血管侵犯（VI）评估中面临两大挑战：缺乏公开数据集、以及肿瘤-血管界面诊断模糊导致专家间差异大；而准确VI评估对术前分期和手术规划至关重要。

Method: 构建了CURVAS-PDACVI开放基准数据集（含每例CT扫描的5份独立专家标注），并设计了涵盖空间重叠、概率校准与VI临床判别能力的多维度评估框架；对比评估了6种前沿方法，特别分析其在高歧义病例中的表现。

Result: 实验表明：高全局体积重叠性能不等于在关键肿瘤-血管界面处可靠；二值分割方法在平均指标上表现好，但在低专家共识的复杂病例中易崩溃或过延伸；而建模专家分歧的方法生成更校准的概率图，在模糊区域更鲁棒。

Conclusion: 体积精度不能替代临床局部决策效用，应发展不确定性感知的概率模型以支持PDAC术前决策。
...
TL;DR: 本文提出Fake3DGS数据集和一种3D感知的检测方法，用于识别基于3D高斯溅射（3D Gaussian Splatting）生成的伪造图像，弥补了现有2D检测器在3D伪造内容鉴别上的不足。


<details>
  <summary>Details</summary>
Motivation: 随着3D重建与神经渲染（尤其是3D高斯溅射）的发展，高质量3D内容编辑与重渲染变得简单可行，但由此引发的3D内容真实性安全问题尚未被系统研究，尤其缺乏面向3D伪造的检测方法与基准数据集。

Method: 构建Fake3DGS数据集，包含经几何、外观和空间布局可控篡改生成的3D高斯溅射场景及其渲染图像；提出一种利用多视角一致性及高斯溅射表征特征的3D感知检测方法。

Result: 实验表明当前SOTA 2D检测器难以区分原始与3D篡改图像，而所提3D感知方法显著提升了对修改后3D内容的识别能力。

Conclusion: 3D伪造检测亟需超越2D证据的新方法，Fake3DGS数据集与所提3D感知检测框架为该方向提供了重要基础与验证。
...
TL;DR: 本文提出ClipTBP框架，通过clip-level对齐损失和主/辅边界损失，提升视频时刻检索中多答案片段的语义建模与时间边界预测精度，尤其在模糊查询下更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有方法在snippet级计算相似度，忽略同一查询对应多个答案片段之间的关系，易受上下文视觉相似片段干扰，难以排除无关片段。

Method: 提出ClipTBP框架：1）引入clip-level对齐损失，显式建模答案片段间的语义关系；2）结合主边界损失与辅助边界损失进行精确时间边界预测。

Result: ClipTBP在多种基线模型上持续提升性能，在模糊查询场景下展现出更强的时间边界预测鲁棒性。

Conclusion: clip-level语义关系建模与双重边界监督可有效缓解多答案干扰问题，提升视频时刻检索的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有OWSSL方法无法在无显式监督的情况下为新类别提供语义可解释的预测，且缺乏提取潜在语义信息的机制，导致预测标签与候选文本标签无语义对应关系。

Method: 提出SEmantic Capture for Open-world Semi-supervised learning（SECOS），利用外部知识提取并跨模态对齐已知与新类别的语义表征，实现对候选文本标签的直接预测，无需后处理。

Result: SECOS在更宽松的后验匹配设定下仍比现有OWSSL方法最高提升5.4%，验证其有效性；代码已开源。

Conclusion: SECOS通过语义捕获与跨模态对齐，解决了OWSSL中新类别缺乏显式监督和语义不一致的问题，满足实际应用中严格、语义驱动的分类需求。
...
TL;DR: SPUR是一个面向科学实验图像感知、理解和推理的综合基准，包含4264个问答对，用于评估多模态大语言模型在细粒度感知、跨图板关系理解及专家级推理三方面的能力，结果显示现有模型距离专家水平仍有显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前AI for Science（AI4S）研究中，多模态大语言模型在科学图像解释方面存在关键瓶颈，缺乏能全面评估其感知、理解与推理能力的权威基准。

Method: 构建SPUR基准，涵盖面板级细粒度感知（数值、形态、信息定位）、跨面板关系理解（平均14.3个面板/样本）和专家级推理（五类实验范式），并系统评测20种MLLMs与4种多模态思维链方法。

Result: 现有20种MLLMs和4种MCoT方法在SPUR各项任务上均远未达到人类专家水平，尤其在跨面板关系理解和定量推理方面表现薄弱。

Conclusion: SPUR揭示了当前MLLMs在科学图像理解上的根本性局限，亟需在模型架构、训练策略和推理机制等方面取得突破，以推动AI4S发展。
...
TL;DR: 本文提出了一种轻量级CNN框架AttXNet，用于无人机桥梁巡检中的裂缝自动检测，兼顾精度、速度与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决无人机桥梁巡检中裂纹特征弱、成像质量差、类别严重不平衡及计算资源受限四大实际挑战。

Method: 构建包含轻量骨干网络、CBAM注意力模块、基于巡检场景先验的定向增强策略和Focal Loss的统一轻量CNN框架。

Result: 在SDNET2018数据集上达到825 FPS推理速度、11.21M参数和1.82G FLOPs；F1-score提升2.51%，召回率提升3.95%；Grad-CAM显示注意力模块使模型聚焦于裂纹轨迹。

Conclusion: 该方法在精度、速度与鲁棒性间取得良好平衡，适用于地面站辅助的无人机实时桥梁巡检部署。
...
TL;DR: 本文提出SpaAct框架，通过引入Action Retrospection和Future Frame Selection两个空间激活任务，增强视觉语言模型（VLM）在视觉-语言导航（VLN）中的动态空间感知能力，并结合TriPA渐进式自适应课程学习方法，显著提升VLM在VLN任务上的性能，达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在VLN任务中缺乏对空间动态变化的感知能力，需同时具备向后动作推理（why）与向前状态转移预测（how）两种互补能力。

Method: 提出SpaAct训练框架，包含两个空间激活任务：Action Retrospection（从视觉变化反推动作序列）与Future Frame Selection（基于历史与动作预测未来帧）；并设计TriPA三因子渐进式课程学习策略，按难度递增组织训练样本。

Result: 在VLN-CE标准基准上，SpaAct持续提升VLM导航性能，达到当前最优（SOTA）结果。

Conclusion: 通过轻量级空间监督与渐进式课程学习，可有效激活VLM的动态空间意识，为VLM适配具身导航任务提供新范式。
...
TL;DR: 本文提出了一种名为Focal U-shaped Network (FUN)的端到端框架，用于快照式高光谱成像（HSI）的重建与目标检测联合优化，引入了计算高效的focal modulation替代自注意力机制，并构建了一个新的HSI目标检测数据集。


<details>
  <summary>Details</summary>
Motivation: 传统推扫式高光谱成像速度慢，难以实现实时目标检测；而快照式高光谱成像虽可瞬时采集，但后处理重建耗时严重制约其实时性。

Method: 提出FUN网络：共享U型骨干网络实现多任务学习（HSI重建+目标检测），引入focal modulation机制替代自注意力以降低计算复杂度，并构建含363幅HSI、8712个标注目标的新数据集。

Result: FUN在重建与检测两项任务上均达SOTA性能，参数量减少40%，计算量降低30%，具备实时边缘部署潜力。

Conclusion: FUN通过高效多任务联合学习与轻量化设计，显著提升了快照式高光谱成像系统在实时目标检测中的实用性，为边缘智能遥感应用提供了新范式。
...
TL;DR: 本文提出了一种用于颈椎CT-MRI刚性-形变混合配准的新方法MSR，并发布了首个全面标注的R-D-Reg数据集，以解决该领域因解剖复杂性和缺乏高质量标注数据而导致的研究不足问题。


<details>
  <summary>Details</summary>
Motivation: 颈椎CT-MRI配准对术前规划至关重要，但因解剖结构复杂、个体差异大、易损伤椎动脉和脊髓，且缺乏高质量多模态标注数据，相关研究仍不充分，尤其在刚性-形变混合建模方面。

Method: 构建并公开发布首个全面标注的CT-MRI数据集R-D-Reg；提出MSR框架，包含独立椎体局部刚性配准模块和融合Mamba（全局建模）与Swin Transformer（局部建模）的MSL形变模块，并通过自适应门控融合刚性与形变场。

Result: MSR在颈椎CT-MRI配准任务中实现了更优的局部解剖一致性保持，显著提升配准精度；R-D-Reg数据集和代码已开源。

Conclusion: MSR为复杂关节结构的多模态刚性-形变混合配准提供了有效新范式，R-D-Reg数据集填补了该领域高质量标注数据空白，推动临床精准诊疗发展。
...
TL;DR: 本文提出EviMem，结合IRIS（通过不足信号进行迭代检索）和LaceMem（分层式对话证据记忆架构），以显式诊断证据缺口并实现目标导向的查询优化，显著提升长期对话记忆中时序与多跳问题的回答准确率。


<details>
  <summary>Details</summary>
Motivation: 现有单次检索方法在处理时序和多跳问题时表现不佳，而迭代方法缺乏对证据缺口的显式诊断，导致查询优化缺乏针对性。

Method: 提出EviMem框架，包含IRIS（基于充分性评估检测并诊断证据缺口，驱动目标查询优化）和LaceMem（支持细粒度缺口诊断的粗到细记忆层次结构）。

Result: 在LoCoMo数据集上，EviMem相比MIRIX将时序问题判断准确率从73.3%提升至81.6%，多跳问题从65.9%提升至85.2%，且延迟降低4.5倍。

Conclusion: 显式证据缺口诊断与分层记忆结构可有效提升长期对话中复杂问题的检索与推理性能。
...
TL;DR: 本文提出了一种基于深度学习的自动分割CT图像中rPCI解剖区域的方法，使用nnU-Net和Swin UNETR在62例标注CT上进行评估，结果表明nnU-Net（Dice=0.82）接近专家间一致性（0.88），验证了无创影像学评估rPCI的可行性。


<details>
  <summary>Details</summary>
Motivation: sPCI虽临床常用但具侵入性，且缺乏标准化的影像学对应方法（rPCI）；现有rPCI定义了3D解剖区域，亟需自动化、可重复的CT分割方法以支持非侵入性评估。

Method: 采用nnU-Net和Swin UNETR两种模型，在62例经三位临床研究者标注、两位放射科专家验证的CT数据上，通过五折交叉验证评估分割性能，指标包括Dice相似系数、95%豪斯多夫距离和平均表面距离。

Result: nnU-Net整体Dice为0.82，接近专家间一致性（0.88），优于Swin UNETR（0.76）；难点集中于右腰部和小肠区域。

Conclusion: 该研究证实了自动rPCI区域分割的可行性，为实现非侵入性、影像驱动的腹膜癌指数评估奠定了技术基础。
...
TL;DR: 本文提出了一种基于NeRF的视频快照压缩成像（SCI）重建方法，通过补丁级光线采样、Inter-和Intra-Ray Transformer（RayFormer）建模结构相似性，并引入总变差先验提升重建质量，实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF-based视频SCI方法采用随机光线采样，难以捕捉内容结构相似性，导致重建质量受限。

Method: 提出补丁级光线采样策略；设计Inter-和Intra-Ray Transformer（RayFormer）建模空间邻近点间的光线间相似性及视线方向上相邻点的光线内相关性；结合补丁采样引入总变差先验以增强空间平滑性。

Result: 在仿真与真实场景实验中均达到当前最优（SOTA）重建性能。

Conclusion: 所提方法有效提升了视频SCI的重建质量，验证了结构建模与正则化联合优化的有效性。
...
TL;DR: 本文提出了一种新颖的预训练策略，旨在引导模型在预训练阶段避免学习领域特定特征，从而提升其在遥感图像分割任务中的泛化能力，并在多个数据集上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在遥感图像分割中常使用ImageNet预训练，但因ImageNet与遥感图像之间存在显著领域差异（场景与模态不同），导致迁移效果受限；而构建大规模遥感专用预训练数据集又费时费力且泛化性差。

Method: 提出一种简单但有效的预训练策略，使模型在预训练过程中主动抑制对领域特定特征的学习，以增强跨场景、跨模态的泛化能力。

Result: 在iSAID、MFNet、PST900和Potsdam四个遥感语义分割数据集上微调后，分别取得67.4% mIoU、56.9% mIoU、84.22% mIoU和91.88% mF1，均为当时最优结果。

Conclusion: 该策略有效缓解了领域鸿沟问题，提升了预训练模型的通用性，为构建适用于计算机视觉与遥感领域的统一基础模型奠定了基础。
...
TL;DR: 本文提出了一种面向越南语场景文本图像描述生成的新型多模态融合框架PhonoSTFG，强调语言学信息在视觉、OCR文本与语言知识融合中的关键作用，并发布了首个大规模越南语场景文本描述数据集ViTextCaps。


<details>
  <summary>Details</summary>
Motivation: 越南语是声调语言，变音符号影响词义，OCR错误普遍且词边界模糊，现有将文本视为语言无关的融合方法效果不佳，因此需要融入语言结构知识的多模态融合。

Method: 提出HSTFG（异构场景文本融合图）作为通用图融合框架，并通过拓扑分析发现跨模态图边对场景文本融合有害；进而设计专用于越南语语言推理的PhonoSTFG（语音学场景文本融合图），并构建首个大规模越南语场景文本描述数据集ViTextCaps。

Result: 揭示了跨模态图边在场景文本融合中的负面作用；PhonoSTFG在越南语场景文本描述任务上取得更好性能；ViTextCaps包含15,729张图像、74,970条描述，52.8%词汇存在变音符号冲突风险。

Conclusion: 越南语场景文本描述需语言学引导的多模态融合；PhonoSTFG验证了融入语音/正字法知识的有效性；ViTextCaps为该领域提供了重要基准资源。
...
TL;DR: 本文提出了一种名为Flatness-aware Prompt Pretraining (FPP)的新预训练框架，用于提升测试时提示调优（TPT）的校准性与性能。FPP通过在更平坦的损失曲面区域初始化提示，无需额外标注数据或计算开销，即可在不修改现有TPT流程的前提下显著改善模型校准与准确率。


<details>
  <summary>Details</summary>
Motivation: 现有TPT方法虽能提升视觉语言模型适应性，但常导致模型预测校准性差；已有正则化方法虽改善校准却牺牲性能，作者发现其本质是隐式推动优化至更平坦极小值，故提出从预训练阶段即引导提示初始化至平坦区域。

Method: 提出Flatness-aware Prompt Pretraining（FPP），在预训练阶段显式优化提示以落入损失曲面更平坦的区域，作为TPT的初始化起点；整个方法仅替换原有TPT的提示初始化，其余流程保持不变。

Result: FPP在多个基准上显著提升TPT的校准误差（ECE）和分类准确率，且无需标注数据、不增加测试时计算成本；消融实验验证平坦性初始化对校准与性能提升的关键作用。

Conclusion: 损失曲面的平坦性是决定TPT校准质量的关键因素；FPP通过平坦感知的预训练初始化，实现了校准性与性能的协同提升，为高效可靠的测试时适应提供了新范式。
...
Method: 提出可微分知识单元（DKU），通过隐式学习的概念分类器输出概率和主任务分类概率，利用蕴含规则与模糊推理生成逻辑调整向量，调制主分类logits；构建双向逻辑关系规则库，并约束概念间及概念与类别间的区分性以增强监督信号。

Result: 在PASCAL-VOC、COCO和MedMNIST数据集上验证了该方法的有效性，提升了模型性能；域泛化与难样本消融实验表明，所提隐式知识发现与集成优于基线方法。

Conclusion: DKU实现了无需概念标签的知识发现与可微分整合，增强了模型泛化能力，为领域知识嵌入提供了新范式。
...
TL;DR: 本文提出了一种名为GourNet的轻量级CNN模型，用于芒果叶病害识别，在MangoLeafBD数据集上达到97%准确率。
...
Method: 提出基于卷积神经网络的轻量级深度学习模型GourNet；使用MangoLeafBD（MBD）数据集（含8类：7种病害+健康）；图像经调整尺寸、归一化和数据增强预处理；数据按80%训练、10%验证、10%测试划分。
...
TL;DR: 本文提出PRISM三阶段训练流程，通过在监督微调（SFT）与基于可验证奖励的强化学习（RLVR）之间插入一个显式的分布对齐阶段，缓解大模型微调中的分布漂移问题；该阶段采用基于在线策略蒸馏（OPD）的黑盒响应级对抗博弈，利用感知与推理双专家混合专家（MoE）判别器提供解耦校正信号，并辅以高质量人工筛选数据，显著提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准的大规模多模态模型（LMMs）后训练流程（SFT→RLVR）存在分布漂移问题：SFT既损害原始能力，又无法准确匹配监督分布；该问题在多模态推理中尤为严重，因感知错误与推理失败具有不同漂移模式并在RL中叠加恶化。

Method: 提出PRISM三阶段流程：SFT → 基于在线策略蒸馏（OPD）的分布对齐（黑盒响应级对抗博弈，使用感知/推理双专家MoE判别器）→ RLVR；并构建113K高保真演示数据（来自Gemini 3 Flash），强调密集视觉定位与最难未解题的逐步推理。

Result: 在Qwen3-VL模型上实验表明，PRISM在多种RL算法（GRPO/DAPO/GSPO）和多模态基准上持续提升RLVR性能：4B和8B模型平均准确率分别较SFT→RLVR基线提升+4.4和+6.0分。

Conclusion: 显式分布对齐是提升多模态模型后训练效果的关键环节；PRISM通过解耦感知与推理的对抗校正机制及高质量数据，有效缓解漂移，为LMMs高效对齐提供了新范式。
...
TL;DR: 本文提出了一种改进的SISA框架，用于在CNN中实现类级别机器遗忘，通过强化重放机制和门控网络提升选择性遗忘效率，在保持模型性能的同时降低再训练开销。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型对私有用户数据依赖增加，用户要求删除其数据后的影响成为伦理与法律挑战，机器遗忘旨在无需完全重新训练即可移除特定数据的影响。

Method: 改进SISA（分片、隔离、切片与聚合）框架，引入强化重放机制和门控网络，支持CNN架构下的类级别遗忘。

Result: 在多个图像数据集和CNN配置上的实验表明，该方法能有效实现类级别遗忘，同时维持模型性能并显著减少再训练开销。

Conclusion: 基于SISA的类级别遗忘方法具有在隐私敏感AI应用中落地的潜力，代码已开源。
...
TL;DR: 本文提出VPDR，一种客户端隐私插件，用于原型基础的个性化联邦学习（ProtoPFL），通过方差自适应原型扰动（VPP）和蒸馏引导裁剪正则化（DCR）提升隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有ProtoPFL中使用的各向同性高斯原型扰动（IGPP）存在过扰动判别维度、难以平衡裁剪阈值与表征保真度的问题；且直接共享原型有隐私风险。

Method: 提出VPDR插件，包含两个核心组件：1）方差自适应原型扰动（VPP），依据类别维度方差动态分配噪声；2）蒸馏引导裁剪正则化（DCR），使特征范数自适应集中于裁剪阈值附近并保持预测一致性；理论证明其群组机制在相同隐私约束下不弱于各向同性基线。

Result: 在多领域基准上实验表明，VPDR在个性化联邦微调中显著优于IGPP，在保障隐私的同时提升模型效用，并保持对现实攻击的鲁棒性。

Conclusion: VPDR通过判别性维度噪声优化与自适应裁剪机制，有效缓解ProtoPFL中的隐私-效用权衡难题，为安全高效的多领域个性化联邦学习提供了新思路。
...
TL;DR: 本文提出了一种轻量级'在线架构'策略，通过在CNN中插入全局平均池化（GAP）层来增强平移不变性，显著减少参数量和模型大小，同时提升平移鲁棒性和图像质量评估性能。


<details>
  <summary>Details</summary>
Motivation: 标准CNN虽被假定为平移不变，但实际对单像素位移极为敏感，源于全连接层的空间依赖性，亟需更鲁棒且高效的架构设计。

Method: 在VGG-16等网络中不同深度插入全局平均池化（GAP）层，解耦特征识别与空间位置；并将其应用于LPIPS框架以提升感知图像质量评估的泛化能力。

Result: 参数量减少98%（5.2M→82K），模型大小减少90%（138M→14M）；ImageNet Top-1准确率保持66.4%，平移鲁棒性翻倍（相对损失从0.09降至0.05）；LPIPS改进版在KADID-10k和RAID数据集上Spearman相关系数分别达0.89和0.95。

Conclusion: 强制架构层面的平移不变性比传统数据增强更高效、更符合生物合理性，是实现鲁棒性的更优路径；但离散池化引入的周期性混叠限制了像素级完全稳定。
...
TL;DR: 本文提出FGINet模型，通过频域掩码编码、分层门控频率注入和超球面紧凑学习，提升AI生成图像检测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法在跨生成模型泛化上表现不佳，主要由于频域捷径偏差和语义-频域表征冲突。

Method: 提出Frequency-aware Gated Injection Network（FGINet），包含Band-Masked Frequency Encoder（BMFE）、Layer-wise Gated Frequency Injection（LGFI）和Hyperspherical Compactness Learning（HCL）。

Result: FGINet在多个挑战性数据集上达到SOTA性能，展现出强泛化能力。

Conclusion: 融合频域鲁棒建模与视觉基础模型层次化特征对提升生成图像检测泛化性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感深度学习模型在时间一致性、细粒度空间结构建模、预训练需求和可解释性方面存在不足；而扩散模型中系统利用高斯噪声学习表征的能力启发作者探索其在判别任务中的潜力。

Method: 提出Noise2Map框架：利用自监督去噪预训练+有监督微调；采用任务特定噪声调度与时间步条件控制；共享骨干网络，分别适配语义分割（SS）和变化检测（CD）任务；跳过传统扩散模型的迭代采样，直接预测目标图。

Result: 在SpaceNet7、WHU和xView2火灾损毁建筑数据集上，Noise2Map平均排名为7个对比模型中第1（语义分割）和第1（变化检测，按跨数据集F1主指标+IoU平局打破规则）；消融实验证明其对噪声调度与时间步控制鲁棒，并支持多任务学习。

Conclusion: 扩散模型的噪声过程可有效迁移到判别式遥感任务中；Noise2Map实现了高效、可解释、鲁棒且统一的语义分割与变化检测，为扩散模型在非生成任务中的应用提供了新范式。
...
TL;DR: 本文提出HiMix框架，通过混合增强和分层伪影感知表示提升合成图像检测的泛化能力。
...
Method: 提出HiMix框架，包含Mixup驱动的分布增强（MDA）模块和分层伪影感知表示（HAR）模块：MDA通过构造真实与伪造图像间的连续过渡样本并进行像素级mixup来扩展训练分布；HAR通过跨层整合与粗到细特征融合聚合全局与局部伪影信息。

Result: 在多个基准测试中，HiMix达到当前最优性能，实现了对未见伪造图像的良好泛化，并产生更分离的logits。

Conclusion: HiMix通过扩展训练分布和构建伪影感知表征，显著提升了合成图像检测模型的泛化能力与鲁棒性。
...
TL;DR: 本文提出TAVR框架，通过跨场景视频参考生成高保真说话头像，突破传统单图参考的局限性，结合三阶段训练策略（同场景预训练、跨场景微调、强化学习对齐）提升身份一致性和背景适应性，并构建新基准验证其鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有说话头像方法依赖静态单图参考，缺乏时序和表情线索，难以在自定义背景下生成高保真结果。

Method: 提出TAVR框架，采用跨场景视频作为输入，集成token选择模块与三阶段训练：同场景视频预训练（外观复制）、跨场景参考微调（域适应）、任务特定强化学习（身份相似性奖励对齐）。

Result: 在自建158对跨场景视频基准上实验表明，TAVR在定量与定性指标上均超越现有基线，支持灵活推理时视频引用，且已部署至生产环境。

Conclusion: TAVR成功将说话头像生成从单图参考范式拓展至跨场景视频参考，显著提升了生成质量、身份一致性和背景泛化能力，具有实际应用价值。
...
TL;DR: 本文提出了一种无需训练的隧道缺陷检测框架TunnelMIND，通过语言引导与密集视觉一致性重校准，将粗粒度语义提议转化为可靠掩码，并进一步结构化为含类别、位置、几何、严重性及上下文属性的缺陷实体，最终生成符合专家知识约束的工程报告。


<details>
  <summary>Details</summary>
Motivation: 现有无训练基础模型在隧道等干扰严重场景中仅能提供粗粒度开放词汇提议，难以直接支持缺陷定位、测量、分级和工程文档等实际需求。

Method: 提出TunnelMIND框架：1）利用语言引导生成缺陷提议；2）在推理时通过密集视觉一致性对空间支持进行重校准；3）将掩码重构为结构化缺陷实体（含类别、位置、几何、严重性、上下文）；4）结合检索增强解释与专家知识约束生成工程可读报告。

Result: 在可见光、探地雷达（GPR）和路面缺陷任务上F1分数分别达0.68、0.78、0.72。

Conclusion: TunnelMIND证明了无训练隧道检测可从粗略定位迈向面向工程评估的结构化缺陷证据生成。
...
TL;DR: 本文提出了一种动态聚类采样方法（DynamiCS），通过每轮训练动态地对大数据簇降采样、小数据簇升采样，以改善视觉-语言模型（VLM）训练中长尾概念表征不足的问题，在降低计算成本的同时提升长尾性能。
...
Method: 提出动态聚类采样方法（DynamiCS）：每轮训练时对语义聚类进行尺度调整——对大簇降采样、小簇升采样，保持聚类相对顺序并强调长尾；区别于仅追求语义分布扁平化的现有方法。

Result: 实验表明，DynamiCS可降低VLM训练计算成本，并显著提升模型对长尾概念的识别与理解性能。

Conclusion: 动态、聚类感知的采样策略比静态或单纯分布均衡的方法更有利于长尾概念学习，为高效且公平的VLM训练提供了新范式。
...
TL;DR: 本文提出了TripVVT-10K数据集、TripVVT模型和TripVVT-Bench评测基准，旨在提升视频虚拟试衣在真实场景下的性能与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频虚拟试衣模型受限于大规模野外三元组数据稀缺及掩码使用不当，导致性能受限。

Method: 构建了首个大规模野外三元组视频数据集TripVVT-10K；提出基于扩散Transformer的TripVVT框架，用稳定的人体掩码先验替代易出错的服装掩码；建立涵盖多样场景的TripVVT-Bench评测基准。

Result: TripVVT在视频质量、试穿保真度及对复杂野外视频的泛化能力上优于当前学术与商用系统。

Conclusion: TripVVT系列资源（数据集、模型、评测基准）为可控、真实且时序稳定的视频虚拟试衣研究提供了坚实基础，并已开源。
...
TL;DR: 本文探讨了在社交媒体短视频中进行自动化视觉主题检测的方法，评估了多种视觉语言模型（VLMs）的零样本分类能力，并提出将聚类建模为最小代价多割问题以实现无监督模式发现；结果显示当前VLMs尚不能有效识别气候变化相关类别，但DINOv2和ConvNeXt V2图像嵌入模型能生成有意义的视觉聚类。


<details>
  <summary>Details</summary>
Motivation: 数字内容尤其是社交媒体短视频的激增改变了公共话语中话题的讨论与理解方式，亟需发展适用于此类数据的自动化视觉主题检测方法。

Method: （1）采用零样本图像分类评估VideoChatGPT、PandaGPT、VideoLLaVA等VLMs，并与帧级CLIP基线对比；（2）将聚类建模为最小代价多割问题，对ConvNeXt V2和DINOv2等图像嵌入模型进行无监督聚类分析。

Result: VLMs目前无法准确识别气候变化相关视觉类别；DINOv2聚类更侧重风格与抽象语义差异，ConvNeXt V2则呈现更细粒度的区分能力；两者均产生具有判别性的视觉聚类结果。

Conclusion: 尽管当前VLMs在特定领域（如气候变化）视觉理解上存在局限，基于图像嵌入的无监督聚类仍可有效揭示短视频中的视觉主题结构，为实践者提供了可行的技术路径与实用建议。
...
Method: 构建包含2209个实例的FineState-Bench基准（涵盖桌面/网页/移动端、4类交互、23种UI组件），提出四阶段诊断指标（SR@Loc、SR@Int、ES-SR@Loc、ES-SR@Int），并设计可插拔的可视化诊断助手（VDA）提供描述与定位提示以辅助视觉接地分析。

Result: 在FineState-Bench上，精确目标状态成功率（ES-SR@Int）最高仅32.8%（网页端），跨平台平均为22.8%；加入VDA定位提示后，Gemini-2.5-Flash提升14.9个百分点，表明视觉接地仍有较大提升空间。

Conclusion: 细粒度状态驱动GUI交互仍是重大挑战；FineState-Bench和FineState-Metrics为该领域提供了更精细、可诊断的评估范式，VDA验证了视觉接地优化的有效性，但整体性能仍不足以支撑可靠部署。
...
TL;DR: 本文提出Shot Transition Detection (STD)任务，通过TransVLM模型显式检测视频中连续的转场时间段，而非传统仅定位切点的Shot Boundary Detection；模型融合光流运动先验与颜色信息，提升时序感知能力，并构建合成数据引擎与新基准以解决类别不平衡问题，性能超越多种方法并已落地应用。


<details>
  <summary>Details</summary>
Motivation: 传统Shot Boundary Detection（SBD）将任务建模为孤立切点检测，在复杂转场下易产生错误分段；需转向更鲁棒、符合真实转场结构的连续时间段检测范式。

Method: 提出Shot Transition Detection（STD）新任务；设计TransVLM视觉语言模型框架，将光流作为运动先验注入输入层，采用颜色与运动特征拼接+轻量融合策略，不增加语言主干视觉token开销；构建可扩展的转场视频合成数据引擎及STD专用基准。

Result: TransVLM在STD任务上全面超越传统启发式方法、专用时空网络及主流VLM；实验充分验证其优越性；模型已部署至生产环境。

Conclusion: STD是比传统SBD更合理、更具实用价值的视频转场建模方式；TransVLM通过显式建模运动与语义协同，为视频理解中的细粒度时序分析提供了新思路与有效工具。
...
TL;DR: 本文提出Echo-α，一种用于超声影像解读的代理式多模态推理模型，通过‘调用-推理’框架融合病灶精确定位与临床整体推理能力，在肾脏和乳腺超声多中心基准测试中显著提升定位（F1@0.5）与诊断准确率（overall accuracy），并增强模型可解释性与跨中心泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在超声解读中难以兼顾精确病灶定位（专用检测器强于定位但弱于推理）与整体临床推理（MLLMs强于推理但医学领域接地弱），亟需统一二者优势的新范式。

Method: 提出Echo-α模型，采用‘调用-推理’框架：协调器官特异性检测器输出、融合全局视觉上下文，并将证据转化为有依据的诊断决策；通过九任务监督课程学习初始化，再以不同奖励权衡进行序列强化学习，衍生出专注病灶锚定的Echo-α-Grounding和专注最终诊断的Echo-α-Diagnosis。

Result: 在多中心肾/乳腺超声数据集上，Echo-α-Grounding在跨中心测试集达56.73%/43.78% F1@0.5，Echo-α-Diagnosis达74.90%/49.20%总体准确率，均优于竞争基线。

Conclusion: 代理式多模态推理可将专用检测器输出转化为可验证的临床证据，为构建更准确、可解释、可迁移的超声AI系统提供了可行路径。
...
Method: 引入结合结构张量与拉普拉斯尺度空间的多尺度频率分析，定义每高斯、每轴的频率违规度量η，并据此进行各向异性分裂；同时加入多视角一致性准则聚合η观测值，实现早期快速稠密化。

Result: 在标准基准上实验表明，该方法收敛更快、重建质量更优，尤其在高频率区域表现突出。

Conclusion: 结构感知的稠密化策略有效解耦了几何精度与纹理保真度，为实时新视角合成提供了更高效、高质量的高斯表示优化范式。
...
TL;DR: 本文提出了一种新的DeepFake检测评估框架，引入'真实音频-真实视频但语义不一致（RARV-SMM）'类别，揭示现有模型在语义层面不一致伪造内容上的检测缺陷，并提出基于ImageBind的语义强化策略提升检测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有DeepFake检测多为二分类或四类音频-视频操纵类型分类，但忽视了当音视频本身真实却存在语义不一致（如真实人脸与不匹配的真实语音）时的伪造情形，这类语义级伪造更具隐蔽性和现实挑战性。

Method: 构建包含RARV-SMM新类别的扩展四类评估设定；设计三种RARV-SMM变体以暴露模型对音视频语义偏离的敏感性；提出融合ImageBind多模态语义嵌入与RARV-SMM监督的语义强化训练策略。

Result: 在FakeAVCeleb和LAV-DF数据集上验证：主流模型在RARV-SMM样本上性能显著下降；所提语义强化策略有效提升各类别（尤其RARV-SMM）检测准确率，并增强跨数据集泛化能力。

Conclusion: 语义不一致是DeepFake检测中被长期忽视的关键维度；仅依赖源完整性判断不可靠；引入语义一致性建模并结合强语义表征（如ImageBind）是构建更现实、鲁棒检测器的必要路径。
...
TL;DR: 本文提出ResiHMR，首个面向截肢人群的单图像三维人体网格重建框架，通过残肢关键点、拓扑自适应锚点优化与几何驱动的残肢表面重建，实现解剖结构一致且符合假肢生物力学的建模。


<details>
  <summary>Details</summary>
Motivation: 现有单图像人体网格恢复方法基于完整肢体先验，在截肢者上性能下降，因其固定拓扑模型无法表达残肢结构。

Method: ResiHMR引入残肢关键点，包含两个核心模块：(i) 拓扑自适应的残肢锚点-因子优化模块，约束估计于观测到的解剖有效运动学子图；(ii) 基于几何的残肢重建模块，估计残肢边界与凸形末端几何。

Result: 在真实截肢图像数据集上，ResiHMR在SMPLify-X和HSMR骨干下均提升重建质量：SMPLify-X下的完整关节2D MPJPE从41.32降至37.40，HSMR下的残肢2D MPJPE从73.61大幅降至23.19。

Conclusion: ResiHMR是首个显式重建残肢表面并进行拓扑自适应优化的单图像HMR系统，更贴合假肢生物力学与实际应用需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有大多数学习型编解码器针对固定率失真点优化，难以高效进行码率自适应传输，需重复编码或多码流存储，成本高。

Method: 提出TAFA-GSGC框架，融合分层残差精炼、通道组熵编码，并引入目标对齐特征聚合（TAFA）模块以降低增强层残差间的跨层冗余。

Result: 支持最多9级单调质量提升的可伸缩解码；在D1和D2指标上相较PCGCv2平均BD-Rate分别节省-4.99%和-5.92%。

Conclusion: TAFA-GSGC实现了单模型单比特流下的高效多质量点云几何压缩，在保持高压缩效率的同时显著提升带宽自适应能力。
...
TL;DR: This paper reviews 3D reconstruction techniques in manufacturing, highlighting a gap in unified frameworks, classifying methods into data acquisition, point cloud generation, and post-processing/applications, and noting the growing role of deep learning and hybrid sensor systems.


<details>
  <summary>Details</summary>
Motivation: To identify research gaps and provide a structured understanding of current 3D reconstruction techniques in manufacturing, especially amid rapid advances in deep learning and sensor technologies.

Method: Systematic review of 106 recent publications, classification of techniques into three categories (data acquisition, point cloud generation, post-processing/applications), and analysis of adoption trends, accuracy, and application domains.

Result: Non-contact methods (e.g., structured light, stereo vision) dominate, especially for quality inspection (40%); deep learning improves accuracy and speed; sub-millimeter accuracy is achievable in controlled settings but challenges remain with reflective/dynamic scenes; hybrid multi-sensor systems are emerging as a trend.

Conclusion: There is a critical need for unified 3D reconstruction frameworks in manufacturing; hybrid systems combining sensors and AI methods represent the most promising direction for overcoming current limitations.
...
TL;DR: 本文提出了一种用于评估视频美学的分层评分框架AesVideo-Bench，并构建了视频美学奖励模型AesRM（含Base与CoT两个版本），通过三阶段渐进式训练显著提升了美学评估的准确性、鲁棒性与可解释性，并在实际视频生成模型（Wan2.2）对齐中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成研究过度关注视觉保真度，忽视电影级美学（如色彩协调、影调设计）；而图像美学研究定义粗略、缺乏系统评估体系，亟需面向视频的细粒度、可量化的美学评估方法。

Method: 提出包含视觉美学（VA）、视觉保真度（VF）、视觉合理性（VP）三大维度、共15项细粒度标准的分层美学评估框架；构建专家标注的大规模偏好数据集AesVideo-Bench；设计AesRM-Base（直接预测偏好）与AesRM-CoT（生成符合15条标准的思维链）两类奖励模型，并采用三阶段训练策略（原子能力学习→冷启动→GRPO），其中AesRM-CoT引入自一致性CoT合成与CoT过程奖励。

Result: AesRM在多个美学基准上超越基线模型，位置偏差更低、鲁棒性更强；与Wan2.2模型对齐后，生成视频在美学质量上明显优于现有美学奖励方法。

Conclusion: 本工作建立了首个系统化、可扩展的视频美学评估与建模范式，为生成式视频向专业级美学表现演进提供了关键基础设施与技术路径。
...
TL;DR: 本文提出了一种不确定性感知的超图精炼网络（UHR-Net），通过不确定性导向的实例对比预训练和不确定性引导的超图精炼模块，提升病灶分割在边界模糊与小病灶场景下的鲁棒性与精度。


<details>
  <summary>Details</summary>
Motivation: 病灶常与周围组织相似、边界不清，导致分割模型在边界/过渡区域预测不稳定；多尺度特征提取易稀释小病灶线索，引发欠分割或过分割。

Method: 1）提出不确定性导向的实例对比（UO-IC）预训练策略，结合几何感知复制粘贴增强与病灶样背景难负样本挖掘；2）设计不确定性引导的超图精炼（UGHR）模块，利用熵图指导超图中前景/背景原型分离，解耦高阶交互以优化模糊区域分割。

Result: 在五个公开基准上一致优于强基线模型。

Conclusion: UHR-Net有效提升了对边界不清和小病灶的分割鲁棒性与准确性，验证了不确定性建模与超图结构建模在医学图像分割中的协同价值。
...
TL;DR: 本文提出S²VAE，一种以几何为先的潜在学习框架，通过使用Power Spherical分布的变分自编码器，显式建模场景的3D状态（如相机运动、深度、点结构），在深度估计、相机姿态恢复和点云重建任务中优于传统高斯潜变量，尤其在高压缩率下。


<details>
  <summary>Details</summary>
Motivation: 现有视觉世界建模系统虽能生成合理运动，但常无法保持底层3D几何或物理一致的相机动力学；关键限制在于用于编码几何结构的潜在表示，而非模型容量本身。

Method: 提出S²VAE框架，基于Visual Geometry Grounded Transformer（VGGT）表征，设计一种新型变分自编码器，其潜在空间采用Power Spherical分布的乘积形式，强制瓶颈层具有超球面结构以保留方向与几何语义。

Result: 在深度估计、相机姿态恢复和点云重建任务上，几何对齐的超球面潜在表示持续优于传统高斯瓶颈，尤其在高压缩率条件下表现更优。

Conclusion: 潜空间的几何结构应作为物理可解释视觉与世界建模的首要设计原则。
...
TL;DR: 本文提出AdvDMD方法，将分布匹配蒸馏（DMD）与强化学习（RL）无缝统一，利用对抗训练的判别器作为奖励模型，在中间和最终去噪状态上联合训练，并在线更新，从而在极少采样步数下显著提升生成质量，甚至超越原教师模型。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成质量高但采样步数多；现有蒸馏方法（如DMD）在步数受限时性能下降明显；已有结合RL的蒸馏方法存在组合式设计带来的不必要复杂性。

Method: 提出AdvDMD：1）以DMD2中对抗训练的判别器作为奖励模型；2）在去噪过程的中间态与终态联合监督；3）判别器与蒸馏模型在线协同更新；4）采用统一SDE反向模拟及差异化训练调度以提升稳定性与效率。

Result: 在DPG-Bench上，4步AdvDMD超越SD3.5原40步模型；在GenEval上，SD3的AdvDMD显著提升性能；在Qwen-Image上，2步AdvDMD优于TwinFlow。

Conclusion: AdvDMD通过深度融合DMD与RL，实现了更稳定、高效的少步蒸馏，在多个基准和模型上验证了其优越性与泛化能力。
...
TL;DR: 本文提出首个端到端可学习的任意骨架运动捕捉框架，联合优化视频到姿态和姿态到旋转两个阶段，通过引入目标资产的参考姿态-旋转对解决旋转歧义性，显著降低旋转误差并提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有单目视频运动捕捉方法采用分步式流程（视频→关节点位置→逆运动学求旋转），存在旋转歧义（如骨骼扭转）和非可微IK阻碍端到端优化的问题。

Method: 提出端到端可学习框架，引入目标资产的参考姿态-旋转对与绑定姿势共同定义旋转坐标系；摒弃网格中间表示，直接从视频预测关节点位置；两阶段共享骨架感知的全局-局部图引导多头注意力（GL-GMHA）模块。

Result: 在Truebones Zoo和Objaverse数据集上，旋转误差从约17度降至约10度，在未见骨架上达6.54度；推理速度比基于网格的方法快约20倍。

Conclusion: 该方法通过坐标系锚定和联合优化，有效解决了姿态到旋转映射的歧义性，实现了高精度、高效率、泛化性强的任意骨架运动捕捉。
...
TL;DR: 本文提出3D-ReGen，一种基于初始3D形状条件的3D再生模型，支持增强、重建与编辑等任务，采用VecSet新条件机制，在无需额外标注下通过自监督学习实现高质量可控3D生成。
...
Method: 提出3D-ReGen模型，引入基于VecSet的新型条件机制，利用自监督预训练任务和数据增强，从现成3D数据集中学习通用再生先验。

Result: 在几何一致性与细粒度质量两方面均取得优异表现，在多个可控3D生成任务上达到SOTA性能。

Conclusion: 3D-ReGen提供了一种简单而强大的可控3D再生范式，显著拓展了3D内容生成的实用性与灵活性。
...
TL;DR: 本文提出pHVI-ISPNet，一种基于HVI色彩空间的RAW-to-RGB夜间摄影渲染框架，通过多项关键改进提升感知质量，在NTIRE 2025 NPR挑战数据集上实现CIE2000和LPIPS指标的新SOTA。
...
Method: 提出基于HVI色彩空间的pHVI-ISPNet框架，包含：RAW域特征处理与小波特征传播、样本级动态损失系数、基于特征分布的损失项以保障色彩恒常性。

Result: 在NTIRE 2025 NPR挑战数据集上，达到具有竞争力的保真度，并在CIE2000色差和LPIPS指标上刷新SOTA。
...
TL;DR: 本文提出了一种可微分的简单点检测方法，用于在连续值图像上进行拓扑保持学习，并设计了骨架提取算法和变分模型以增强深度学习分割结果的拓扑一致性。


<details>
  <summary>Details</summary>
Motivation: 现有简单点检测方法局限于二值图像且不可微，难以融入基于梯度的深度学习；形态学与纯数据驱动方法常无法保证拓扑一致性。

Method: 提出连续域简单点的可微定义与计算方法；构建高效拓扑保持骨架提取算法；设计基于保留非简单点的变分模型，可嵌入任意带softmax/sigmoid输出的分割网络。

Result: 在多个基准上显著提升了分割结果的拓扑完整性与结构精度；代码已开源。

Conclusion: 所提方法实现了端到端可微的拓扑约束建模，为图像分割与骨架化等任务提供了理论可靠、实践有效的拓扑保持解决方案。
...
TL;DR: PhyCo 是一个将物理一致性引入视频扩散模型的框架，通过物理模拟数据集、物理监督微调和视觉语言模型引导的奖励优化，实现无需仿真器即可生成物理上合理且可控的视频。


<details>
  <summary>Details</summary>
Motivation: 现代视频扩散模型在外观合成方面表现出色，但在物理一致性方面存在不足，如物体漂移、碰撞反弹不真实、材料响应与物理属性不匹配等。

Method: PhyCo 包含三个核心组件：(i) 包含10万+张光真实感物理仿真视频的大规模数据集，系统变化摩擦、恢复系数、形变和力等参数；(ii) 基于ControlNet对预训练扩散模型进行物理监督微调，以像素对齐的物理属性图作为条件；(iii) 利用微调后的视觉语言模型（VLM）进行奖励优化，对生成视频进行物理相关问答并提供可微反馈。

Result: 在Physics-IQ基准测试中，PhyCo显著优于强基线模型；人类评估证实其对物理属性的控制更清晰、更忠实。

Conclusion: PhyCo 展示了一条可扩展路径，使生成式视频模型具备物理一致性与可控性，并能泛化到合成训练环境之外。
...
TL;DR: 本文提出A4Mer，一种自监督的嵌套式Transformer模型，用于从3D姿态数据中学习人类行为的层次化表示（Action Atoms与Action Motifs），并构建了多视角、带SMPL标注的大规模Action Motif Dataset（AMD）以支持研究。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对人类动作内在组成性（如关节运动的可复用时序模式）的有效建模，需一种能自动发现语义化、可迁移运动片段的层次化表示。

Method: 提出A4Mer：基于分段-聚合的嵌套Transformer架构，将3D姿态序列划分为可变长片段并编码为Action Atoms（底层隐变量），再通过自监督掩码预测任务学习其时序组合形成的Action Motifs（高层隐变量）；同时构建AMD数据集，创新采用足部摄像头实现强遮挡下的帧级SMPL标注。

Result: A4Mer成功提取出语义清晰、跨动作可复用的Action Motifs，在动作识别、运动预测和运动插值任务上均取得显著性能提升。

Conclusion: 层次化、自监督学习的动作表示（Action Atoms + Action Motifs）是建模人类行为组成性的有效途径，A4Mer及其配套AMD数据集为后续行为理解提供了新范式与基础资源。
...
TL;DR: AEGIS是一个用于评估AI生成学术图像鉴伪能力的综合性基准，涵盖领域复杂性、伪造模拟多样性和多维鉴伪评估三个关键进步，揭示了当前模型在学术图像鉴伪上的根本局限。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分反映AI生成学术图像鉴伪的真实挑战，亟需一个更全面、更具针对性的评估基准。

Method: 构建AEGIS基准，包含七个学术类别、39个细分子类；模拟四种常见学术伪造策略，覆盖25种生成模型；从检测、推理和定位三方面进行多维评估，并评测25个主流MLLM、9个专家模型及1个统一多模态模型。

Result: GPT-5.1整体性能仅48.80%，专家模型定位IoU仅30.09%；11种生成模型下鉴伪准确率低于50%；MLLM在文本伪影识别达84.74%，专家检测器在二值真实性检测达79.54%。

Conclusion: AEGIS揭示了当前AI生成学术图像鉴伪技术严重滞后于生成技术发展，且不同模型家族在不同任务上呈现互补优势，亟需跨维度协同提升。
...
<details>
  <summary>Details</summary>
Motivation: 支气管镜导航中CT与体内解剖结构因呼吸运动（5–20 mm）产生偏差，传统屏气协议难以重复且干扰临床流程。

Method: 利用配对的吸气/呼气CT隐式定义患者特异性气道变形空间，通过CT配准将呼吸运动简化为每帧单标量呼吸相位；将其嵌入mesh-anchored高斯溅射框架，用轻量RGB估计器直接从内窥镜图像推断呼吸相位，实现无屏气、无外部传感的连续重建。

Result: 在RESPIRE仿真平台实验表明：几何重建保真度高、训练速度提升20倍以上、目标定位精度达1.22 mm（满足临床3 mm要求），显著优于单CT基线方法。

Conclusion: 患者特异性呼吸建模结合端到端相位估计可消除屏气依赖，提升支气管导航的实用性与精度，为动态解剖环境下的实时导航提供了新范式。
...
TL;DR: 本文提出智能视觉生成的概念，强调从外观合成转向基于结构、动态、领域知识和因果关系的视觉生成，并构建了五级分类法来指导该领域的演进。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型在空间推理、状态持久性、长时序一致性及因果理解方面仍存在不足，需向更智能的视觉生成范式转变。

Method: 提出五级生成分类法（原子生成、条件生成、上下文内生成、代理生成、世界建模生成），并分析关键技术驱动力与评估方法缺陷。

Result: 明确了当前评估体系的局限性，指出其过度关注感知质量而忽视结构性、时序性和因果性错误；通过基准回顾、野外压力测试和专家约束案例研究，提供能力中心视角。

Conclusion: 智能视觉生成应成为未来研究重点，需结合结构化建模、因果推理与世界知识，推动生成系统从被动渲染走向主动、交互、世界感知的代理式生成。
...
TL;DR: 本文提出FD-loss方法，在表示空间中有效优化Fréchet Distance（FD），显著提升生成模型质量，并揭示FID指标的局限性，提出多表示空间评估指标FDr^k。


<details>
  <summary>Details</summary>
Motivation: Fréchet Distance（FD）虽理论上优越，但因计算开销大而长期未被用作训练目标；同时，现有评估指标（如Inception FID）可能无法准确反映真实视觉质量。

Method: 提出FD-loss：将FD估计所需的大规模样本（如50k）与梯度计算的小批量（如1024）解耦，在不同表示空间中优化FD；无需教师蒸馏、对抗训练或逐样本目标。

Result: 在ImageNet 256×256上，仅一步生成器达0.72 FID；FD-loss可将多步生成器直接转化为强单步生成器；发现FID可能误排序视觉质量，进而提出多表示空间指标FDr^k。

Conclusion: FD可在表示空间中高效优化，作为训练目标和评估指标均具潜力；应探索多样化表示空间下的分布距离，推动生成模型发展。
...
TL;DR: GenWildSplat是一种无需每场景优化的前馈式稀疏视图户外三维重建框架，利用学习到的几何先验预测深度、相机参数和3D高斯，在合成与真实数据课程学习下实现对光照变化和瞬态遮挡的良好泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖场景特定优化，需大量逐场景训练且在稀疏视角下失效；评估场景有限，泛化能力存疑。

Method: 提出GenWildSplat前馈框架：输入未标定网络图像，预测深度、相机参数及规范空间中的3D高斯；引入外观适配器调节目标光照下的外观，语义分割处理瞬态物体；通过合成与真实数据的课程学习提升鲁棒性。

Result: 在PhotoTourism和MegaScenes基准上达到最先进的前馈渲染质量，支持实时推理，无需测试时优化。

Conclusion: GenWildSplat显著提升了稀疏、未标定、户外图像下的通用三维重建能力，摆脱了对逐场景优化的依赖，增强了在复杂真实条件（如光照变化、瞬态遮挡）下的泛化性和实用性。
...
TL;DR: 本文提出HERMES++，一种统一的驾驶世界模型，融合3D场景理解与未来几何预测，通过BEV表示、LLM增强查询、当前-未来链接和联合几何优化提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在自动驾驶世界建模中偏重未来场景生成而忽视全面3D理解；大语言模型虽具强语义推理能力，却无法预测未来几何演化，导致语义理解与物理仿真脱节。

Method: 提出HERMES++框架：1）采用鸟瞰图（BEV）表征整合多视角空间信息；2）引入LLM增强的世界查询实现理解分支到预测分支的知识迁移；3）设计当前-未来链接，以语义上下文约束几何演化；4）采用联合几何优化策略，结合显式几何约束与隐式潜在正则化。

Result: 在多个基准上验证有效，在未来点云预测和3D场景理解任务中均超越专用模型。

Conclusion: HERMES++成功弥合了语义理解与几何预测之间的鸿沟，为自动驾驶世界建模提供了统一、高效且几何一致的解决方案。
